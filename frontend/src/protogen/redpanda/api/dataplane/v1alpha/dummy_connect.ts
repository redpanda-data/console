// This file is a trick to force protoc-gen-openapiv2 into including the types used here into the openapi spec. They are not normally included, because they are not explicitly referenced in any proto (as protobuf ANY is used in errordetails).

// @generated by protoc-gen-connect-es v1.0.0 with parameter "target=ts,import_extension="
// @generated from file redpanda/api/dataplane/v1alpha/dummy.proto (package redpanda.api.dataplane.v1alpha, syntax proto3)
/* eslint-disable */
// @ts-nocheck

import { Empty, MethodKind } from "@bufbuild/protobuf";
import { DummyMethodResponse } from "./dummy_pb";

/**
 * @generated from service redpanda.api.dataplane.v1alpha.DummyService
 */
export const DummyService = {
  typeName: "redpanda.api.dataplane.v1alpha.DummyService",
  methods: {
    /**
     * buf:lint:ignore RPC_REQUEST_STANDARD_NAME
     * buf:lint:ignore RPC_RESPONSE_STANDARD_NAME
     * buf:lint:ignore RPC_REQUEST_RESPONSE_UNIQUE
     *
     * @generated from rpc redpanda.api.dataplane.v1alpha.DummyService.DummyMethod
     */
    dummyMethod: {
      name: "DummyMethod",
      I: Empty,
      O: DummyMethodResponse,
      kind: MethodKind.Unary,
    },
  }
} as const;

