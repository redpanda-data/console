...
export type UpdatingConnectorData = {
	clusterName: string;
	connectorName: string;
};
export type RestartingTaskData = {
	clusterName: string;
	connectorName: string;
	taskId: number;
};
...
export class ConnectClusterStore {
	...
	async setup() {
		if (!this.isInitialized) {
			...
			this.additionalClusterInfo = api.connectAdditionalClusterInfo.get(
				this.clusterName,
			)!;
			this.features.secretStore =
				!!this.additionalClusterInfo?.enabledFeatures?.some(
					(x) => x === "SECRET_STORE",
				);
		...
		if (secrets) {
			try {
				...
				for (const [key, secret] of secrets.secrets) {
					...
					if (property) {
						property.value = secret.getSecretString(
							key,
							createSecretResponse?.secretId,
						);
	...
	deleteConnector = flow(function* (
		...
		if (secrets) {
			yield Promise.all(
				secrets.ids.map((secretId) =>
					retrier(() => api.deleteSecret(this.clusterName, secretId), {
						attempts: 3,
						delayTime: 200,
					}),
				),
	...
	updateConnnector = flow(function* (
		this: ConnectClusterStore,
		connectorName: string,
	) {
		...
		if (newConfigObj) {
			...
			if (remoteConnector)
				yield api.updateConnector(
					this.clusterName,
					connectorName,
					connectorState.getConfigObject(),
				);
		}
	});
	...
	getConnector(
		...
		const identifier = connectorName
			? `${pluginClassName}/${connectorName}`
			: pluginClassName;
		let connectorStore = this.connectors.get(identifier);
		if (!connectorStore) {
			const connectorType = this.additionalClusterInfo.plugins.first(
				(x) => x.class == pluginClassName,
			)!.type;
			connectorStore = new ConnectorPropertiesStore(
				this.clusterName,
				pluginClassName,
				connectorType,
				initialConfig,
	...
	get cluster(): ClusterConnectors | null {
		if (this.isInitialized) {
			const cluster = api.connectConnectors?.clusters?.first(
				(c) => c.clusterName == this.clusterName,
			);
	...
	validateConnectorState(
		connectorName: string,
		state: ConnectorPossibleStatesLiteral[],
	) {
		if (this.isInitialized) {
...
export class SecretsStore {
	...
	get ids(): string[] {
		return Array.from(this.secrets, ([_key, secret]) => secret.id).filter(
...
export class ConnectorPropertiesStore {
		...
		return observable<PropertyGroup>({
			...
			get filteredProperties(): Property[] {
				...
				return this.properties.filter(
					(p) => p.entry.definition.importance == PropertyImportance.High,
				);
			},
		});
	}
	...
	async initConfig() {
		...
		try {
			...
			if (this.appliedConfig == null) {
				...
				for (const p of allProps)
					if (p.errors.length > 0) p.lastErrorValue = p.value;
			}
				...
				for (const groupDef of step.groups) {
					const groupProps = groupDef.config_keys
						.map((k) => {
							...
							if (!prop)
								console.log(
									"step[*].group[*].config_keys references a property that does not exist in propsByName!",
									{
										step: step.name,
										group: groupDef,
					...
					this.allGroups.push(
						this.createPropertyGroup(step, groupDef, groupProps),
					);
				}
			}
			...
			for (const g of this.allGroups)
				for (const p of g.properties) p.propertyGroup = g;

			// Notify groups about errors in their children
			for (const g of this.allGroups)
				g.propertiesWithErrors.push(
					...g.properties.filter((p) => p.showErrors),
				);
	...
	async validate(config: object) {
		...
		try {
			...
			const removedProps = [...this.propsByName.keys()].filter(
				(key) => !srcNames.has(key),
			);
			...
			for (const source of srcProps) {
				...
				if (!target.errors.isEqual(source.errors)) {
					...
					if (target.errors.length > 1) {
						...
						const betterStartValue = target.errors.findIndex(
							(x) => !x.includes("which has no default value"),
						);
			...
			// Set last error values, so we know when to show the validation error
			for (const g of this.allGroups)
				for (const p of g.properties) p.lastErrorValue = p.value;
		} catch (err: any) {
	...
	createCustomProperties(properties: ConnectorProperty[]): Property[] {
		...
		const allProps = properties
			.map((p) => {
				...
				const defaultValue: any = sanitizeDefaultValue(
					p.definition.default_value,
					definitionType,
				);
				...
				if (this.appliedConfig != null && this.appliedConfig[name])
					property.value = sanitizeValue(
						this.appliedConfig[name],
						definitionType,
					);
