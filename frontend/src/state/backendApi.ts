...
const apiStore = {
	...
	topicPartitionErrors: new Map<
		string,
		Array<{ id: number; partitionError: string }>
	>(),
	topicWatermarksErrors: new Map<
		string,
		Array<{ id: number; waterMarksError: string }>
	>(),
	...
	partitionReassignments: undefined as
		| PartitionReassignments[]
		| null
		| undefined,
	...
	debugBundleStatuses:
		[] as GetDebugBundleStatusResponse_DebugBundleBrokerStatus[],
				...
				api.userData = {
					...
					user: {
						...
						meta: {
							avatarUrl: "",
							email: "",
							name: "local fake user for debugging",
						},
					},
				};
	...
	refreshTopics(force?: boolean) {
		cachedApiRequest<GetTopicsResponse>(
			`${appConfig.restBasePath}/topics`,
			force,
		).then((v) => {
			if (v?.topics != null) {
	...
	async getTopicOffsetsByTimestamp(
		topicNames: string[],
		...
		const response = await appConfig.fetch(
			`${appConfig.restBasePath}/topics-offsets?${query}`,
			{
				method: "GET",
		...
		const r = await parseOrUnwrap<GetTopicOffsetsByTimestampResponse>(
			response,
			null,
		);
	...
	refreshTopicDocumentation(topicName: string, force?: boolean) {
		cachedApiRequest<TopicDocumentationResponse>(
			...
			const text =
				v.documentation.markdown == null
					? null
					: decodeBase64(v.documentation.markdown);
	...
	async deleteTopic(topicName: string) {
		return rest(
			`${appConfig.restBasePath}/topics/${encodeURIComponent(topicName)}`,
			{ method: "DELETE" },
		).catch(addError);
	},

	async deleteTopicRecords(
		topicName: string,
		offset: number,
		partitionId?: number,
	) {
		const partitions =
			partitionId != undefined
				...
				: this.topicPartitions
						?.get(topicName)
						?.map((partition) => ({ partitionId: partition.id, offset }));
		...
		return this.deleteTopicRecordsFromMultiplePartitionOffsetPairs(
			topicName,
			partitions,
		);
	},
	...
	async deleteTopicRecordsFromAllPartitionsHighWatermark(topicName: string) {
		const partitions = this.topicPartitions
			?.get(topicName)
			?.map(({ waterMarkHigh, id }) => ({
				partitionId: id,
	...
	refreshPartitions(
		...
		return cachedApiRequest<GetAllPartitionsResponse | null>(url, force).then(
			(response) => {
				...
				transaction(() => {
					...
					for (const t of response.topics) {
						...
						for (const p of t.partitions) {
							...
							const validLogDirs = p.partitionLogDirs.filter(
								(e) => !e.error && e.size >= 0,
							);
							const replicaSize =
								validLogDirs.length > 0 ? validLogDirs.max((e) => e.size) : 0;
		...
		cachedApiRequest<GetPartitionsResponse | null>(
			...
			if (response?.partitions) {
				...
				for (const p of response.partitions) {
					...
					// replicaSize
					const validLogDirs = p.partitionLogDirs.filter(
						(e) => (e.error == null || e.error == "") && e.size >= 0,
					);
					const replicaSize =
						validLogDirs.length > 0 ? validLogDirs.max((e) => e.size) : 0;
					p.replicaSize = replicaSize >= 0 ? replicaSize : 0;
	...
	refreshTopicAcls(topicName: string, force?: boolean) {
		...
		cachedApiRequest<GetAclOverviewResponse | null>(
			`${appConfig.restBasePath}/acls?${query}`,
			force,
		).then((v) => {
	...
	refreshTopicConsumers(topicName: string, force?: boolean) {
		cachedApiRequest<GetTopicConsumersResponse>(
			...
			(v) => this.topicConsumers.set(topicName, v.topicConsumers),
			addError,
		);
	},
	...
	async refreshAcls(request: GetAclsRequest, force?: boolean): Promise<void> {
		...
		await cachedApiRequest<GetAclOverviewResponse | null>(
			`${appConfig.restBasePath}/acls?${query}`,
			force,
		).then((v) => {
			if (v) {
	...
	refreshQuotas(force?: boolean) {
		cachedApiRequest<QuotaResponse | null>(
			`${appConfig.restBasePath}/quotas`,
			force,
		).then((v) => (this.Quotas = v ?? null), addError);
	},

	async refreshSupportedEndpoints(): Promise<EndpointCompatibilityResponse | null> {
		const r = await rest<EndpointCompatibilityResponse>(
			`${appConfig.restBasePath}/console/endpoints`,
		);
		if (!r) return null;
	...
	refreshClusterOverview(force?: boolean) {
		cachedApiRequest<ClusterOverview>(
			`${appConfig.restBasePath}/cluster/overview`,
			force,
		).then((v) => {
	...
	refreshBrokers(force?: boolean) {
		cachedApiRequest<BrokerWithConfigAndStorage[]>(
			`${appConfig.restBasePath}/brokers`,
			force,
		).then((v) => {
			this.brokers = v;
		}, addError);
	...
	refreshCluster(force?: boolean) {
		cachedApiRequest<ClusterInfoResponse>(
			`${appConfig.restBasePath}/cluster`,
			force,
		).then((v) => {
	...
	refreshBrokerConfig(brokerId: number, force?: boolean) {
		cachedApiRequest<BrokerConfigResponse>(
			`${appConfig.restBasePath}/brokers/${brokerId}/config`,
			force,
		)
			.then((v) => {
	...
	refreshConsumerGroups(force?: boolean) {
		cachedApiRequest<GetConsumerGroupsResponse>(
			`${appConfig.restBasePath}/consumer-groups`,
			force,
		).then((v) => {
	...
	async editConsumerGroupOffsets(
		...
		const r = await parseOrUnwrap<EditConsumerGroupOffsetsResponse>(
			response,
			null,
		);
		return r.topics;
	...
	async deleteConsumerGroupOffsets(
		...
		const r = await parseOrUnwrap<DeleteConsumerGroupOffsetsResponse>(
			response,
			null,
		);
		return r.topics;
	...
	refreshAdminInfo(force?: boolean) {
		cachedApiRequest<AdminInfo | null>(
			`${appConfig.restBasePath}/admin`,
			force,
		).then((info) => {
			if (info == null) {
				this.adminInfo = null;
			...
			for (const binding of info.roleBindings) {
				binding.resolvedRole = info.roles.first(
					(r) => r.name == binding.roleName,
				)!;
				if (binding.resolvedRole == null)
					console.error(
						"could not resolve roleBinding to role: " + toJson(binding),
					);
			}

			// resolve bindings, and roles of each user
			for (const user of info.users) {
				user.bindings = user.bindingIds.map(
					(id) => info.roleBindings.first((rb) => rb.ephemeralId == id)!,
				);
				if (user.bindings.any((b) => b == null))
					console.error(
						"one or more rolebindings could not be resolved for user: " +
							toJson(user),
					);
				...
				for (const roleName in user.audits)
					user.grantedRoles.push({
						...
						grantedBy: user.audits[roleName].map(
							(bindingId) =>
								info.roleBindings.first((b) => b.ephemeralId == bindingId)!,
						),
					});
			}
	...
	refreshSchemaSubjects(force?: boolean) {
		cachedApiRequest<SchemaRegistrySubject[]>(
			`${appConfig.restBasePath}/schema-registry/subjects`,
			force,
		).then((subjects) => {
		...
		return rq
			.then((references) => {
				...
				for (const ref of references) {
					if (ref.error) {
						console.error(
							"error in refreshSchemaReferencedBy, reference entry has error",
							{ subjectName, version, error: ref.error, refRaw: ref },
						);
						continue;
					}
				...
				if (!subjectVersions) {
					subjectVersions = observable(
						new Map<number, SchemaReferencedByEntry[]>(),
					);
	...
	async deleteSchemaSubject(
		subjectName: string,
		permanent: boolean,
	): Promise<SchemaRegistryDeleteSubjectResponse> {
		const response = await appConfig.fetch(
	...
	async deleteSchemaSubjectVersion(
		...
		return parseOrUnwrap<SchemaRegistryDeleteSubjectVersionResponse>(
			response,
			null,
		);
	},
	...
	async startPartitionReassignment(
		...
		return await parseOrUnwrap<AlterPartitionReassignmentsResponse>(
			response,
			null,
		);
	},

	async setReplicationThrottleRate(
		brokerIds: number[],
	...
	async setThrottledReplicas(
		...
		for (const t of topicReplicas) {
			...
			const leaderReplicas = t.leaderReplicas
				.map((e) => `${e.partitionId}:${e.brokerId}`)
				.join(",");
			res.configs.push({
			...
			const followerReplicas = t.followerReplicas
				.map((e) => `${e.partitionId}:${e.brokerId}`)
				.join(",");
			res.configs.push({
	...
	async resetReplicationThrottleRate(
		brokerIds: number[],
	): Promise<PatchConfigsResponse> {
		const configRequest: PatchConfigsRequest = { resources: [] };
	...
	async changeConfig(
		request: PatchConfigsRequest,
	): Promise<PatchConfigsResponse> {
		const response = await appConfig.fetch(
			`${appConfig.restBasePath}/operations/configs`,
			{
				method: "PATCH",
				headers: [["Content-Type", "application/json"]],
				body: toJson(request),
	...
	async refreshConnectClusters(force?: boolean): Promise<void> {
		return cachedApiRequest<KafkaConnectors | null>(
			`${appConfig.restBasePath}/kafka-connect/connectors`,
			force,
		).then((v) => {
			// backend error
			if (!v) {
	...
	async changeTopicConfig(
		topicName: string | null,
		configs: PatchTopicConfigsRequest["configs"],
	): Promise<void> {
		const url = topicName
			? `${appConfig.restBasePath}/topics/${encodeURIComponent(topicName)}/configuration`
...
*/
    ...
    */
	...
	async validateConnectorConfig(
		...
		const result = await parseOrUnwrap<ConnectorValidationResult>(
			response,
			null,
		);
	...
	async publishRecords(
		request: PublishRecordsRequest,
	): Promise<ProduceRecordsResponse> {
		// POST "/topics-records"
		const response = await appConfig.fetch(
			`${appConfig.restBasePath}/topics-records`,
			{
				method: "POST",
				headers: [["Content-Type", "application/json"]],
				body: JSON.stringify(request),
	...
	async refreshServiceAccounts(force?: boolean): Promise<void> {
		await cachedApiRequest<GetUsersResponse | null>(
			`${appConfig.restBasePath}/users`,
			force,
	...
	async deleteServiceAccount(principalId: string): Promise<void> {
		const response = await appConfig.fetch(
			`${appConfig.restBasePath}/users/${encodeURIComponent(principalId)}`,
			{
				method: "DELETE",
			},
		);
	...
	get canDownloadDebugBundle() {
		return (
			...
			this.debugBundleStatuses.filter(
				(status) =>
					status.value.case === "bundleStatus" &&
					status.value.value.status === DebugBundleStatus_Status.SUCCESS,
			).length > 0
		);
	},
	...
	get isDebugBundleInProgress() {
		return this.debugBundleStatuses.some(
			(status) =>
				status.value.case === "bundleStatus" &&
				status.value.value.status === DebugBundleStatus_Status.RUNNING,
		);
	},
	...
	async createDebugBundle(
		request: CreateDebugBundleRequest,
	): Promise<CreateDebugBundleResponse> {
		const client = appConfig.debugBundleClient!;
		if (!client) {
			// this shouldn't happen but better to explicitly throw
			throw new Error("Debug bundle client is not initialized");
...
export const transformsApi = observable({
	...
	async refreshTransforms(_force: boolean): Promise<void> {
		...
		let nextPageToken = "";
		while (true) {
			const res = await client.listTransforms({
				request: { pageSize: 500, pageToken: nextPageToken },
			});
...
export function createMessageSearch() {
	const messageSearch = {
		...
		async startSearch(
			...
			const messageSearchAbortController = (this.abortController =
				new AbortController());
			...
			if (
				searchRequest.startOffset == PartitionOffsetOrigin.End ||
				req.filterInterpreterCode != null
			) {
			...
			try {
				for await (const res of client.listMessages(req, {
					...
					try {
						switch (res.controlMessage.case) {
							...
							case "progress":
								console.log(
									"progress: " + res.controlMessage.value.messagesConsumed,
								);
								this.bytesConsumed = Number(
									res.controlMessage.value.bytesConsumed,
								);
								this.totalMessagesConsumed = Number(
									res.controlMessage.value.messagesConsumed,
								);
								break;
							case "done":
							...
							case "error":
								...
								console.info(
									"ws backend error: " + res.controlMessage.value.message,
								);
								toast({
									title: "Backend Error",
							...
							case "data":
									...
									m.headers.push({
										...
										value: {
											payload: JSON.stringify(
												new TextDecoder().decode(h.value),
											),
											encoding: "text",
											schemaId: 0,
											size: h.value.length,
								...
								const keyPayload = new TextDecoder().decode(
									...
									default:
										console.log("unhandled key encoding type", {
											...
											encodingName:
												key?.encoding != null
													? proto3
															.getEnumType(PayloadEncoding)
															.findNumber(key.encoding)?.localName
													: undefined,
											message: res,
										});
								}
								...
								const valuePayload = new TextDecoder().decode(
										...
										console.log("unhandled value encoding type", {
											...
											encodingName:
												val?.encoding != null
													? proto3
															.getEnumType(PayloadEncoding)
															.findNumber(val.encoding)?.localName
													: undefined,
											message: res,
										});
								}
				...
				if (messageSearchAbortController.signal.aborted) {
					...
					console.error(
						"startMessageSearchNew: error in await loop of client.listMessages",
						{ error: e },
					);
					throw e;
				}
...
function addFrontendFieldsForConnectCluster(cluster: ClusterConnectors) {
	...
	cluster.canViewCluster =
		allowAll || allowedActions.includes("viewConnectCluster");
	cluster.canEditCluster =
		allowAll || allowedActions.includes("editConnectCluster");
	cluster.canDeleteCluster =
		allowAll || allowedActions.includes("deleteConnectCluster");
...
function prepareSynonyms(configEntries: ConfigEntry[]) {
	...
	for (const e of configEntries) {
		...
		if (e.synonyms.length > 0)
			if (e.synonyms[0].source == e.source) e.synonyms.splice(0, 1);
...
export async function partialTopicConfigs(
	...
	const response = await appConfig.fetch(
		`${appConfig.restBasePath}/topics-configs?${query}`,
	);
...
async function parseOrUnwrap<T>(
	response: Response,
	text: string | null,
): Promise<T> {
