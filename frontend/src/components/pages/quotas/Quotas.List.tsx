						...
						<DataTable<{
							...
							columns={[
								...
								},
								{
									size: 100,
									header: () => (
										<InfoText tooltip="Limit throughput of produce requests">
											Producer Rate
										</InfoText>
									),
									...
									cell: ({ row: { original } }) =>
										formatBytes(
											original.settings.first(
												(k) => k.key === QuotaType.PRODUCER_BYTE_RATE,
											)?.value,
										),
								},
								{
									size: 100,
									header: () => (
										<InfoText tooltip="Limit throughput of fetch requests">
											Consumer Rate
										</InfoText>
									),
									...
									cell: ({ row: { original } }) =>
										formatBytes(
											original.settings.first(
												(k) => k.key === QuotaType.CONSUMER_BYTE_RATE,
											)?.value,
										),
								},
								{
									...
									cell: ({ row: { original } }) =>
										formatRate(
											original.settings.first(
												(k) => k.key === QuotaType.CONTROLLER_MUTATION_RATE,
											)?.value,
										),
								},
							]}
					...
					extra={
						...
						>
							<Button variant="solid">
								Redpanda Console documentation for roles and permissions
							</Button>
						</a>
					}
				/>
