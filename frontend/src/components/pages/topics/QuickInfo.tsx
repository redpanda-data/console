...
export const TopicQuickInfoStatistic = observer((p: { topic: Topic }) => {
	...
	if (partitions === undefined) {
		...
		messageSum = partitions
			.sum((p) => p.waterMarkHigh - p.waterMarkLow)
			.toString();
	}
	...
	const cleanupPolicy = configEntries?.find(
		(x) => x.name == "cleanup.policy",
	)?.value;

	const retentionMs = filteredConfigEntries?.find(
		(e) => e.name === "retention.ms",
	);
	const retentionBytes = filteredConfigEntries?.find(
		(e) => e.name === "retention.bytes",
	);

	const segmentMs = filteredConfigEntries?.find((e) => e.name === "segment.ms");
	const segmentBytes = filteredConfigEntries?.find(
		(e) => e.name === "segment.bytes",
	);
						...
						{segmentMs && segmentBytes && (
							<Text as="dd">
								...
								<>
									...
									{isFinite(Number(segmentBytes.value)) &&
										Number(segmentBytes.value) !== -1 &&
										" / partition"}
								</>
							</Text>
						)}
							...
							<Text as="dd">
								...
								{retentionMs.value === "-1" && retentionBytes.value === "-1" ? (
									...
									<>
										...
										{formatConfigValue(
											retentionBytes.name,
											retentionBytes.value,
											"friendly",
										)}
										{isFinite(Number(retentionBytes.value)) &&
...
function filterTopicConfig(
	config: ConfigEntry[] | null | undefined,
): ConfigEntry[] | null | undefined {
	...
	if (
		config.find(
			(e) => e.name == "cleanup.policy" && (e.value ?? "").includes("compact"),
		)
	) {
		// this is a compacted topic, 'retention.bytes', 'retention.ms' don't apply, so hide them
		newConfig.removeAll(
			(e) => e.name == "retention.bytes" || e.name == "retention.ms",
		);
	}
