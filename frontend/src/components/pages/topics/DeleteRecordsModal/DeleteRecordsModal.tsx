	...
	return (
		<>
			<div className={styles.twoCol}>
				<TrashIcon />
				<p>
					You are about to delete records in your topic. Choose on what
					partitions you want to delete records. In the next step you can choose
					the new low water mark for your selected partitions.
				</p>
			</div>
			<RadioOptionGroup<PartitionOption>
				...
				options={[
					...
					},
					{
						...
						subTitle:
							"Delete records within a specific partition in this topic only.",
						content: (
							...
							>
								<SingleSelect<number | undefined>
									...
									onChange={
										onSpecificPartitionSelected as (
											v: number | undefined,
										) => void
									}
	...
	return (
		<>
			<div className={styles.twoCol}>
				<TrashIcon />
				<p>
					Choose the new low offset for your selected partitions. Take note that
					this is a soft delete and that the actual data may still be on the
					hard drive but not visible for any clients, even if they request the
					data.
				</p>
			</div>
...
const ManualOffsetContent = observer(
	({
		...
		if (
			api.topicPartitionErrors?.get(topicName) ||
			api.topicWatermarksErrors?.get(topicName)
		) {
			const partitionErrors = api.topicPartitionErrors
				.get(topicName)
				?.map(({ partitionError }, idx) => (
					<li key={`${topicName}-partitionErrors-${idx}`}>{partitionError}</li>
				));
			const waterMarksErrors = api.topicWatermarksErrors
				.get(topicName)
				?.map(({ waterMarksError }, idx) => (
					<li key={`${topicName}-watermarkErrors-${idx}`}>{waterMarksError}</li>
				));
			const message = (
				<>
...
export default function DeleteRecordsModal(
	...
	const [specifiedPartition, setSpecifiedPartition] = useState<null | number>(
		null,
	);
	...
	const handleFinish = async (
		responseData: DeleteRecordsResponseData | null | void,
	) => {
		...
		const errorPartitions = responseData.partitions.filter(
		...
		if (errorPartitions.length > 0) {
			setErrors(
				errorPartitions.map(
					({ partitionId, error }) => `Partition ${partitionId}: ${error}`,
				),
			);
	...
	const isOkButtonDisabled = () => {
		...
		if (step === 1) {
			return (
				...
				(isSpecficPartition && specifiedPartition === null)
			);
		}
	...
	const onOk = () => {
		...
		if (isAllPartitions && isHighWatermark) {
			api
				.deleteTopicRecordsFromAllPartitionsHighWatermark(topic.topicName)
				.then(handleFinish);
			...
			api
				.getTopicOffsetsByTimestamp([topic.topicName], timestamp)
				.then((topicOffsets) => {
					if (isAllPartitions) {
						const pairs = topicOffsets[0].partitions.map(
							({ partitionId, offset }) => ({
								partitionId,
								offset,
							}),
					...
					{hasErrors && (
						<Alert status="error" mb={2}>
							...
							<Flex flexDirection="column" gap={4} p={2}>
								<Text>
									Errors have occurred when processing your request. Please
