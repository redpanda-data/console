...
export class TopicConfiguration extends Component<{
	...
	render() {
		...
		const entries =
			api.topicConfig.get(this.props.topic.topicName)?.configEntries ?? [];
			...
			<Flex my={8} flexDirection="column" alignItems="center">
				<Flex flexDirection="column" maxWidth="4xl">
					<Result
						...
						subTitle={
							<>
								Redpanda Console received the following error while fetching the
								configuration for topic <Code p={1}>{topicName}</Code> from
								Kafka:
							</>
						}
					/>
