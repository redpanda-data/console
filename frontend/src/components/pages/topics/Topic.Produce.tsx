...
const encodingOptions: EncodingOption[] = [
...

	{
		...
		label: "Binary (Base64)",
		tooltip:
			"Message value is binary, represented as a base64 string in the editor",
	},
];

const protoBufInfoElement = (
	<Text>
		Protobuf schemas can define multiple types. Specify which type you want to
		use for this message.{" "}
		<Link
			target="_blank"
			rel="noopener noreferrer"
			href="https://protobuf.dev/reference/protobuf/google.protobuf/"
		>
			Learn more here.
		</Link>
	</Text>
...
const PublishTopicForm: FC<{ topicName: string }> = observer(
	({ topicName }) => {
		...
		useEffect(() => {
			if (
				keyPayloadOptions.encoding === PayloadEncoding.BINARY &&
				!isValidBase64(keyPayloadOptions.data)
			) {
				setError("key.data", {
					type: "manual",
			...
			keyPayloadOptions.encoding,
			keyPayloadOptions.data,
			setError,
			clearErrors,
		]);
		...
		useEffect(() => {
			if (
				valuePayloadOptions.encoding === PayloadEncoding.BINARY &&
				!isValidBase64(valuePayloadOptions.data)
			) {
				setError("value.data", {
					type: "manual",
					message: "Invalid Base64 format",
			...
			valuePayloadOptions.encoding,
			valuePayloadOptions.data,
			setError,
			clearErrors,
		]);
		...
		const showKeySchemaSelection =
			keyPayloadOptions.encoding === PayloadEncoding.AVRO ||
			keyPayloadOptions.encoding === PayloadEncoding.PROTOBUF;
		const showValueSchemaSelection =
			valuePayloadOptions.encoding === PayloadEncoding.AVRO ||
		...
		useEffect(() => {
			return autorun(() => {
				api.schemaSubjects
					?.filter(
						(x) =>
							uiSettings.schemaList.showSoftDeleted ||
							(!uiSettings.schemaList.showSoftDeleted && !x.isSoftDeleted),
					)
					?.filter((x) =>
						x.name
							.toLowerCase()
							.includes(uiSettings.schemaList.quickSearch.toLowerCase()),
					)
					.forEach((x) => {
		...
		const onSubmit: SubmitHandler<Inputs> = async (data) => {
			...
			if (result) {
				toast({
					...
					description: (
						<>
							Record published on partition{" "}
							<span className="codeBox">{result.partitionId}</span> with offset{" "}
		...
		const filteredEncodingOptions = encodingOptions.filter(
			(x) => x.value != PayloadEncoding.AVRO,
		);
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<Grid width={["100%", "100%", 600]} gap={6} flexDirection="column">
					...
					<Flex gap={4} flexDirection="column">
							...
							<Label text="Data">
								<Box data-testid="produce-key-editor">
									...
									<Button
										size="sm"
										variant="link"
										onClick={() => setKeyExpanded(!isKeyExpanded)}
										px={0}
										mt={1}
									>
						...
						{errors?.key?.data && (
							<Text color="red.500">{errors.key.data.message}</Text>
						)}
					</Flex>
					...
					<Flex gap={4} flexDirection="column">
						...
						<Flex gap={2} flexDirection="column">
							...
							{errors?.value?.data && (
								<Text color="red.500">{errors.value.data.message}</Text>
							)}
					...
					{!!errors?.root?.serverError && (
						<Alert status="error">{errors.root.serverError.message}</Alert>
					)}
...
export class TopicProducePage extends PageComponent<{ topicName: string }> {
	initPage(p: PageInitHelper): void {
		...
		p.addBreadcrumb(
			substringWithEllipsis(topicName, 50),
			"/topics/" + topicName,
		);
