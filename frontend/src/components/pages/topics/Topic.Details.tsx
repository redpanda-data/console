    ...
    render() {
        ...
        this.topicTabs = [
            ...
            new TopicTab(() => topic, 'partitions', 'viewPartitions', <Flex gap={1}>
                Partitions
                {!!leaderLessPartitionIds && <Tooltip placement="top" hasArrow label={`This topic has ${leaderLessPartitionIds.length} leaderless ${leaderLessPartitionIds.length === 1 ? 'partition' : 'partitions'}`}><Box><MdError size={18} color={colors.brandError} /></Box></Tooltip>}
                {!!underReplicatedPartitionIds && <Tooltip placement="top" hasArrow label={`This topic has ${underReplicatedPartitionIds.length} under-replicated ${underReplicatedPartitionIds.length === 1 ? 'partition' : 'partitions'}`}><Box><MdOutlineWarning size={18} color={colors.brandWarning} /></Box></Tooltip>}
