...
class TopicDetails extends PageComponent<{ topicName: string }> {
	...
	render() {
		...
		const leaderLessPartitionIds = (
			api.clusterHealth?.leaderlessPartitions ?? []
		).find(({ topicName }) => topicName === this.props.topicName)?.partitionIds;
		const underReplicatedPartitionIds = (
		...
		this.topicTabs = [
			new TopicTab(
				...
				(t) => (
					<TopicMessageView
						topic={t}
						refreshTopicData={(force: boolean) => this.refreshData(force)}
					/>
				),
			),
			new TopicTab(
			...
			new TopicTab(
				...
				[
					(t) =>
						t.documentation == "NOT_CONFIGURED"
							? mkDocuTip("Topic documentation is not configured")
							: null,
					(t) =>
						t.documentation == "NOT_EXISTENT"
							? mkDocuTip(
									"Documentation for this topic was not found in the configured repository",
									warnIcon,
								)
							: null,
				],
			...
			<>
				<PageContent key={"b"}>
						...
						<Button
							...
							onClick={() => {
								appGlobal.history.push(
									`/topics/${encodeURIComponent(topic.topicName)}/produce-record`,
								);
							}}
						>
							Produce Record
					...
					<Section>
						<Tabs
							isFitted
							tabs={this.topicTabs.map(
								({ id, title, content, isDisabled }) => ({
									key: id,
									disabled: isDisabled,
									title,
									content,
	...
	addBaseFavs(topicConfig: ConfigEntry[]): void {
		const cleanupPolicy = topicConfig.find(
