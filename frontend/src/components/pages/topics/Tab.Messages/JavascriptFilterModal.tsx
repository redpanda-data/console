		...
		<Modal isOpen onClose={onClose}>
			...
			<ModalContent minW="4xl">
				...
				<ModalBody>
					...
					<Grid templateColumns={{ base: "1fr", md: "3fr 2fr" }} gap={6}>
						...
						<GridItem display="flex" gap={4} flexDirection="column">
							...
							<UnorderedList>
								<ListItem>
									return true allows messages, return false discards them.
								</ListItem>
								<ListItem>
									Available params are <Code>offset</Code>,{" "}
									<Code>partitionID</Code> (number), <Code>key</Code> (any),{" "}
									<Code>value</Code> (any), and <Code>headers</Code> (object),{" "}
									<Code>keySchemaID</Code> (number) and{" "}
						...
						<GridItem>
							...
							<UnorderedList styleType="none" margin={0} spacing={4}>
								...
								<ListItem>
									...
									only returns messages where keys equal <Code>'example'</Code>{" "}
									in their string presentation (after decoding)
								</ListItem>
							</UnorderedList>
						</GridItem>
					</Grid>
				...
				<ModalFooter>
					...
					>
						<Button
							data-testid="add-javascript-filter-close"
							onClick={() => onClose()}
							variant="outline"
						>
							Close
