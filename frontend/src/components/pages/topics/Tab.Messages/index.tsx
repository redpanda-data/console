...
const defaultSelectChakraStyles: SingleSelectProps<
...
const inlineSelectChakraStyles: SingleSelectProps<
	PayloadEncoding | number
>["chakraStyles"] = {
...
export class TopicMessageView extends Component<TopicMessageViewProps> {
		...
		if (query.has("o")) {
			...
			searchParams.offsetOrigin =
				searchParams.startOffset >= 0
					? PartitionOffsetOrigin.Custom
					: searchParams.startOffset;
		}
		if (query.has("q"))
			uiState.topicSettings.quickSearch = String(query.get("q"));
							...
							<AlertDescription>
								...
								<Box mt="4">
									<div className="codeBox">
										{(this.fetchError as Error).message ??
											String(this.fetchError)}
									</div>
								</Box>
	...
	SearchControlsBar = observer(() => {
		...
		const canUseFilters =
			(api.topicPermissions.get(topic.topicName)?.canUseSearchFilters ??
				true) &&
			!isServerless();
		const [customStartOffsetValue, setCustomStartOffsetValue] = useState(
			0 as number | string,
		);
		const customStartOffsetValid = !isNaN(Number(customStartOffsetValue));

		const [currentJSFilter, setCurrentJSFilter] = useState<FilterEntry | null>(
			null,
		);
					...
					>
						<Label text="Start Offset">
							<Flex gap={3}>
								...
								{searchParams.offsetOrigin ==
									PartitionOffsetOrigin.Timestamp && (
						...
						<Label text="Max Results">
							<SingleSelect<number>
								...
								options={[1, 3, 5, 10, 20, 50, 100, 200, 500].map((i) => ({
									value: i,
								}))}
							/>
						</Label>
								...
								({
									partition: (
										<Label text="Partition">
											<RemovableFilter
												onRemove={() => {
													uiState.topicSettings.dynamicFilters.remove(
														"partition",
													);
													searchParams.partitionID =
														DEFAULT_SEARCH_PARAMS["partitionID"];
												}}
											>
												<SingleSelect<number>
							...
							<Menu>
								...
								<MenuList>
									<MenuItem
										...
										isDisabled={uiState.topicSettings.dynamicFilters.includes(
											"partition",
										)}
										onClick={() =>
											uiState.topicSettings.dynamicFilters.pushDistinct(
												"partition",
											)
										}
									>
						...
						{Boolean(
							this.messageSearch.searchPhase &&
								this.messageSearch.searchPhase.length > 0,
						) && (
							<StatusIndicator
								identityKey="messageSearch"
								fillFactor={
									(this.messageSearch.messages?.length ?? 0) /
								...
								messagesConsumed={String(
									this.messageSearch.totalMessagesConsumed,
								)}
							/>
						)}
					</GridItem>
                    ...
                    */}
					...
					<GridItem gridColumn="1/-1" mt={4} display="flex" gap={4}>
						...
						<Input
							...
							onChange={(x) =>
								(uiState.topicSettings.quickSearch = x.target.value)
							}
						/>
						<Flex gap={2} fontSize="sm" whiteSpace="nowrap" alignItems="center">
							{this.messageSearch.searchPhase === null ||
							this.messageSearch.searchPhase === "Done" ? (
								<>
									<Flex alignItems="center" gap={2}>
										<MdDownload size={14} />{" "}
										{prettyBytes(this.messageSearch.bytesConsumed)}
									</Flex>
									<Flex alignItems="center" gap={2}>
										<MdOutlineTimer size={14} />{" "}
										{this.messageSearch.elapsedMs
											? prettyMilliseconds(this.messageSearch.elapsedMs)
											: ""}
									</Flex>
								</>
							) : (
					...
					<JavascriptFilterModal
						...
						onSave={(filter) => {
							if (filter.isNew) {
								...
								const idx =
									uiState.topicSettings.searchParams.filters.findIndex(
										(x) => x.id === filter.id,
									);
								if (idx !== -1) {
									uiState.topicSettings.searchParams.filters.splice(
										idx,
										1,
										filter,
									);
	...
	searchFunc = (source: "auto" | "manual") => {
		...
		untracked(() => {
			...
			if (searchParams == this.currentSearchRun && source == "auto") {
				console.log(
					"ignoring serach, search params are up to date, and source is auto",
					{
	...
	async loadLargeMessage(
		...
		if (messages && messages.length == 1) {
			...
			console.error("LoadLargeMessage: messages response is empty", {
				messages,
			});
			throw new Error(
				"LoadLargeMessage: Couldn't load the message content, the response was empty",
			);
		}
	}
	...
	MessageTable = observer(() => {
		...
		const hasKeyTags =
			uiState.topicSettings.previewTags.count(
				(x) => x.isActive && x.searchInMessageKey,
			) > 0;

		function onCopyValue(original: TopicMessage) {
			navigator.clipboard
				.writeText(
					getPayloadAsString(original.value.payload ?? original.value.rawBytes),
				)
				.then(() => {
					toast({
						status: "success",
		...
		function onCopyKey(original: TopicMessage) {
			navigator.clipboard
				.writeText(
					getPayloadAsString(original.key.payload ?? original.key.rawBytes),
				)
				.then(() => {
					toast({
						status: "success",
		...
		const isValueDeserializerActive =
			...
			uiState.topicSettings.searchParams.valueDeserializer !==
		...
		const isKeyDeserializerActive =
			...
			uiState.topicSettings.searchParams.keyDeserializer !==
				PayloadEncoding.UNSPECIFIED;
		...
		const dataTableColumns: Record<DataColumnKey, ColumnDef<TopicMessage>> = {
			...
			timestamp: {
				...
				cell: ({
					...
					<TimestampDisplay
						unixEpochMillisecond={timestamp}
						format={tsFormat}
					/>
				),
			},
		...
		const columnsVisibleByDefault: DataColumnKey[] = [
			"timestamp",
			"key",
			"value",
		];

		const newColumns: ColumnDef<TopicMessage>[] = columnsVisibleByDefault.map(
			(key) => dataTableColumns[key],
		);
		...
		if (uiState.topicSettings.previewColumnFields.length > 0) {
			...
			const selectedColumns = new Set(
				uiState.topicSettings.previewColumnFields.map(
					...
					return (
						<Menu computePositionOnMount>
							...
							<MenuList>
								...
								<MenuItem
									isDisabled={original.key.isPayloadNull}
									onClick={() => onCopyKey(original)}
								>
									Copy Key
								</MenuItem>
								<MenuItem
									isDisabled={original.value.isPayloadNull}
									onClick={() => onCopyValue(original)}
								>
									Copy Value
								</MenuItem>
								<MenuItem
	...
	async executeMessageSearch(): Promise<TopicMessage[]> {
		const searchParams = uiState.topicSettings.searchParams;
		const canUseFilters =
			(api.topicPermissions.get(this.props.topic.topicName)
				?.canUseSearchFilters ??
				true) &&
			!isServerless();
		...
		if (canUseFilters) {
			...
			if (functions.length > 0) {
				...
				if (IsDev)
					console.log(
						`constructed filter code (${functions.length} functions)`,
						"\n\n" + filterCode,
					);
			}
		}
			...
			<Modal isOpen={count > 0} onClose={onClose}>
				...
				<ModalContent minW="2xl">
					...
					<ModalBody display="flex" flexDirection="column" gap="4">
						<div>
							Select the format in which you want to save{" "}
							{count == 1 ? "the message" : "all messages"}
						</div>
						<Box py={2}>
							<RadioGroup
								name="format"
						...
						<Checkbox
							isChecked={this.includeRawContent}
							onChange={(e) => (this.includeRawContent = e.target.checked)}
						>
							Include raw data
						</Checkbox>
					</ModalBody>
...
@observer
class MessageKeyPreview extends Component<{
	msg: TopicMessage;
	...
	render() {
		...
		try {
			...
			if (key.encoding == "binary") {
				...
				const previewTags = this.props
					.previewFields()
					.filter((t) => t.searchInMessageValue);
						...
						<span className="cellDiv fade" style={{ fontSize: "95%" }}>
							<div
								className={
									...
									uiState.topicSettings.previewDisplayMode
								}
							>
								{tags.map((t, i) => (
			...
			return (
				<Flex flexDirection="column">
					<span
						className="cellDiv"
						style={{ minWidth: "10ch", width: "auto", maxWidth: "45ch" }}
					>
			...
			return (
				<span style={{ color: "red" }}>
					Error in RenderPreview: {(e as Error).message ?? String(e)}
				</span>
			);
		}
	}
				...
				items={[
					{
						...
						name: (
							<Box minWidth="6rem">
								{msg.key === null || msg.key.size === 0
									? "Key"
									: `Key (${prettyBytes(msg.key.size)})`}
							</Box>
						),
							...
							<Box>
								...
								<ExpandedMessageFooter onDownloadRecord={onDownloadRecord}>
									{onCopyKey && (
										<Button
											variant="outline"
											onClick={() => onCopyKey(msg)}
											isDisabled={msg.key.isPayloadNull}
										>
											Copy Key
										</Button>
									)}
					...
					},
					{
						...
						name: (
							<Box minWidth="6rem">
								{msg.value === null || msg.value.size === 0
									? "Value"
									: `Value (${prettyBytes(msg.value.size)})`}
							</Box>
						),
						component: (
							<Box>
								...
								<ExpandedMessageFooter onDownloadRecord={onDownloadRecord}>
									{onCopyValue && (
										<Button
											variant="outline"
											onClick={() => onCopyValue(msg)}
											isDisabled={msg.value.isPayloadNull}
										>
											Copy Value
										</Button>
									)}
...
const PayloadComponent = observer(
	(p: {
		...
		if (payload.isPayloadTooLarge) {
			return (
				<Flex flexDirection="column" gap="4">
					<Flex alignItems="center" gap="2">
						Because this message size exceeds the display limit, loading it
		...
		try {
			...
			return (
				<span style={{ color: "red" }}>
					Error in RenderExpandedMessage: {(e as Error).message ?? String(e)}
				</span>
			);
		}
	},
...
function highlightControlChars(str: string, maxLength?: number): JSX.Element[] {
	...
	for (const char of str) {
		...
		if (code < 32) {
			...
			elements.push(
				<span className="controlChar">{getControlCharacterName(code)}</span>,
			);
			if (code == 10)
...
const MessageMetaData = observer((props: { msg: TopicMessage }) => {
	...
	const data: { [k: string]: any } = {
		...
		Key: msg.key.isPayloadNull
			? "Null"
			: `${titleCase(msg.key.encoding)} (${prettyBytes(msg.key.size)})`,
		...
		Headers:
			msg.headers.length > 0 ? `${msg.headers.length}` : "No headers set",
...
const MessageSchema = observer((p: { schemaId: number }) => {
	...
	return (
		<>
			<Link
				as={ReactRouterLink}
				to={`/schema-registry/subjects/${encodeURIComponent(s.subject)}?version=${s.version}`}
			>
					...
					columns={[
						...
						},
						{
							...
							cell: ({
								...
								// object
								return (
									<span className="cellDiv">
										{toSafeString(headerValue.payload)}
									</span>
								);
							},
						},
						...
						return (
							<Box py={6} px={10}>
								{typeof header.value?.payload !== "object" ? (
									...
									<KowlJsonView
										srcObj={header.value.payload as object}
										style={{ margin: "2em 0" }}
									/>
								)}
							</Box>
	...
	return (
		...
		>
			...
			<ModalContent minW="4xl">
				...
				<ModalBody>
					<Text>
						Choose which columns will be shown in the messages table, as well as
							...
							<Stack spacing={5} direction="row">
								{columnSettings.map(({ title, dataIndex }) => (
									<Checkbox
										...
										size="lg"
										isChecked={
											!!uiState.topicSettings.previewColumnFields.find(
										...
										onChange={({ target: { checked } }) => {
											if (checked) {
												...
												const idxToRemove =
													uiState.topicSettings.previewColumnFields.findIndex(
														(x) => x.dataIndex === dataIndex,
													);
												uiState.topicSettings.previewColumnFields.splice(
													idxToRemove,
													1,
												);
											}
	...
	return (
		...
		>
			...
			<ModalContent minW="xl">
				...
				<ModalBody display="flex" flexDirection="column" gap={4}>
					<Text>
						Redpanda attempts to automatically detect a deserialization
						strategy. You can choose one manually here.
					</Text>
					<Box>
...
const MessageSearchFilterBar: FC<{ onEdit: (filter: FilterEntry) => void }> =
	observer(({ onEdit }) => {
				...
				>
					...
					{settings.filters?.map((e) => (
						<Tag
							...
							className={
								e.isActive ? "filterTag" : "filterTag filterTagDisabled"
							}
							key={e.id}
						>
							<MdOutlineSettings
							...
							<TagCloseButton
								onClick={() => settings.filters.remove(e)}
								m="0"
								px="1"
								opacity={1}
							/>
...
function hasDeleteRecordsPrivilege(allowedActions: Array<TopicAction>) {
	return (
		allowedActions.includes("deleteTopicRecords") ||
...
export function DeleteRecordsMenuItem(
	...
	const isEnabled =
		!isCompacted &&
		hasDeleteRecordsPrivilege(allowedActions) &&
		isSupported(Feature.DeleteRecords);

	let errorText: string | undefined;
	if (isCompacted)
		errorText =
			"Records on Topics with the 'compact' cleanup policy cannot be deleted.";
