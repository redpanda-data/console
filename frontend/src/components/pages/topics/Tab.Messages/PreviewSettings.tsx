...
const globHelp = (
	<div>
		...
		<Flex gap={2}>
			<Flex grow={1}>
				<div className="globHelpGrid">
					...
					<div className="c3">
						There is only one 'id' property at the root of the object
					</div>
					...
					<div className="c3">
						Star only seraches in direct children. Here, only 2 children contain
						an 'id' prop
					</div>
					<div className="rowSeparator" />
					...
					<div className="c3">
						In the direct child named 'customer', find all properties that
						contain 'Na'
					</div>
					<div className="rowSeparator" />
					...
					<div className="c3">
						To find properties with special characters in their name, use single
						or double-quotes
					</div>
					<div className="rowSeparator" />
				</div>
			...
			>
				<div style={{ opacity: 0.5, fontSize: "smaller", textAlign: "center" }}>
					Example Data
				</div>
		...
		<div>
			<h3>Details</h3>
			<div>
				A glob pattern is just a list of property names seperated by dots. In
				addition to simple property names you can use:
			</div>
			<ul style={{ paddingLeft: "2em", marginTop: ".5em" }}>
				...
				<li>
					<Code>**</Code> Double-Star to matches all current and nested
					properties
				</li>
				<li>
					<Code>"</Code>/<Code>'</Code> Quotes for when a property-name contains
					dots
				</li>
				<li>
					<Code>abc*</Code> One or more stars within a name. Depending on where
					you place the star, you can check if a name starts with, ends with, or
	...
	render() {
		...
		const content = (
			<>
				<div>
					<span>
						When viewing large messages we're often only interested in a few
						specific fields. Add <PatternHelpDrawer />
						<Popover
							trigger={"click"}
							placement="bottom"
							content={globHelp}
							...
							hideCloseButton
						></Popover>{" "}
						to this list to show found values as previews.
					</span>
				</div>
				...
				<div style={{ marginTop: "1em" }}>
					...
					<div className="previewTagsSettings">
						<OptionGroup<"caseSensitive" | "ignoreCase">
							...
							onChange={(e) =>
								(uiState.topicSettings.previewTagsCaseSensitive = e)
							}
						/>
						<OptionGroup
							label="Multiple Results"
							options={{
								"First result": "showOnlyFirst",
								"Show All": "showAll",
							}}
							value={uiState.topicSettings.previewMultiResultMode}
							onChange={(e) =>
								(uiState.topicSettings.previewMultiResultMode = e)
							}
						/>
		...
		return (
			<Flex placeItems="center" gap={1} p={1} borderRadius={1} mb={1.5}>
				...
				<Checkbox
					...
					onChange={(e) => (tag.isActive = e.target.checked)}
				/>
						...
						>
							...
							<span>
								<Checkbox
									...
									onChange={(e) =>
										(tag.searchInMessageValue = e.target.checked)
									}
								>
...
export function getPreviewTags(
	...
	const results: {
		prop: CollectedProperty;
		tag: PreviewTagV2;
		fullPath: string;
	}[] = [];
	const caseSensitive =
		uiState.topicSettings.previewTagsCaseSensitive === "caseSensitive";
	...
	for (const t of tags) {
		...
		const foundProperties = collectElements2(
			targetObject,
			searchPath,
	...
	for (const r of results) {
		...
		const displayName =
			tag.customName && tag.customName.length > 0 ? tag.customName : r.fullPath;
	...
	while (pos < str.length) {
		...
		switch (c) {
			...
			case '"':
				...
				if (end == -1)
					return (
						"missing closing quote, quote was opened at index " + (start - 1)
					);
	...
	for (const test of tests) {
		...
		if (!hasError && result && expected) {
			...
			for (let i = 0; i < result.length && !hasError; i++)
				if (result[i] != expected[i]) hasError = true; // wrong array entry
		}
