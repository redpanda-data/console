...
export function NumInput(p: {
	...
	const [editValue, setEditValue] = useState(
		p.value == null ? undefined : String(p.value),
	);
	useEffect(
		() => setEditValue(p.value == null ? undefined : String(p.value)),
		[p.value],
	);
	...
	return (
		<InputGroup>
			...
			<Input
				...
				disabled={p.disabled}
				value={
					p.disabled && p.placeholder && p.value == null ? undefined : editValue
				}
				onChange={(e) => {
...
function RetentionSizeSelect(p: {
	...
	if (unit == "default") {
		if (
			p.defaultConfigValue != null &&
			p.defaultConfigValue != "" &&
			Number.isFinite(Number(p.defaultConfigValue))
		) {
			placeholder = prettyBytes(p.defaultConfigValue, {
				showLargeAsInfinite: true,
				showNullAs: "default",
			});
		} else {
...
const KeyValuePair = observer(
	(p: { entries: TopicConfigEntry[]; entry: TopicConfigEntry }) => {
		const { entry } = p;
					...
					<Select<UnitType>
						...
						onChange={(arg) => {
							if (isSingleValue(arg) && arg) {
								...
								const changedFromInfinite =
									this.unit == "infinite" && u != "infinite";
