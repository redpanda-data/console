...
export const TopicPartitions: FC<TopicPartitionsProps> = observer(
	({ topic }) => {
		const partitions = api.topicPartitions.get(topic.topicName);
		const paginationParams = usePaginationParams(
			uiState.topicSettings.partitionPageSize,
			partitions?.length ?? 0,
		);
		...
		).find(({ topicName }) => topicName === topic.topicName)?.partitionIds;
					...
					columns={[
								...
								return (
									<Flex alignItems="center" gap={2}>
										...
										{leaderLessPartitions?.includes(partition.id) && (
											<Badge variant="error">Leaderless</Badge>
										)}
						...
						},
						{
							...
							cell: ({ row: { original: partition } }) =>
								!partition.hasErrors &&
								numberToThousandsString(
									partition.waterMarkHigh - partition.waterMarkLow,
								),
						},
						{
							header: "Brokers",
			...
			content={
				...
				>
					...
					{partition.waterMarksError && (
						<Text>{partition.waterMarksError}</Text>
					)}
				</Flex>
			}
		>
