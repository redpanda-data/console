...
class TopicList extends PageComponent {
	...
	@computed get topics() {
		...
		if (uiSettings.topicList.hideInternalTopics) {
			topics = topics.filter(
				(x) => !x.isInternal && !x.topicName.startsWith("_"),
			);
		}
		return topics;
	}
	...
	render() {
		...
		const replicaCount = topics.sum(
			(x) => x.partitionCount * x.replicationFactor,
		);
				...
				<Box pt={6}>
					<SearchBar<Topic>
						...
						onQueryChanged={(filterText) =>
							(uiSettings.topicList.quickSearch = filterText)
						}
...
const TopicsTable: FC<{
	topics: Topic[];
	onDelete: (record: Topic) => void;
}> = ({ topics, onDelete }) => {
	const paginationParams = usePaginationParams(
		uiSettings.topicList.pageSize,
		topics.length,
	);
			...
			columns={[
				{
					...
					cell: ({ row: { original: topic } }) => {
						const leaderLessPartitions = (
							api.clusterHealth?.leaderlessPartitions ?? []
						).find(
							({ topicName }) => topicName === topic.topicName,
						)?.partitionIds;
						const underReplicatedPartitions = (
							api.clusterHealth?.underReplicatedPartitions ?? []
						).find(
							({ topicName }) => topicName === topic.topicName,
						)?.partitionIds;
						...
						return (
							<Flex
								...
								alignItems="center"
							>
								<Link to={`/topics/${encodeURIComponent(topic.topicName)}`}>
									{renderName(topic)}
								</Link>
				...
				},
				{
					...
					cell: ({ row: { original: topic } }) =>
						renderLogDirSummary(topic.logDirSummary),
				},
				{
					id: "action",
					header: "",
				...
				<AlertDialogContent>
					...
					<AlertDialogBody>
						...
						{topicToDelete?.isInternal && (
							<Alert status="error" mb={2}>
								<AlertIcon />
								This is an internal topic, deleting it might have unintended
								side-effects!
							</Alert>
						)}
						<Text>
							Are you sure you want to delete topic{" "}
							<Code>{topicToDelete?.topicName}</Code>?<br />
...
function DeleteDisabledTooltip(props: {
	topic: Topic;
	...
	return (
		<>
			{hasDeletePrivilege(topic.allowedActions)
				...
				: wrap(
						deleteButton,
						"You don't have 'deleteTopic' permission for this topic.",
					)}
		</>
	);
}
...
function hasDeletePrivilege(allowedActions?: Array<TopicAction>) {
	return Boolean(
		allowedActions?.includes("all") || allowedActions?.includes("deleteTopic"),
	);
}
...
function makeCreateTopicModal(parent: TopicList) {
	...
	const tryGetBrokerConfig = (configName: string): string | undefined => {
		return (
			api.clusterInfo?.brokers
				?.find((_) => true)
				?.config.configs?.find((x) => x.name === configName)?.value ?? undefined
		);
	};

	const getRetentionTimeFinalValue = (
		...
		if (value == undefined)
			throw new Error(
				`unexpected: value for retention time is 'undefined' but unit is set to ${unit}`,
			);
	...
	const getRetentionSizeFinalValue = (
		value: number | undefined,
		...
		if (value == undefined)
			throw new Error(
				`unexpected: value for retention size is 'undefined' but unit is set to ${unit}`,
			);
	...
	return createAutoModal<void, CreateTopicModalState>({
		...
		onOk: async (state) => {
			...
			if (state.retentionTimeUnit != "default")
				setVal(
					...
					getRetentionTimeFinalValue(
						state.retentionTimeMs,
						state.retentionTimeUnit,
					),
				);
			...
			const result = await api.createTopic({
				...
				partitionCount:
					state.partitions ?? Number(state.defaults.partitions ?? "-1"),
				replicationFactor:
					state.replicationFactor ??
					Number(state.defaults.replicationFactor ?? "-1"),
