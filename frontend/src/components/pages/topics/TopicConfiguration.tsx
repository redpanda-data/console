...
const ConfigurationEditor: FC<ConfigurationEditorProps> = observer((props) => {
	...
	const editConfig = (configEntry: ConfigEntryExtended) => {
		...
		$state.initialValueType = configEntry.isExplicitlySet
			? "custom"
			: "default";
	...
	if (filter)
		entries = entries.filter(
			(x) => x.name.includes(filter) || (x.value ?? "").includes(filter),
		);
						...
						<ModalBody>
							<Observer>
								{() => {
									...
									const defaultValue =
										defaultEntry?.value ?? $state.editedEntry.value;
									const defaultSource =
										defaultEntry?.source ?? $state.editedEntry.source;
									const friendlyDefault = formatConfigValue(
										$state.editedEntry.name,
										defaultValue,
										"friendly",
									);
									...
									return (
										<div>
											<p>
												Edit <code>{configEntry.name}</code> configuration for
												topic <code>{props.targetTopic}</code>.
											</p>
											<Alert
												bg="blue.50"
												status="info"
						...
						<ModalFooter>
							...
							<Button
								...
								onClick={async () => {
									...
									const operation =
										$state.modalValueType == "custom" ? "SET" : "DELETE";
										...
										toast({
											...
											description: (
												<span>
													...
													<code>{$state.editedEntry.name}</code>
												</span>
											),
										});
										$state.editedEntry = null;
									} catch (err) {
										console.error("error while applying config change", {
											err,
											configEntry: $state.editedEntry,
										});
										$state.modalError =
...
const ConfigEntry = observer(
	(p: {
		...
		const { canEdit, reason: nonEdittableReason } = isTopicConfigEdittable(
			p.entry,
			p.hasEditPermissions,
		);

		const entry = p.entry;
		const friendlyValue = formatConfigValue(
			entry.name,
			entry.value,
			"friendly",
		);
...
function isTopicConfigEdittable(
	...
	if (!hasEditPermissions)
		return {
			canEdit: false,
			reason:
				"You don't have permissions to change topic configuration entries",
		};
	...
	if (isServerless()) {
		...
		return {
			canEdit: false,
			reason: "This configuration is not editable on Serverless clusters",
		};
	}
		...
		switch (entry.frontendFormat) {
			...
			case "PASSWORD":
				return (
					<PasswordInput
						value={entry.currentValue ?? ""}
						onChange={(x) => (entry.currentValue = x.target.value)}
					/>
				);
			...
			case "RATIO":
				return (
					<RatioInput
						...
						onChange={(x) => (entry.currentValue = x)}
					/>
				);
			...
			default:
				return (
					<Input
						...
						onChange={(e) => (entry.currentValue = e.target.value)}
					/>
				);
		}
	},
