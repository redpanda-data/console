...
import {
	RESOURCE_TIERS,
	ResourceTierSelect,
} from "components/ui/connect/resource-tier-select";
...
} from "components/ui/secret/secret-detection";
...
export const RemoteMCPConfigurationTab = () => {
	...
	const { data: mcpServerData } = useGetMCPServerQuery(
		{ id: id || "" },
		{ enabled: !!id },
	);
	const { mutateAsync: updateMCPServer, isPending: isUpdateMCPServerPending } =
		useUpdateMCPServerMutation();
	const { mutateAsync: lintConfig, isPending: isLintConfigPending } =
		useLintMCPConfigMutation();
	const { data: serverToolsData } = useListMCPServerTools({
		mcpServer: mcpServerData?.mcpServer,
	});
	const { data: secretsData } = useListSecretsQuery();

	const [isEditing, setIsEditing] = useState(false);
	const [editedServerData, setEditedServerData] =
		useState<LocalMCPServer | null>(null);
	const [selectedToolId, setSelectedToolId] = useState<string | null>(null);
	const [detectedSecrets, setDetectedSecrets] = useState<string[]>([]);
	const [lintHints, setLintHints] = useState<
		Record<string, Record<string, LintHint>>
	>({});
	const [isExpandedDialogOpen, setIsExpandedDialogOpen] = useState(false);

	const getResourceTierFromServer = useCallback(
		(resources?: { cpuShares?: string; memoryShares?: string }) => {
			if (!resources) {
				return "Small";
			}
	...
	const getCurrentData = useCallback((): LocalMCPServer | null => {
		...
		if (mcpServerData?.mcpServer) {
			return {
				...
				resources: {
					tier: getResourceTierFromServer(mcpServerData.mcpServer.resources),
				},
				tools: Object.entries(mcpServerData.mcpServer.tools).map(
					([name, tool]) => ({
						id: name,
						name,
		...
		try {
			...
			if (
				...
				tagsMap[CLOUD_MANAGED_TAG_KEYS.SERVICE_ACCOUNT_ID] =
					mcpServerData.mcpServer.tags[
						CLOUD_MANAGED_TAG_KEYS.SERVICE_ACCOUNT_ID
					];
			}
			if (mcpServerData.mcpServer.tags[CLOUD_MANAGED_TAG_KEYS.SECRET_ID]) {
				tagsMap[CLOUD_MANAGED_TAG_KEYS.SECRET_ID] =
					mcpServerData.mcpServer.tags[CLOUD_MANAGED_TAG_KEYS.SECRET_ID];
			}
					...
					mcpServer: {
						...
						resources: {
							memoryShares: convertToApiMemoryFormat(
								getResourceTierByName(currentData.resources.tier)?.memory ||
									"512M",
							),
							cpuShares:
								getResourceTierByName(currentData.resources.tier)?.cpu ||
								"200m",
						},
					},
	...
	const handleLintTool = async (toolId: string) => {
		...
		const toolsMap: Record<
			string,
			{ componentType: number; configYaml: string }
		> = {
			[tool.name.trim()]: {
				componentType: tool.componentType,
				configYaml: tool.config,
	...
	const handleLintAllTools = async (): Promise<boolean> => {
		...
		try {
			...
			if (hasIssues) {
				toast.error(
					"Configuration has linting issues. Please fix them before saving.",
				);
				return false;
			}
	...
	const getResourceTierByName = (name: string) =>
		RESOURCE_TIERS.find((tier) => tier.name === name || tier.id === name);

	const getResourceDisplayString = (resources: {
		cpuShares?: string;
		memoryShares?: string;
	}): string => {
	...
	const hasSecretWarnings = React.useMemo(() => {
		...
		return detectedSecrets.some(
			(secretName) => !existingSecrets.includes(secretName),
		);
	...
	return (
		<div>
			<div className="space-y-6">
									...
									<div className="flex gap-2">
										{isEditing ? (
											<>
												<Button
													disabled={
														isUpdateMCPServerPending || isLintConfigPending
													}
													onClick={handleSave}
													variant="secondary"
												>
											...
											{isEditing ? (
												...
												<div className="flex h-10 items-center rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
													<code className="font-mono text-sm">
														{getResourceDisplayString(
															mcpServerData?.mcpServer?.resources || {},
														)}
													</code>
												</div>
											)}
										...
										<div className="flex flex-col gap-2 space-y-4">
											...
											<div className="space-y-2">
												...
												{displayData.tags.map((tag, index) => {
													const duplicateKeys = isEditing
														? getDuplicateKeys(displayData.tags)
														: new Set();
													const isDuplicateKey =
														tag.key.trim() !== "" &&
														duplicateKeys.has(tag.key.trim());
													return (
														<div
															className="flex items-center gap-2"
															key={`tag-${tag.key}-${tag.value}`}
														>
				...
				<div
					className={`grid grid-cols-1 gap-6 ${hasSecretWarnings && isEditing ? "xl:grid-cols-3" : ""}`}
				>
					{/* Main tools configuration - takes 2 columns on xl screens when secrets panel is shown, full width otherwise */}
					<div
						className={hasSecretWarnings && isEditing ? "xl:col-span-2" : ""}
					>
						<Card className="px-0 py-0" size="full">
							<CardHeader className="border-b p-4 dark:border-border [.border-b]:pb-4">
								<CardTitle className="flex items-center gap-2">
											...
											{displayData.tools.map((tool) => (
												<RemoteMCPToolButton
													...
													hasLintIssues={
														isEditing &&
														lintHints[tool.id] &&
														Object.keys(lintHints[tool.id]).length > 0
													}
													id={tool.id}
													isEditing={isEditing}
										...
										<div className="space-y-4">
															...
															<SelectTrigger>
																<SelectValue>
																	<RedpandaConnectComponentTypeBadge
																		componentType={selectedTool.componentType}
																	/>
																</SelectValue>
															</SelectTrigger>
															<SelectContent>
																{Object.values(MCPServer_Tool_ComponentType)
																	.filter(
																		(value) =>
																			typeof value === "number" &&
																			value !==
																				MCPServer_Tool_ComponentType.UNSPECIFIED,
																	)
																	.map((componentType) => (
																		<SelectItem
																			key={componentType}
											...
											<div className="space-y-2">
												<YamlEditorCard
													...
													key={selectedTool.id}
													onChange={(value) =>
														handleUpdateTool(selectedTool.id, { config: value })
													}
													onExpand={() => setIsExpandedDialogOpen(true)}
													onLint={
														isEditing
															? () => handleLintTool(selectedTool.id)
															: undefined
													}
													options={{
														readOnly: !isEditing,
					...
					{Boolean(hasSecretWarnings && isEditing) && (
						<div className="xl:col-span-1">
							<div className="sticky top-4">
								<QuickAddSecrets
									existingSecrets={existingSecrets.filter(
										(secretId): secretId is string => Boolean(secretId),
									)}
						...
						<Card className="px-0 py-0" size="full">
							<CardHeader className="border-b p-4 dark:border-border [.border-b]:pb-4">
								...
								<Text className="text-sm" variant="muted">
									The service account is used by the MCP server to authenticate
									to other systems within the Redpanda Cloud platform (e.g.
									Redpanda broker).
								</Text>
							</CardHeader>
							<CardContent className="px-4 pb-4">
