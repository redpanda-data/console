...
class ReassignPartitions extends PageComponent {
	...
	componentDidMount() {
		...
		this.startRefreshingTopicConfigs =
			this.startRefreshingTopicConfigs.bind(this);
		this.stopRefreshingTopicConfigs =
			this.stopRefreshingTopicConfigs.bind(this);
		...
		this.resetSelectionOnErrorsReactionDisposer = autorun(() => {
			if (this.selectedBrokerIds.length == 0) {
				...
				if (
					selectedTopicPartitions.length == 0 ||
					selectedTopicPartitions.all((x) => x.length == 0)
				)
			...
			if (
				this.selectedBrokerIds.any(
					(x) =>
						api.clusterInfo?.brokers.find((b) => b.brokerId == x) == undefined,
				)
			)
				reset = true;
			...
			if (reset == false)
				if (this.selectedTopicPartitions == null) reset = true;
	...
	render() {
		...
		const partitionCountOnlyReplicated = api.topics?.sum(
			(t) => t.partitionCount * (t.replicationFactor - 1),
		);

		const step = steps[this.currentStep];
		const nextButtonCheck = step.nextButton.isEnabled(this);
		const nextButtonEnabled = nextButtonCheck === true;
		const nextButtonHelp =
			typeof nextButtonCheck === "string" ? (nextButtonCheck as string) : null;
					...
					<PageContent>
						...
						<Section py={4}>
							<Flex>
								<Statistic
									title="Broker Count"
									value={api.clusterInfo?.brokers.length}
								/>
								<Statistic
									title="Leader Partitions"
									value={partitionCountLeaders ?? "..."}
								/>
								<Statistic
									title="Replica Partitions"
									value={partitionCountOnlyReplicated ?? "..."}
								/>
								<Statistic
					...
					>
						...
						<ModalContent minW="5xl">
							...
							<ModalBody>
								<div>
									<div>
										There are {this.topicsWithThrottle.length} topics with
										throttling applied to their replicas.
										<br />
										...
										>
											...
											>
												Kowl sets those two configuration entries when
												throttling a topic reassignment:
										...
										<br />
										So if you previously used Kowl to reassign any of the
										partitions of the following topics, the throttling config
							...
							<ModalFooter gap={2}>
								...
								<Button
									onClick={async () => {
										...
										const result = await api.resetThrottledReplicas(
											this.removeThrottleFromTopicsContent,
										);
										...
										if (errors.length == 0) {
											...
											console.error(
												"errors in removeThrottleFromTopics",
												errors,
											);
										}
	...
	onNextPage() {
		...
		if (this.currentStep == 1) {
			...
			if (targetBrokers.any((b) => b == null))
				throw new Error(
					"one or more broker ids could not be mapped to broker entries",
				);
			...
			this.reassignmentRequest =
				topicAssignmentsToReassignmentRequest(topicAssignments);
			...
			setTimeout(async () => {
				try {
					...
					toast({
						status: "error",
						description:
							"Error starting partition reassignment.\nSee console for more information.",
						duration: 3000,
					});
	...
	async startReassignment(
		request: PartitionReassignmentRequest,
	): Promise<boolean> {
		if (
			uiSettings.reassignment.maxReplicationTraffic != null &&
			uiSettings.reassignment.maxReplicationTraffic > 0
		) {
		...
		try {
			...
			const errors = response.reassignPartitionsResponses
				...
				.filterNull();
			const startedCount = response.reassignPartitionsResponses.sum((x) =>
				x.partitions.count((p) => !p.errorCode),
			);
	...
	async setTrafficLimit(
		request: PartitionReassignmentRequest,
	): Promise<boolean> {
		const maxBytesPerSecond = Math.round(
			uiSettings.reassignment.maxReplicationTraffic ?? 0,
		);
		...
		for (const t of request.topics) {
			...
			for (const p of t.partitions) {
				...
				const brokersOld = api.topicPartitions
					?.get(t.topicName)
					?.first((p) => p.id == partitionId)?.replicas;
				const brokersNew = p.replicas;

				if (brokersOld == null || brokersNew == null) {
					console.log(
						"traffic limit: skipping partition because old or new brokers can't be found",
						{ topicName, partitionId, brokersOld, brokersNew },
					);
					continue;
				}
	...
	startRefreshingTopicConfigs() {
		if (IsDev)
			console.log("starting refreshTopicConfigs", { stack: new Error().stack });
		if (this.refreshTopicConfigsTimer == null)
			this.refreshTopicConfigsTimer = setInterval(
				this.refreshTopicConfigs,
				6000,
			);
	}
	stopRefreshingTopicConfigs() {
		if (IsDev)
			console.log("stopping refreshTopicConfigs", { stack: new Error().stack });
	...
	async refreshTopicConfigs() {
		try {
			...
			console.error(
				"error while refreshing topic configs, stopping auto refresh",
				{ error: err },
			);
			this.stopRefreshingTopicConfigs();
		} finally {
	...
	@computed get maximumSelectedReplicationFactor(): number {
		...
		for (const topicName in this.partitionSelection) {
			...
			if (topic)
				if (topic.replicationFactor > maxRf) maxRf = topic.replicationFactor;
		}
		return maxRf;
	}
	...
	},
	{
		...
		nextButton: {
			...
			computeWarning: (rp) => {
				...
				const selectedRacks = selectedBrokers
					.map((x) => x.rack ?? "")
					.distinct();
