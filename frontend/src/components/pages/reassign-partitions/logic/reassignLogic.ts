...
function computeReassignments(
	...
	for (const t of selectedTopicPartitions) {
		...
		for (const partition of t.partitions)
			resultAssignments[t.topic.topicName][partition.id] = {
				partition: partition,
				brokers: [],
			};
	}
	...
	for (let round = 0; round < 10; round++) {
		const { range: skewBefore } = calcRange(
			allExBrokers,
			(x) => x.plannedLeader,
		);
		const leaderSwitchCount = balanceLeaders(
			selectedTopicPartitions,
			resultAssignments,
			allExBrokers,
			apiData,
		);
		const { range: skewAfter } = calcRange(
			allExBrokers,
			(x) => x.plannedLeader,
		);
	...
	const criticalBrokers = findRiskyPartitions(
		allExBrokers,
		selectedTopicPartitions,
		resultAssignments,
		getExBroker,
...
const untrackedCompute = function (
	apiData: ApiData,
	selectedTopicPartitions: TopicPartitions[],
	targetBrokers: Broker[],
): TopicAssignments {
	return untracked(() =>
		computeReassignments(apiData, selectedTopicPartitions, targetBrokers),
	);
};
...
function computeTopicAssignments(
	...
	const brokerReplicaCount: BrokerReplicaCount[] = targetBrokers.map((b) => ({
		broker: b,
		assignedReplicas: 0,
	}));
	...
	for (const partition of partitions) {
		...
		const replicaAssignments = computeReplicaAssignments(
			partition,
			replicationFactor,
			brokerReplicaCount,
			allBrokers,
...
function computeReplicaAssignments(
	...
	const sourceBrokers = partition.replicas.map(
		(id) => allBrokers.first((b) => b.brokerId == id)!,
	);
	...
	const replicasInRack = (rack: string) =>
		brokerReplicaCount
			.filter((x) => x.broker.rack == rack)
			.sum((x) => x.assignedReplicas);
		...
		for (const p of t.partitions) {
			...
			if (betterLeader != plannedLeader) {
				...
				resultAssignments[t.topic.topicName][p.id].brokers = newBrokers.map(
					(exBroker) =>
						apiData.brokers.first((b) => b.brokerId == exBroker.brokerId)!,
				);
...
function findRiskyPartitions(
	...
	for (const t of selectedTopicPartitions) {
		...
		for (const partition of partitions) {
			...
			const replicaBrokers = partitionAssignments.brokers.map((b) =>
				getExBroker(b.brokerId),
			);
			...
			if (
				riskyPartition.criticalRacks.length > 0 ||
				riskyPartition.criticalBrokers.length > 0
			)
...
function reportRiskyPartitions(riskyPartitions: RiskyPartition[]) {
	for (const {
		key: topic,
		items: partitionIssues,
	} of riskyPartitions.groupInto((x) => x.topic)) {
		// topics with rf:1 will always be an issue
		if (topic.replicationFactor <= 1) continue;

		// check if any partition has any issues
		const issues = partitionIssues.filter(
			(x) => x.criticalBrokers.length > 0 || x.criticalRacks.length > 0,
		);
		...
		const table = issues.map((x) => ({
			...
			criticalBrokers: x.criticalBrokers
				.map((b) => String(b.brokerId))
				.join(", "),
			criticalRacks: x.criticalRacks.join(", "),
		}));
		console.error(
			`Issues in topic "${topic.topicName}" (RF: ${topic.replicationFactor}) (${issues.length} issues).`,
		);
		console.table(table);
	}
}
	...
	private recomputeActual(apiData: ApiData) {
		...
		for (const [topic, partitions] of apiData.topicPartitions) {
			...
			for (const p of partitions) {
				// replicas
				const replicasAssignedToThisBroker = p.replicas.count(
					(x) => x == this.brokerId,
				);
				...
				if (replicasAssignedToThisBroker > 0) {
					...
					const logDirEntry = p.partitionLogDirs.first(
						(x) => !x.error && x.brokerId == this.brokerId,
					);
...
function checkArguments(
	...
	const topicsMissingPartitionData = apiData.topics.filter(
		(t) => apiData.topicPartitions.get(t.topicName) == null,
	);
	if (topicsMissingPartitionData.length > 0)
		throw new Error(
			"apiData is missing topicPartitions for these topics: " +
				topicsMissingPartitionData.map((t) => t.topicName).join(", "),
		);
