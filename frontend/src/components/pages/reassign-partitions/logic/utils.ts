...
export function partitionSelectionToTopicPartitions(
	...
	for (const topicName in partitionSelection) {
		...
		const relevantPartitions = partitionIds.map((id) =>
			allPartitions.first((p) => p.id == id),
		);
...
export function computeMovedReplicas(
	...
	for (const [topicName, allPartitions] of apiTopicPartitions) {
		...
		const selectedPartitions = allPartitions.filter((p) =>
			partitionSelection[topicName].includes(p.id),
		);
 ...
 */
export function removeRedundantReassignments(
	...
	for (const t in topicAssignments) {
		...
		for (const partitionId in topicAssignment) {
			...
			const brokersBefore = curTopicPartitions.first(
				(p) => p.id == Number(partitionId),
			)?.replicas;
			...
			if (brokersBefore.length != brokersAfter.length) {
				console.warn(
					"remove empty reassignments: brokersBefore.length != brokersAfter.length",
				);
				continue;
			}
