...
export class ReassignmentTracker {
	...
	async refreshReassignments() {
		...
		const liveReassignments = (clone(api.partitionReassignments) ?? []).map(
			(r) => ({ id: this.computeId(r), ...r }),
		);
		...
		transaction(() => {
			...
			for (const r of liveReassignments) {
				const existingState = this.trackingReassignments.first(
					(x) => x.id == r.id,
				);
			...
			const expiredTrackers = this.trackingReassignments.filter((x) => {
				...
				const age =
					(new Date().getTime() - x.actualTimeCompleted.getTime()) / 1000;
				if (age > 8) {
	...
	updateReassignmentState(state: ReassignmentState) {
		...
		for (const p of state.partitions) {
			const logDirs = topicPartitions
				?.first((e) => e.id == p.partitionId)
				?.partitionLogDirs.filter((l) => !l.error);
		...
		state.totalTransferSize = state.partitions.sum(
			(p) => p.maxReplicaSize * p.addingReplicas.length,
		);

		const newTransferred = state.partitions.sum((p) =>
			p.currentSize.sum((x) => x.replicaSize),
		);
		const newRemaining = state.totalTransferSize - newTransferred;

		if (
			state.remainingPrev == null ||
			state.remainingPrev.value != newRemaining
		) {
			...
			state.progressPercent =
				(1 - state.remaining.value / state.totalTransferSize) * 100;
		}
		...
		if (
			state.remaining &&
			state.remainingPrev &&
			state.remaining.value != state.remainingPrev.value
		) {
			const intervalSec =
				(state.remaining.timestamp.getTime() -
					state.remainingPrev.timestamp.getTime()) /
				1000;
		...
		if (state.estimateSpeed !== null && state.remaining !== null) {
			...
			state.estimateCompletionTime = new Date(
				new Date().getTime() + remainingTimeSec * 1000,
			);
		}
