...
export class StepSelectPartitions extends Component<{
	...
	render() {
		...
		return (
			<div style={{ margin: "1em 1em 2em 1em" }}>
				{/* Current Selection */}
				<SelectionInfoBar
					partitionSelection={this.props.partitionSelection}
						...
						return (
							<SelectPartitionTable
								...
								getSelectedPartitions={() =>
									this.getSelectedPartitions(topic.topicName)
								}
							/>
						);
					}}
					columns={[
						{
							...
							cell: observer(({ row }: { row: Row<TopicWithPartitions> }) => {
								const { checked, indeterminate } = this.getTopicCheckState(
								...
								return (
									<Checkbox
										...
										onChange={() =>
											this.setTopicSelection(row.original, !checked)
										}
									/>
								);
							}),
						},
						{
							...
							header: () => (
								<SearchTitle
									...
									observableSettings={uiSettings.reassignment}
								/>
							),
							...
							cell: ({ row: { original: record } }) => {
								const content = filterActive ? (
									<Highlighter
										searchWords={[query]}
										textToHighlight={record.topicName}
									/>
								) : (
									record.topicName
								);
								...
								if (this.props.throttledTopics.includes(record.topicName)) {
									return (
										<Box wordBreak="break-word" whiteSpace="break-spaces">
											<span>{content}</span>
											<WarningToolip
												content="Topic replication is throttled"
												position="top"
											/>
										</Box>
									);
						...
						},
						{
							...
							accessorKey: "partitions",
							cell: ({ row: { original: record } }) =>
								record.partitions?.map((p) => p.leader).distinct().length ??
								"N/A",
						},
						{
							header: "Size",
							cell: ({ row: { original: r } }) =>
								renderLogDirSummary(r.logDirSummary),
	...
	getTopicCheckState(topicName: string): {
		checked: boolean;
				...
				columns={[
					{
						header: "Check",
						cell: observer(
							({ row: { original: partition } }: { row: Row<Partition> }) => {
					...
					},
					{
						...
						cell: ({ row: { original: partition } }) =>
							prettyBytesOrNA(partition.replicaSize),
						size: Infinity,
					},
				]}
			/>
