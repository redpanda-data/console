...
export class StepSelectBrokers extends Component<{
	...
	render() {
		if (!this.brokers || this.brokers.length == 0) {
			console.error("brokers", {
				brokers: this.brokers,
				apiClusterInfo: api.clusterInfo,
			});
		...
		return (
			<>
				<div style={{ margin: "2em 1em" }}>
					...
					<p>
						Choose the target brokers to move the selected partitions to. Kowl
						will consider them as desired targets and distribute partitions
					...
					columns={[
						{
							...
							header: observer(() => {
								...
								const allIdsSet = new Set<number>(
									this.brokers.map(({ brokerId }) => brokerId),
								);
							...
							cell: observer(
								({ row: { original: broker } }: { row: Row<Broker> }) => {
						...
						{ header: "Rack", accessorKey: "rack" },
						{
							...
							cell: ({ row: { original } }) =>
								prettyBytesOrNA(original.logDirSize),
						},
					]}
				/>
			</>
