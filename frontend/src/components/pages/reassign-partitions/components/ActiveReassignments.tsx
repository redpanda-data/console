...
export class ActiveReassignments extends Component<{
	...
	render() {
		...
		const currentReassignments =
			reassignmentTracker.trackingReassignments ?? [];
		...
		return (
			<>
				...
				{/* Title */}
				<div
					className="currentReassignments"
					style={{
						display: "flex",
						placeItems: "center",
						marginBottom: ".5em",
				...
				<ReassignmentDetailsDialog
					...
					onClose={() => (this.reassignmentDetails = null)}
				/>
				<ThrottleDialog
				...
				{this.props.throttledTopics.length > 0 && (
					...
					>
						<span>
							There are <b>{this.props.throttledTopics.length}</b> throttled
							topics - click here to fix
						</span>
					</Button>
				)}
			</>
	...
	@computed get throttleSettings(): {
		followerThrottle: number | undefined;
		leaderThrottle: number | undefined;
	} {
		...
		const result = {
			...
			followerThrottle: followerThrottle
				? Number(followerThrottle.value)
				: undefined,
		};
...
export const ThrottleDialog: FC<{
	...
	const noChange =
		$state.newThrottleValue === lastKnownMinThrottle ||
	...
	const applyBandwidthThrottle = async () => {
		...
		try {
			...
			if ($state.newThrottleValue && $state.newThrottleValue > 0) {
				await api.setReplicationThrottleRate(
					allBrokers,
					$state.newThrottleValue,
				);
			} else {
			...
			<ModalContent minW="3xl">
				...
				<ModalBody>
					<Flex flexDirection="column" gap={4}>
						<Box mx={4}>
							<Text>
								Using throttling you can limit the network traffic for
								reassignments.
							</Text>
							<UnorderedList mt={2} px={6}>
								<ListItem>
									Throttling applies to all replication traffic, not just to
									active reassignments.
								</ListItem>
								<ListItem>
									Once the reassignment completes you'll have to remove the
									throttling configuration. <br />
									Console will show a warning below the "Current Reassignments"
									table when there are throttled topics that are no longer being
									reassigned.
								</ListItem>
							</UnorderedList>
						</Box>
						<BandwidthSlider
							value={throttleValue}
							onChange={(x) => ($state.newThrottleValue = x)}
						/>
					</Flex>
				</ModalBody>
...
const CancelReassignmentButton: FC<{ onConfirm: () => void }> = ({
	onConfirm,
}) => {
...
@observer
export class ReassignmentDetailsDialog extends Component<{
	...
	render() {
		...
		const addingReplicas = state.partitions
			.flatMap((p) => p.addingReplicas)
			.distinct();
		const removingReplicas = state.partitions
			.flatMap((p) => p.removingReplicas)
			.distinct();
			...
			<Flex flexDirection="column" gap={12}>
				...
				<Flex gap={4}>
					<Checkbox
						...
						onChange={(e) => (this.shouldThrottle = e.target.checked)}
					>
						<span>
	...
	isThrottled(): boolean {
		...
		const leaderThrottleValue = config.configEntries.first(
			(e) => e.name == "leader.replication.throttled.replicas",
		);
		...
		if (leaderThrottleEntries) {
			...
			for (const p of this.lastState.partitions) {
				...
				const hasThrottle = leaderThrottleEntries.any(
					(e) =>
						e.partitionId == p.partitionId &&
						sourceBrokers.includes(e.brokerId),
				);
		...
		if (followerThrottleEntries) {
			...
			for (const p of this.lastState.partitions) {
				...
				const hasThrottle = followerThrottleEntries.any(
					(e) =>
						e.partitionId == p.partitionId &&
						targetBrokers.includes(e.brokerId),
				);
	...
	async cancelReassignment() {
		...
		try {
			...
			console.log("cancel reassignment result", {
					...
					right={
						<>
							{state.estimateSpeed != null && (
								<span style={{ paddingRight: "1em", opacity: "0.6" }}>
									({prettyBytesOrNA(state.estimateSpeed)}/s)
								</span>
							)}
							<span>
								{prettyBytesOrNA(transferred)} /{" "}
								{prettyBytesOrNA(state.totalTransferSize)}
							</span>
						</>
					}
				/>
...
export class ETACol extends Component<{ state: ReassignmentState }> {
	render() {
		...
		const remainingMs = (
			state.estimateCompletionTime.getTime() - new Date().getTime()
		).clamp(0, undefined);

		return (
			<span>
				{prettyMilliseconds(remainingMs, {
