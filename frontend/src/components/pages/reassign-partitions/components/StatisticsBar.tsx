...
@observer
export class SelectionInfoBar extends Component<{
	...
	render() {
		...
		const allSelectedPartitions = this.selectedPartitions.flatMap(
			(p) => p.partitions,
		);
		const partitionCountLeaders = allSelectedPartitions.length; // every partition has a leader
		const totalPartitionsCount = allSelectedPartitions.sum(
			(t) => t.replicas.length,
		);
		...
		const data = [
			...
			{
				title: "Involved Racks",
				value: brokers?.map((b) => b.rack).distinct().length ?? "...",
			},
			{
				title: "Size",
				value: prettyBytesOrNA(
					allSelectedPartitions.sum((p) => p.replicas.length * p.replicaSize),
				),
			},
		];
	...
	@computed get selectedPartitions(): {
		topic: string;
		...
		for (const [topic, partitions] of api.topicPartitions) {
			...
			const relevantPartitions = partitions.filter((p) =>
				this.props.partitionSelection[topic].includes(p.id),
			);
