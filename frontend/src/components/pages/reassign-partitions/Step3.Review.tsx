		...
		return (
			<>
				<div style={{ margin: "2em 1em" }}>
					...
					<p>
						Kowl computed the following reassignment plan to distribute the
						selected partitions onto the selected brokers.
					</p>
				</div>
				...
				<DataTable<TopicWithMoves>
					...
					columns={[
						...
						},
						{
							...
							cell: ({ row: { original: topic } }) =>
								topic.selectedPartitions.sum((p) => p.numAddedBrokers),
						},
						{
							...
							cell: ({ row: { original: topic } }) =>
								prettyBytesOrNA(
									topic.selectedPartitions.sum(
										(p) => p.numAddedBrokers * p.replicaSize,
									),
								),
						},
					]}
						...
						<Box py={6} px={10}>
							{topic.selectedPartitions ? (
								<ReviewPartitionTable
									...
									assignments={
										this.props.assignments.topics.first(
		...
		return (
			<div style={{ margin: "4em 1em 3em 1em" }}>
				...
				<ul style={{ marginTop: "0.5em" }}>
					<li>
						Throttling applies to all replication traffic, not just to active
						reassignments.
					</li>
					<li>
						Once the reassignment completes you'll have to remove the throttling
						configuration. <br />
						Console will show a warning below the "Current Reassignments" table
						when there are throttled topics that are no longer being reassigned.
					</li>
				</ul>
			</div>
		);
	...
	summary() {
		...
		const trafficStats = this.props.topicsWithMoves.map((t) => {
			const partitionStats = t.selectedPartitions.map((p) => {
				...
				const participatingTransferPairs = Math.min(
					senders.length,
					receivers.length,
				);
				const potentialBandwidth =
					participatingTransferPairs * maxReplicationTraffic;
		...
		const estimatedTimeSec = trafficStats.sum((t) =>
			t.partitionStats.sum((p) => p.estimatedTimeSec),
		);

		const totalTraffic = trafficStats.sum((t) =>
			t.partitionStats.sum((p) => p.totalTraffic),
		);

		const isThrottled =
			settings.maxReplicationTraffic != null &&
		...
		const estimatedTime = !isThrottled
			...
			: estimatedTimeSec < 10
				? "< 10 seconds"
				: prettyMilliseconds(estimatedTimeSec * 1000, {
		...
		const data = [
			{
				...
				value: this.props.topicsWithMoves.sum((t) =>
					t.selectedPartitions.sum((p) => p.numAddedBrokers),
				),
			},
				...
				>
					{data.map((item) => (
						<div key={item.title}>
							...
							<div style={{ fontSize: "calc(1em * 24 / 14)" }}>
								{item.value}
							</div>
						</div>
					))}
			...
			<DataTable<Partition>
				...
				columns={[
					...
					},
					{
						...
						cell: ({ row: { original: partition } }) => {
							const partitionAssignments = props.assignments.partitions.first(
							...
							return (
								<BrokerList
									brokerIds={partitionAssignments.replicas}
									leaderId={partitionAssignments.replicas[0]}
								/>
							);
						},
					},
