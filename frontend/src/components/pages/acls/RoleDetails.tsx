...
class RoleDetailsPage extends PageComponent<{ roleName: string }> {
	...
	initPage(p: PageInitHelper): void {
		...
		p.addBreadcrumb(
			...
			`/security/roles/${this.props.roleName}`,
		);
	...
	render() {
		...
		const numberOfPrincipals =
			rolesApi.roleMembers.get(this.roleName)?.length ?? 0;
				...
				<PageContent>
						...
						<Button
							...
							onClick={() => {
								appGlobal.history.push(
									`/security/roles/${this.props.roleName}/edit`,
								);
							}}
						>
							Edit
					...
					<Flex flexDirection="column">
						...
						<Text>
							...
							{numberOfPrincipals === 1 ? "principal" : "principals"}
						</Text>
						<Box my={2}>
							<SearchField
								width="300px"
