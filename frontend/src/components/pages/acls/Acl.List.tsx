...
class AclList extends PageComponent<{ tab: AclListTab }> {
	...
	render() {
		...
		if (!api.serviceAccounts || !api.serviceAccounts.users)
		...
		const tabs = [
			...
			},
			{
				...
				component: (
					<AclsTab
						data-testid="acls-tab"
						principalGroups={principalGroupsView.principalGroups}
					/>
				),
			},
			{
...
const PermissionsListTab = observer(() => {
	...
	return (
		<Flex flexDirection="column" gap="4">
			<Box>
				This page provides a detailed overview of all effective permissions for
				each principal, including those derived from assigned roles. While the
				ACLs tab shows permissions directly granted to principals, this tab also
				incorporates roles that may assign additional permissions to a
				principal. This gives you a complete picture of what each principal can
				do within your cluster.
			</Box>
						...
						columns={[
							...
							},
							{
								...
								cell: (ctx) => {
									const entry = ctx.row.original;
									return (
										<UserRoleTags userName={entry.name} showMaxItems={2} />
									);
								},
							},
						]}
...
const UsersTab = observer(() => {
	...
	return (
		<Flex flexDirection="column" gap="4">
			<Box>
				These users are SASL-SCRAM users that are managed by your cluster. Other
				authentication identities (OIDC, Kerberos, mTLS) will not be listed
				here. You can view their permissions in the permissions list.
			</Box>
												...
												<button
													onClick={() => {
														appGlobal.history.push(
															`/security/users/${entry.name}/edit`,
														);
													}}
												>
													<Icon as={PencilIcon} />
...
const RolesTab = observer(() => {
	...
	return (
		<Flex flexDirection="column" gap="4">
			<Box>
				Roles are groups of ACLs abstracted under a single name. Roles can be
									...
									return (
										<Flex flexDirection="row" gap={4}>
											<button
												onClick={() => {
													appGlobal.history.push(
														`/security/roles/${entry.name}/edit`,
													);
												}}
											>
												<Icon as={PencilIcon} />
...
const AclsTab = observer(
	(p: {
		...
		const [edittingPrincipalGroup, setEdittingPrincipalGroup] =
			useState<AclPrincipalGroup | null>(null);

		let groups = p.principalGroups.filter((g) => g.principalType == "User");
		try {
			const quickSearchRegExp = new RegExp(
				uiSettings.aclList.configTable.quickSearch,
				"i",
			);
			groups = groups.filter((aclGroup) =>
				aclGroup.principalName.match(quickSearchRegExp),
			);
		} catch (e) {
		...
		return (
			<Flex flexDirection="column" gap="4">
				<Box>
					This tab displays all Kafka Access Control Lists (ACLs), grouped by
					each principal. A principal represents any entity that can be
					authenticated, such as a user, service, or system (e.g., a SASL-SCRAM
					user, OIDC identity, Kerberos principal, or mTLS client). The ACLs tab
					shows only the permissions directly granted to each principal, without
					considering any permissions that may be derived from assigned roles.
					For a complete view of all effective permissions, including those
					granted through roles, refer to the Permissions List tab.
				</Box>

				<Alert status="info">
					<AlertIcon />
					Roles are a more flexible and efficient way to manage user
					permissions, especially with complex organizational hierarchies or
							...
							columns={[
								...
								},
								{
									...
									cell: ({
										...
										!host || host == "*" ? (
											<Badge variant="subtle">Any</Badge>
										) : (
											host
										),
								},
								{
									...
									header: "",
									cell: ({ row: { original: record } }) => {
										const userExists = api.serviceAccounts?.users.includes(
											record.principalName,
										);
											...
											if (acls) {
												try {
													await api.deleteACLs({
														...
														principal:
															record.principalType + ":" + record.principalName,
														host: record.host,
														operation: "Any",
														permissionType: "Any",
													});
													toast({
														...
														description: (
															<Text as="span">
																Deleted ACLs for{" "}
																<Code>{record.principalName}</Code>
															</Text>
														),
													});
												} catch (err: unknown) {
													console.error("failed to delete acls", {
														error: err,
													});
													setAclFailed({ err });
												}
										...
										return (
											<Menu>
												...
												<MenuList>
													<MenuItem
														isDisabled={
															!userExists || !Features.deleteUser || !hasAcls
														}
														onClick={(e) => {
				...
				<AlertDialogContent>
					...
					<AlertDialogBody>
						<div className="codeBox">
							{aclFailed !== null && toJson(aclFailed.err)}
						</div>
					</AlertDialogBody>
					<AlertDialogFooter>
