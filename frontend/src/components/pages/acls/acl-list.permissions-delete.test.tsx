...
vi.mock('config', async (importOriginal) => {
  ...
  return { ...actual, isServerless: () => false };
});
...
vi.mock('./models', () => ({
  principalGroupsView: { principalGroups: [] },
}));
...
vi.mock('../../../state/ui-state', () => ({
  uiState: { pageBreadcrumbs: [], pageTitle: '' },
}));
...
vi.mock('react-query/api/security', () => ({
  useDeleteRoleMutation: () => ({ mutateAsync: vi.fn() }),
  useListRolesQuery: () => ({ data: { roles: [] }, error: null, isError: false }),
}));
...
describe('Permissions List - delete dropdown for different principal types', () => {
  ...
  test('Group principal does not show "Delete User" options in dropdown', async () => {
    ...
    // Permissions List tab is index 3 in the Tabs component
    render(<AclList tab="permissions-list" />);
