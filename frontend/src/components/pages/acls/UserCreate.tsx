	...
	async onCreateUser(): Promise<boolean> {
		try {
			...
			for (const r of this.selectedRoles) {
				roleAddPromises.push(
					rolesApi.updateRoleMembership(r, [this.username], [], false),
				);
			}
...
const CreateUserConfirmationModal = observer(
	(p: { state: CreateUserModalState; closeModal: () => void }) => {
		return (
			<>
				...
				<Alert status="info" mt={4} mb={4}>
					<AlertIcon />
					You will not be able to view this password again. Make sure that it is
					copied and saved.
				</Alert>
