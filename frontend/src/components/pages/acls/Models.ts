...
function modelPatternTypeToUIType(
	resourcePatternType: AclStrResourcePatternType,
	resourceName: string,
) {
...
function collectTopicAcls(acls: AclFlat[]): TopicACLs[] {
	...
	for (const { items } of topics) {
		...
		const topicPermissions: {
			[key in (typeof topicOperations)[number]]: AclStrPermission;
		} = {
			Alter: "Any",
			AlterConfigs: "Any",
			Create: "Any",
		...
		const topicAcl: TopicACLs = {
			patternType: modelPatternTypeToUIType(
				first.resourcePatternType,
				selector,
			),
			selector,
...
function collectConsumerGroupAcls(acls: AclFlat[]): ConsumerGroupACLs[] {
	...
	for (const { items } of consumerGroups) {
		...
		const groupPermissions: {
			[key in (typeof groupOperations)[number]]: AclStrPermission;
		} = {
			Delete: "Any",
			Describe: "Any",
			Read: "Any",
...
function collectTransactionalIdAcls(acls: AclFlat[]): TransactionalIdACLs[] {
	...
	for (const { items } of transactionalIds) {
		...
		const transactionalIdPermissions: {
			[key in (typeof transactionalIdOperations)[number]]: AclStrPermission;
		} = {
			Describe: "Any",
			Write: "Any",
		};
...
function collectClusterAcls(acls: AclFlat[]): ClusterACLs {
	...
	const clusterPermissions: {
		[key in (typeof clusterOperations)[number]]: AclStrPermission;
	} = {
		Alter: "Any",
		AlterConfigs: "Any",
		ClusterAction: "Any",
...
export function unpackPrincipalGroup(group: AclPrincipalGroup): AclFlat[] {
	...
	for (const topic of group.topicAcls) {
		...
		const resourcePatternType =
			topic.patternType == "Any" ? "Literal" : topic.patternType;
	...
	for (const consumerGroup of group.consumerGroupAcls) {
		...
		// If the user selects 'Any' in the ui, we need to submit pattern type "Literal" and "*" as resourceName
		const resourcePatternType =
			consumerGroup.patternType == "Any"
				? "Literal"
				: consumerGroup.patternType;
	...
	for (const transactionalId of group.transactionalIdAcls) {
		...
		// If the user selects 'Any' in the ui, we need to submit pattern type "Literal" and "*" as resourceName
		const resourcePatternType =
			transactionalId.patternType == "Any"
				? "Literal"
