...
export const AclPrincipalGroupEditor = observer(
	(p: {
		...
		const noNameOrNameInUse =
			p.type == "create" &&
			(!group.principalName ||
				api.ACLs?.aclResources.any((r) =>
					r.acls.any((a) => a.principal == "User:" + group.principalName),
				));
			...
			try {
				...
				if (p.type == "edit") {
					if (group.sourceEntries.length > 0) {
						const requests = group.sourceEntries.map((acl) => {
							...
							const foundIdx = allToCreate.findIndex((x) =>
								comparer.structural(acl, x),
							);
							if (foundIdx !== -1) {
				...
				const requests = allToCreate.map((x) =>
					api.createACL({
						...
						resourcePatternType: x.resourcePatternType as unknown as
							| "Literal"
							| "Prefixed",
						operation: x.operation as unknown as Exclude<
							AclStrOperation,
							"Unknown" | "Any"
						>,
				...
				if (rejected.length) {
					console.error("some create acl requests failed", {
						results,
						rejected,
					});
		...
		return (
			<Modal isOpen onClose={() => {}}>
				...
				<ModalContent minW="6xl">
					<ModalHeader>
						{p.type == "create" ? "Create ACL" : "Edit ACL"}
					</ModalHeader>
					<ModalBody>
						<VStack gap={6} w="full">
							<AnimatePresence>
								{error && (
									<MotionDiv
										animProps={animProps_radioOptionGroup}
							...
							<HStack gap={10} alignItems="flex-end" w="full">
								<Label
									...
									textSuffix={
										<LabelTooltip nowrap left maxW={500}>
											...
											<br />
											Do not include the prefix so <code>my-user</code> instead
											of <code>User:my-user</code>.<br />
								...
								<Label
									...
									textSuffix={
										<LabelTooltip nowrap left maxW={500}>
											The host the user needs to connect from in order for the
											permissions to apply.
											<br />
											Can be set to left empty or set to <code>*</code> to allow
											any host.
										</LabelTooltip>
									}
								>
								...
								<Button
									...
									onClick={() => {
										...
										if (group.transactionalIdAcls.length == 0)
											group.transactionalIdAcls.push(
												createEmptyTransactionalIdAcl(),
											);
							...
							{noNameOrNameInUse == true ? (
								<Box
									...
									fontSize="small"
								>
									Creating new ACLs requires an unused principal name to be
									entered
								</Box>
							) : null}
					...
					<ModalFooter gap={2}>
						...
						<Button
							...
							isDisabled={
								!isFormValid ||
								!isTopicsValid ||
								!isConsumerGroupsValid ||
								!isTransactionalIDValid ||
								noNameOrNameInUse
							}
						>
							OK
			...
			>
				<Flex flexDirection="column" flexGrow={1} gap={10} p={6}>
					{isCluster ? (
						...
						<FormField
							label={`Selector (${resourceName} Name)`}
							errorText={errorText}
							isInvalid={isInvalid}
						>
							<InputGroup
								zIndex={1}
								data-testid={`${resourceName}-input-group`}
							>
