		...
		return (
			<>
				<PageContent>
								...
								onConfirm={async () => {
									...
									for (const [roleName, members] of rolesApi.roleMembers) {
										if (members.any((m) => m.name == userName)) {
											...
											// then remove it
											promises.push(
												rolesApi.updateRoleMembership(roleName, [], [userName]),
											);
										}
									}
					...
					<Box>
						Below are all of the permissions assigned to this principal.
					</Box>
	...
	(p: {
		...
		const groups = principalGroupsView.principalGroups.filter((g) => {
			if (
				...
				(g.principalName == p.userName || g.principalName === "*")
			)
				return true;
			if (g.principalType == "RedpandaRole" && roles.includes(g.principalName))
				return true;
			return false;
		});
				...
				{p.groups.map((r) => (
					<>
						{r.principalType == "RedpandaRole" ? (
							<ChakraLink
								as={ReactRouterLink}
								to={`/security/roles/${r.principalName}/details`}
							>
								<Heading as="h3" mt="4">
									{r.principalName}
								</Heading>
...
export const AclPrincipalGroupPermissionsTable = observer(
	(p: { group: AclPrincipalGroup }) => {
		const entries: {
			type: string;
			selector: string;
			operations: {
		...
		for (const transactId of group.transactionalIdAcls) {
			...
			if (transactId.all == "Allow") allow.push("All");
				...
				for (const [permName, value] of Object.entries(
					transactId.permissions,
				)) {
					if (value == "Allow") allow.push(permName);
					if (value == "Deny") deny.push(permName);
				}
