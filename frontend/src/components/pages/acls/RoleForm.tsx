...
export const RoleForm = observer(({ initialData }: RoleFormProps) => {
	...
	const roleNameAlreadyExist =
		rolesApi.roles.includes(formState.roleName) && !editMode;
		...
		<Box>
			<form
				onSubmit={async (e) => {
					...
					try {
						setIsLoading(true);
						const usersToRemove = originalUsernames.filter(
							(item) => currentUsernames.indexOf(item) === -1,
						);
						...
						history.push(
							`/security/roles/${encodeURIComponent(newRole.roleName)}/details`,
						);
					} catch (err) {
						toast({
							status: "error",
				...
				<Flex gap={10} flexDirection="column">
					<Flex flexDirection="row" gap={20}>
						<Box>
							<FormField
								label="Role name"
								isInvalid={roleNameAlreadyExist}
						...
						<Button
							...
							onClick={() => {
								...
								if (formState.consumerGroupsACLs.length == 0)
									formState.consumerGroupsACLs.push(
										createEmptyConsumerGroupAcl(),
									);
								...
								if (formState.transactionalIDACLs.length == 0)
									formState.transactionalIDACLs.push(
										createEmptyTransactionalIdAcl(),
									);
					...
					<Flex
						...
						gap={4}
						data-testid="create-role-topics-section"
					>
					...
					<Flex
						flexDirection="column"
						gap={4}
						data-testid="create-role-consumer-groups-section"
					>
					...
					<Flex
						flexDirection="column"
						gap={4}
						data-testid="create-role-transactional-ids-section"
					>
					...
					<Flex
						flexDirection="column"
						gap={4}
						data-testid="create-role-cluster-section"
					>
					...
					<Flex flexDirection="column" gap={4}>
						...
						<FormField
							label="Assign this role to principals"
