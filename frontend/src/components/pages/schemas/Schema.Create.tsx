...
@observer
export class SchemaAddVersionPage extends PageComponent<{
	subjectName: string;
}> {
	initPage(p: PageInitHelper): void {
			...
			<Flex gap="4" mt="4">
				<Button
					...
					isDisabled={
						...
						isMissingName ||
						isValidating ||
						editorState.isInvalidKeyOrValue
					}
					...
					onClick={async () => {
						...
						if (
							...
							validationResponse.isCompatible === false
						) {
						...
						try {
							...
							const r = await api
								.createSchema(editorState.computedSubjectName, {
									...
									references: editorState.references.filter(
										(x) => x.name && x.subject,
									),
								})
							...
							const latestVersion =
								api.schemaDetails.get(subjectName)?.latestActiveVersion;
				...
				<Button
					...
					onClick={async () => {
						setValidating(true);
						const r = await validateSchema(editorState).finally(() =>
							setValidating(false),
						);
				...
				<Button
					...
					onClick={() => {
						if (p.parentSubjectName)
							appGlobal.history.replace(
								`/schema-registry/subjects/${encodeURIComponent(p.parentSubjectName)}`,
							);
...
const SchemaEditor = observer(
	(p: {
		...
		const showTopicNameInput =
			state.strategy == "TOPIC" || state.strategy == "TOPIC_RECORD_NAME";
		...
		return (
			<>
				...
				{isAddVersion && (
					<Alert status="info">
						<AlertIcon />
						When adding a new schema version, the only thing that can be changed
						is the schema definition and its references. The rest of the fields
						have been disabled.
					</Alert>
				)}
