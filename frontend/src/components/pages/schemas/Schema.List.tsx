...
class SchemaList extends PageComponent<{}> {
	...
	triggerSearchBySchemaId() {
		...
		if (trimmedValue.length && !isNaN(searchAsNum)) {
			...
			api
				.refreshSchemaUsagesById(searchAsNum)
	...
	render() {
		...
		if (uiSettings.schemaList.quickSearch) {
			filteredSubjects = filteredSubjects
				...
				.filter((s) =>
					this.isFilterMatch(uiSettings.schemaList.quickSearch, s),
				);
		}
			...
			<PageContent key="b">
				...
				<Flex gap="1rem" alignItems="center">
					<SmallStat title="Mode">
						{api.schemaMode ?? <InlineSkeleton width="100px" />}
					</SmallStat>
					<Divider height="2ch" orientation="vertical" />
					<SmallStat title="Compatibility">
						{api.schemaCompatibility ?? <InlineSkeleton width="100px" />}
					</SmallStat>
				</Flex>
				...
				<Button
					...
					width="fit-content"
					onClick={() =>
						appGlobal.history.push("/schema-registry/edit-compatibility")
					}
					disabledReason={
				...
				>
					...
					<DrawerContent>
						<DrawerCloseButton />
						<DrawerHeader data-testid="schema-search-header">
							Schema Search Help
						</DrawerHeader>
						...
						<DrawerBody>
							...
							There are two ways to filter schemas, and they work a little
							differently.
							<Heading size="md" mt={4}>
								Schema ID
							</Heading>
							If a number matches a schema ID, the results include all subjects
					...
					<DataTable<SchemaRegistrySubject>
						...
						columns={[
							...
							},
							{
								...
								cell: ({ row: { original: r } }) => (
									<SchemaCompatibilityColumn name={r.name} />
								),
								size: 100,
							},
							{
								...
								cell: ({ row: { original: r } }) => (
									<LatestVersionColumn name={r.name} />
								),
								size: 100,
							},
							{
