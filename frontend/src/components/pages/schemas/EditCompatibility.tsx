...
import {
	getFormattedSchemaText,
	schemaTypeToCodeBlockLanguage,
} from "./Schema.Details";
			...
			<Section>
				<VStack gap={4}>
					...
					<Text textAlign="center">
						...
						<br />
						To view all registered schemas, their documentation and their
						versioned history simply provide the connection credentials in the
...
@observer
class EditSchemaCompatibilityPage extends PageComponent<{
	subjectName: string;
}> {
					...
					onClose={() => {
						...
						if (subjectName)
							appGlobal.history.replace(
								`/schema-registry/subjects/${encodeURIComponent(subjectName)}`,
							);
...
function EditSchemaCompatibility(p: {
	...
	const { subjectName } = p;
	const subject =
		subjectName != null ? api.schemaDetails.get(subjectName) : undefined;
	const schema = subject?.schemas.first(
		(x) => x.version == subject.latestActiveVersion,
	);

	// type should be just "SchemaRegistryCompatibilityMode"
	const [configMode, setConfigMode] = useState(
		subjectName ? subject?.compatibility : api.schemaCompatibility,
	);
	...
	const onSave = () => {
		const changeReq = subjectName
			? api.setSchemaRegistrySubjectCompatibilityMode(
					subjectName,
					configMode as SchemaRegistryCompatibilityMode,
				)
			: api.setSchemaRegistryCompatibilityMode(
					configMode as SchemaRegistryCompatibilityMode,
				);
	...
	return (
		<>
			<Text>
				Compatibility determines how schema validation occurs when producers are
						...
						<Stack gap="4">
							{subject && (
								<Radio value="DEFAULT">
									Default
									<Text fontSize="small">
										Use the globally configured default.
									</Text>
								</Radio>
							)}
							...
							<Radio value="BACKWARD">
								Backward
								<Text fontSize="small">
									Consumers using the new schema (for example, version 10) can
									read data from producers using the previous schema (for
									example, version 9).
								</Text>
							</Radio>
							...
							<Radio value="BACKWARD_TRANSITIVE">
								...
								<Text fontSize="small">
									Consumers using the new schema (for example, version 10) can
									read data from producers using all previous schemas (for
									example, versions 1-9).
								</Text>
							</Radio>
							...
							<Radio value="FORWARD">
								Forward
								<Text fontSize="small">
									Consumers using the previous schema (for example, version 9)
									can read data from producers using the new schema (for
									example, version 10).
								</Text>
							</Radio>
							...
							<Radio value="FORWARD_TRANSITIVE">
								Transitive Forward
								<Text fontSize="small">
									Consumers using any previous schema (for example, versions
									1-9) can read data from producers using the new schema (for
									example, version 10).
								</Text>
							</Radio>
							...
							<Radio value="FULL">
								Full
								<Text fontSize="small">
									A new schema and the previous schema (for example, versions 10
									and 9) are both backward and forward compatible with each
									other.
								</Text>
							</Radio>
							...
							<Radio value="FULL_TRANSITIVE">
								Transitive Full
								<Text fontSize="small">
									Each schema is both backward and forward compatible with all
									registered schemas.
								</Text>
							</Radio>
						</Stack>
