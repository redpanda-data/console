...
class SchemaDetailsView extends PageComponent<{ subjectName: string }> {
	...
	render() {
		const isSoftDeleted = api.schemaSubjects?.find(
			(x) => x.name == this.subjectNameRaw,
		)?.isSoftDeleted;
			...
			<PageContent key="b">
				...
				<Flex gap="1rem" alignItems="center">
					...
					<SmallStat title="Active Versions">
						{subject.schemas.count((x) => !x.isSoftDeleted)}
					</SmallStat>
				</Flex>
				...
				<Flex gap="2">
					<Button
						...
						onClick={() =>
							appGlobal.history.push(
								`/schema-registry/subjects/${this.subjectNameEncoded}/edit-compatibility`,
							)
						}
						disabledReason={
					...
					<Button
						...
						onClick={() =>
							appGlobal.history.push(
								`/schema-registry/subjects/${this.subjectNameEncoded}/add-version`,
							)
						}
						disabledReason={
...
function getVersionFromQuery(): "latest" | number | undefined {
	...
	if (query.has("version")) {
		...
		console.log(
			`unknown version string in query: "${versionStr}" will be ignored, proceeding with "latest"`,
		);
	}
...
const SubjectDefinition = observer(
	(p: { subject: SchemaRegistrySubjectDetails }) => {
								...
								<SingleSelect
									...
									options={subject.versions.map((v) => ({
										...
										label:
											...
											(subject.versions[subject.versions.length - 1] == v
												? " (latest)"
												: ""),
									}))}
								...
								<Button
									...
									onClick={() =>
										openPermanentDeleteModal(
											`${subject.name} version ${schema.version}`,
											() => {
												api
													.deleteSchemaSubjectVersion(
														subject.name,
														schema.version,
														true,
													)
													.then(async () => {
														...
														const newDetails = api.schemaDetails.get(
															subject.name,
														);
														if (
															!newDetails ||
															!newDetails.latestActiveVersion
														) {
															appGlobal.history.push("/schema-registry/");
														} else {
															setSelectedVersion(
													...
													.catch((err) => {
														toast({
															...
															isClosable: true,
															title:
																"Failed to permanently delete schema version",
															description: String(err),
														});
													});
											},
									...
									onClick={() => {
										api
											...
											.then(async (r) => {
												...
												const updatedDetails = api.schemaDetails.get(
													subject.name,
												);
												if (updatedDetails)
													appGlobal.history.push(
												...
												api
													...
													.then(() => {
														toast({
															...
															description:
																"You can recover or permanently delete it.",
														});
					...
					{schema.isSoftDeleted && (
						<Alert status="warning" variant="left-accent">
							...
							<Box>
								...
								<AlertDescription>
									This schema has been soft-deleted. It is still required by
									other schemas. It remains readable.
								</AlertDescription>
							</Box>
						</Alert>
					)}
...
const VersionDiff = observer((p: { subject: SchemaRegistrySubjectDetails }) => {
	...
	const defaultVersionRight =
		subject.latestActiveVersion == -1
			? defaultVersionLeft
			: subject.latestActiveVersion;

	const [selectedVersionLeft, setSelectedVersionLeft] =
		useState(defaultVersionLeft);
	const [selectedVersionRight, setSelectedVersionRight] =
		useState(defaultVersionRight);

	const schemaLeft = subject.schemas.first(
		(x) => x.version == selectedVersionLeft,
	);
	const schemaRight = subject.schemas.first(
		(x) => x.version == selectedVersionRight,
	);
...
const SchemaReferences = observer(
	(p: {
		subject: SchemaRegistrySubjectDetails;
		schema: SchemaRegistryVersionedSchema;
	}) => {
		...
		const referencedBy =
			referencedByVersions && referencedByVersions.get(version);
