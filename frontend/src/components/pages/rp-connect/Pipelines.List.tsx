...
const EmptyPlaceholder = () => {
	...
	return enableConnectTiles ? (
		<ConnectTiles onChange={handleConnectionChange} hideHeader />
	) : (
		oldUI
	);
};
...
export const PipelineStatus = observer((p: { status: Pipeline_State }) => {
	switch (p.status) {
		...
		default:
			return (
				<Flex alignItems="center" gap="2">
					<MdOutlineQuestionMark fontSize="17px" width="auto" color="red" />{" "}
					Unknown
				</Flex>
			);
	}
...
export const CreateConnectionMenuButton = () => {
	const [isMenuOpen, setIsMenuOpen] = useState(false);
	const [_, setPersistedConnectionName] = useSessionStorage<
		Partial<ConnectTilesFormData>
	>(CONNECT_TILE_STORAGE_KEY, {});
	const navigate = useNavigate();
						...
						columns={[
							{
								...
								cell: ({ row: { original } }) => (
									<Link
										to={`/rp-connect/${encodeURIComponentPercents(original.id)}`}
									>
