...
const PipelineListPageContent = () => {
	...
	const { mutate: deleteMutation, isPending: isDeletingPipeline } =
		useDeletePipelineMutation();
	...
	const pipelines = useMemo(
		() =>
			(pipelinesData?.pipelines || [])
				.filter(
					(pipeline): pipeline is APIPipeline =>
						!!pipeline &&
						pipeline.tags?.__redpanda_cloud_pipeline_type !== "agent",
				)
	...
	const columns = useMemo<ColumnDef<Pipeline>[]>(
		() => [
			...
			},
			{
				...
				header: "Description",
				cell: ({ row }) => (
					<Text className="max-w-md truncate">
			...
			},
			{
				...
				header: "State",
				cell: ({ row }) => (
					<PipelineStatusBadge state={row.getValue("state")} />
				),
				size: 120,
			},
			{
...
const RedpandaConnectContent = () => (
	<div className="flex flex-col gap-6">
		<Text>
			...
			high-performance data pipelines that drive real-time analytics and
			actionable business insights. Integrate data across systems with hundreds
			of prebuilt connectors, change data capture (CDC) capabilities, and
...
export const PipelineListPage = () => {
	...
	const { data: kafkaConnectors, isLoading: isLoadingKafkaConnect } =
		useKafkaConnectConnectorsQuery();
			...
			<div className="flex flex-col gap-4">
				{showKafkaConnectLoadingHint ? (
					<div className="flex min-h-10 items-center gap-2 text-muted-foreground text-sm">
						<Spinner />
						<Text variant="muted">
							Checking for Kafka Connect availability...
						</Text>
					</div>
				) : (
					<div className="h-10" />
