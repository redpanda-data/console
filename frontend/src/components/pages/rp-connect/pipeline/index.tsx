...
import {
	Skeleton,
	SkeletonGroup,
} from "components/redpanda-ui/components/skeleton";
...
const Footer = memo(
	({ mode, onSave, onCancel, isSaving, disabled }: FooterProps) => {
		if (mode === "view") {
			return (
...
export default function PipelinePage() {
	...
	const persistedYamlContent = useOnboardingYamlContentStore(
		(state) => state.yamlContent,
	);
	const setPersistedYamlContent = useOnboardingYamlContentStore(
		(state) => state.setYamlContent,
	);
	const [editorInstance, setEditorInstance] =
		useState<null | editor.IStandaloneCodeEditor>(null);
	...
	const pipeline = useMemo(
		() => pipelineResponse?.response?.pipeline,
		[pipelineResponse],
	);

	const { data: componentListResponse, isLoading: isComponentListLoading } =
		useListComponentsQuery();
	...
	const yamlEditorSchema = useMemo(() => {
		...
		try {
			...
			return {
				definitions: parsed.definitions as
	...
	const { data: lintResponse, isPending: isLinting } =
		useLintPipelineConfigQuery(debouncedYamlContent, {
			enabled: mode !== "view",
		});
	...
	useEffect(() => {
		...
		if (inputData?.connectionName && inputData?.connectionType) {
			...
			if (generatedYaml) {
				useOnboardingYamlContentStore
					.getState()
					.setYamlContent({ yamlContent: generatedYaml });
			}
		}
	...
	const handleSave = useCallback(async () => {
		...
		if (mode === "create") {
			...
			let serviceAccountConfig:
				| ReturnType<typeof create<typeof Pipeline_ServiceAccountSchema>>
				| undefined;
			if (
				userData.authMethod === "service-account" &&
				userData.serviceAccountId &&
				userData.serviceAccountSecretName
			) {
			...
			const createRequestDataPlane = create(
				CreatePipelineRequestSchemaDataPlane,
				{
			...
			createMutation(createRequest, {
				onSuccess: (response) => {
					...
					const retUnits = cpuToTasks(
						response.response?.pipeline?.resources?.cpuShares,
					);
					const currentUnits = form.getValues("computeUnits");
					if (retUnits && currentUnits !== retUnits) {
						toast.warning(
							`Pipeline has been resized to use ${retUnits} compute units`,
						);
					}
			...
			const updateRequestDataPlane = create(
				UpdatePipelineRequestSchemaDataPlane,
				{
					id: pipelineId,
	...
	return (
		<div>
			...
			<div
				className={cn(
					(mode === "create" || mode === "edit") &&
						"grid grid-cols-[minmax(auto,950px)_260px] gap-4",
				)}
			>
