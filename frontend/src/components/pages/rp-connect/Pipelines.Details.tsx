...
class RpConnectPipelinesDetails extends PageComponent<{ pipelineId: string }> {
	...
	render() {
		...
		const isTransitioningState =
			...
			pipeline.state == Pipeline_State.STOPPING;
				...
				<Box my="4">
					{QuickTable(
						[
							...
							{
								key: "Status",
								value: <PipelineStatus status={pipeline.state} />,
							},
							{
								key: "Resources",
								value: <PipelineResources resources={pipeline.resources} />,
							},
						],
						...
						onClick={() => {
							...
							const watchPipelineUpdates = async () => {
								...
								while (true) {
									...
									if (
										p.state != Pipeline_State.STARTING &&
										p.state != Pipeline_State.STOPPING
									)
										return;
								}
							};
...
async function executeMessageSearch(
	...
	return runInAction(async () => {
		try {
			...
			console.error(
				"error in pipelineLogsMessageSearch: " +
...
export const PipelineResources = observer(
	(p: { resources?: Pipeline_Resources }) => {
		const r = p.resources;
