...
class RpConnectPipelinesCreate extends PageComponent<{}> {
	...
	render() {
		...
		const alreadyExists = pipelinesApi.pipelines.any(
			(x) => x.id == this.fileName,
		);
			...
			<PageContent>
				...
				<Flex flexDirection="column">
					<FormField
						...
						errorText="Pipeline name is already in use"
					>
							...
							<Box>
								...
								{isKafkaConnectPipeline(p.yaml) && (
									<Alert status="error" my={2}>
										<AlertIcon />
										<Text>
											This looks like a Kafka Connect configuration. For help
											with Redpanda Connect configurations,{" "}
											<ChLink
												target="_blank"
												href="https://docs.redpanda.com/redpanda-cloud/develop/connect/connect-quickstart/"
											>
