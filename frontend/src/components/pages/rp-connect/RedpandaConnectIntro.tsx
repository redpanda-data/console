...
`.trim();
	...
	const [selectedInstall, setSelectedInstall] = useState(
		"macOS - Homebrew" as keyof typeof installInstructions,
	);
	const options = Object.keys(installInstructions).map((x) => ({
		value: x as typeof selectedInstall,
	}));
	...
	return (
		<Grid templateColumns="1fr 320px" gap={10}>
			<Stack spacing={3}>
				...
				<Text>
					Redpanda Connect is a declarative data streaming service with wide
					range of{" "}
					<ChakraLink
						isExternal
				...
				<OrderedList display="flex" flexDirection="column" gap="0.5rem">
						...
						<Box>
							<Stack gap={2} mb={6}>
								...
								<Box>
									<CodeBlock
										language="bash"
										codeString={installInstructions[selectedInstall]}
									/>
								</Box>
							</Stack>
						</Box>
					...
					<ListItem>
						<Text mt={3}>
							Build your first pipeline. Start from the Redpanda data generator
							example below. Explore the components using autocomplete
							(CTRL/CMD+Space). For other examples and use cases, see{" "}
							<ChakraLink
								isExternal
					...
					<ListItem>
						<Text mt={3}>
							Make sure that your output topic and logs topic both exist.
						</Text>
					</ListItem>

					<ListItem>
						<Text mt={3}>
							To test the above config in the Terminal, first save your
							configuration from the editor to your working directory as{" "}
					...
					<ListItem>
						...
						<Box maxWidth="400px" marginBlock={3}>
							<CodeBlock
								language="sh"
								codeString="rpk connect run ./config.yaml"
							/>
							<Text mt={3}>
								Anything you write to stdin will be written unchanged to stdout.
							</Text>
						</Box>
					</ListItem>
			...
			<Stack spacing={8} mt={12}>
				...
				<Alert status="info">
					...
					<Box>
						...
						<AlertDescription>
							In the Terminal, to show the full menu of components available,
							use <code>rpk connect list</code>.
							<br />
							<br />
