...
import {
	...
	Link,
	Text,
} from "components/redpanda-ui/components/typography";
...
import {
	type ConnectTilesListFormData,
	connectTilesListFormSchema,
} from "../types/wizard";
...
const searchComponents = (
	...
	const matchesFilters = (component: ConnectComponentSpec) => {
		...
		if (filters?.categories?.length) {
			const hasMatchingCategory = component.categories?.some((cat: string) =>
	...
	if (additionalComponents?.length) {
		const filteredAdditional = additionalComponents.filter((comp) =>
			matchesFilters(comp),
		);
	...
	const additionalNames = new Set(
		additionalComponents?.map((c) => c.name) || [],
	);
...
const ConnectTilesSkeleton = memo(
	({
		...
		const skeletonTiles = Array.from({ length: tileCount }, () => (
			<div
				className="h-[78px] rounded-lg border bg-card p-4"
...
export const ConnectTiles = memo(
	forwardRef<
		BaseStepRef<ConnectTilesListFormData>,
		ConnectTilesProps & MotionProps
	>(
		(
			{
				components,
			...
			const builtInComponents = useMemo(
				() => (components ? parseSchema(components) : []),
				[components],
			);
					...
					{!hideHeader && (
						<CardHeader className="bg-background">
							...
							<CardDescription className="mt-4">
								{description ?? (
									<Text>
										Redpanda Connect is a data streaming service for building
										scalable, high-performance data pipelines that drive
										real-time analytics and actionable business insights.
										Integrate data across systems with hundreds of prebuilt
										connectors, change data capture (CDC) capabilities, and
					...
					<CardContent className="mt-2" id="rp-connect-onboarding-wizard">
						<Form {...form}>
							...
							<div className="relative">
								<div
									className={cn(
										"max-h-[50vh] min-h-[400px] overflow-y-auto py-4",
										tileWrapperClassName,
									)}
														...
														<ConnectTile
															...
															onChange={() => {
																if (isChecked) {
																	...
																	form.setValue(
																		"connectionType",
																		component.type as ConnectComponentType,
																		{
																			shouldValidate: true,
																		},
																	);
											...
											if (hasNoResults) {
												...
												content = (
													<ConnectTilesSkeleton
														gridCols={gridCols}
														tileCount={PRIORITY_COMPONENTS.length}
													>
														{renderTiles()}
													</ConnectTilesSkeleton>
												);
