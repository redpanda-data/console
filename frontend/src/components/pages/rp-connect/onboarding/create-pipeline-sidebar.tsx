...
export const CreatePipelineSidebar = memo(
	({
		...
		const {
			...
			onOpen: openAddConnector,
			onClose: closeAddConnector,
		} = useDisclosure();
		const [selectedConnector, setSelectedConnector] = useState<
			ConnectComponentType | undefined
		>(undefined);
		const components = useMemo(
			() => (rawComponentList ? parseSchema(rawComponentList) : []),
			[rawComponentList],
		);

		const hasInput = useMemo(
			() => editorContent.includes("input:"),
			[editorContent],
		);
		const hasOutput = useMemo(
			() => editorContent.includes("output:"),
			[editorContent],
		);
