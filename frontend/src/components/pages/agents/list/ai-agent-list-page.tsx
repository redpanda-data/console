...
import {
	...
	type AIAgentDeleteHandlerRef,
} from "./ai-agent-delete-handler";
...
const StatusIcon = ({ state }: { state: AIAgent_State }) => {
	...
	return (
		<div className="flex items-center gap-2">
			<IconComponent
				className={`h-4 w-4 ${statusProps.iconColor} ${statusProps.animate ? "animate-spin" : ""}`}
			/>
...
export const createColumns = (
	...
	const { mcpServersMap, handleDeleteWithServiceAccount, isDeletingAgent } =
		options;
	...
	return [
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				const mcpServerIds = Object.values(agent.mcpServers || {}).map(
					(server) => server.id,
				);
...
const AIAgentsListPageContent = ({
	...
	const { mutateAsync: deleteAIAgent, isPending: isDeletingAgent } =
		useDeleteAIAgentMutation();
		...
		async (
			...
			try {
				...
				if (deleteServiceAccountFlag && serviceAccountId && secretName) {
					...
					await deleteHandlerRef.current?.deleteServiceAccount(
						serviceAccountId,
					);
	...
	const aiAgents = React.useMemo(
		() => aiAgentsData?.aiAgents?.map(transformAPIAIAgent) || [],
		[aiAgentsData],
	);
