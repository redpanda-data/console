...
// Pre-load modules BEFORE setting up mocks to avoid interception loops
...
mock.module('config', () => ({
  ...
  addBearerTokenInterceptor: mock(
    (next: (request: unknown) => Promise<unknown>) => async (request: unknown) => await next(request)
  ),
}));
...
// Now import test utilities and component
...
describe('AIAgentsListPage', () => {
  ...
  test('should display loading state when fetching agents', async () => {
    ...
    // Component should transition from loading to empty state
    await waitFor(() => {
      expect(screen.getByText('No AI agents found.')).toBeVisible();
    });

    // Verify loading state is not present after data loads
