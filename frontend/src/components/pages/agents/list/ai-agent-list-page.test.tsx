...
const PAGINATION_AGENTS_FIXTURE = Array.from({ length: 25 }, (_, index) =>
...
describe('AIAgentsListPage', () => {
  ...
  test.each([
    {
      label: 'should stop a running AI agent from the list',
      initialState: AIAgent_State.RUNNING,
      initialStateLabel: 'Running',
      menuItemLabel: 'Stop Agent',
      action: 'stop' as const,
    },
    {
      label: 'should start a stopped AI agent from the list',
      initialState: AIAgent_State.STOPPED,
      initialStateLabel: 'Stopped',
      menuItemLabel: 'Start Agent',
      action: 'start' as const,
    },
  ])('$label', async ({ initialState, initialStateLabel, menuItemLabel, action }) => {
    const user = userEvent.setup();
    ...
    expect(await screen.findByText('Test Agent 1')).toBeVisible();
    ...
    const menuItem = await screen.findByText(menuItemLabel);
  ...
  test('should update pagination footer and disable next button on the last page', async () => {
    ...
    const listAIAgentsResponse = create(ListAIAgentsResponseSchema, {
      aiAgents: PAGINATION_AGENTS_FIXTURE,
      nextPageToken: '',
    });
