...
// Form schema factory - matching proto validation rules
export const createFormSchema = (isGatewayMode: boolean) =>
  z
    .object({
      displayName: z
        .string()
      ...
      provider: isGatewayMode
        ? z.enum(['openai', 'anthropic']).optional()
        : z.enum(['openai', 'anthropic', 'google', 'openaiCompatible']).default('openai'),
      apiKeySecret: isGatewayMode ? z.string().optional() : z.string().min(1, 'API Token is required'),
    ...
    .superRefine((data, ctx) => {
      if (!isGatewayMode && data.provider === 'openaiCompatible') {
...
export const FormSchema = createFormSchema(false);
