...
export class EditOffsetsModal extends Component<{
	...
	render() {
		...
		this.offsetsByTopic = offsets
			.groupInto((x) => x.topicName)
			.map((g) => ({ topicName: g.key, items: g.items }));
							...
							<HStack spacing={6}>
								...
								<div>
									...
									>
										...
										<div>
											{this.offsetsByTopic.length} {single ? "Topic" : "Topics"}{" "}
											/ {offsets.length}{" "}
			...
			<Flex flexDirection="column" gap={4} mt={16}>
				...
				<Text>
					{
						(
							{
								startOffset:
									"Set all offsets to the oldest partition's offset.",
								...
								otherGroup:
									"Copy offsets from another (inactive) consumer group",
				...
				{this.selectedOption === "otherGroup" && (
					<Box mt={2}>
						...
						>
							<SingleSelect
								options={this.otherConsumerGroups.map((g) => ({
									value: g.groupId,
									label: g.groupId,
								}))}
							...
							<DataTable<GroupOffset>
								...
								columns={[
									...
									},
									{
										...
										cell: ({ row: { original } }) => (
											<ColAfter
												selectedTime={this.timestampUtcMs}
												record={original}
											/>
										),
									},
	...
	setPage(page: 0 | 1) {
		if (page == 1) {
			...
			if (op == "startOffset") {
				...
				this.props.offsets.forEach(
					(x) => (x.newOffset = "fetching offsets..." as any),
				);
				const requiredTopics = this.props.offsets
					.map((x) => x.topicName)
					.distinct();
				...
				setTimeout(async () => {
					...
					try {
						offsetsForTimestamp = await api.getTopicOffsetsByTimestamp(
							requiredTopics,
							this.timestampUtcMs,
						);
						...
						showErrorModal(
							...
							<span>
								...
								<span className="codeBox">
									{new Date(this.timestampUtcMs).toUTCString()}
								</span>
								.
							</span>,
				...
				if (other) {
					...
					const getOffset = function (
						...
						return other.topicOffsets
							...
							?.partitionOffsets.first((p) => p.partitionId == partitionId)
							?.groupOffset;
					};
					...
					const alreadyExists = function (
						...
						return currentOffsets.any(
							(x) => x.topicName == topicName && x.partitionId == partitionId,
						);
					};
					...
					showErrorModal(
						...
						null,
						<span>
							Could not find a consumer group named{" "}
							<span className="codeBox">{this.selectedGroup}</span> to compute
							new offsets.
						</span>,
					);
				}
	...
	footer() {
		const disableContinue =
			this.selectedOption == "otherGroup" && !this.selectedGroup;
	...
	updateVisible(visible: boolean) {
		...
		if (visible) {
			setTimeout(() => {
				...
				const topics = this.props.group.topicOffsets
					.map((x) => x.topic)
					.distinct();
	...
	async onApplyEdit() {
		...
		try {
			const editResponse = await api.editConsumerGroupOffsets(
				group.groupId,
				topics,
			);
			...
			if (errors.length > 0) {
				console.error("apply offsets, backend errors", {
					errors: errors,
					request: topics,
				});
			...
			showErrorModal(
				...
				<span>
					Could not apply offsets for consumer group{" "}
					<span className="codeBox">{group.groupId}</span>.
				</span>,
				toJson(err, 4),
			);
		} finally {
...
class ColAfter extends Component<{
	...
	render() {
		...
		if (val == null) {
			return (
				<Tooltip
					label="Offset will not be changed"
					openDelay={1}
					placement="top"
					hasArrow
				>
		...
		if (typeof val == "object") {
			...
			if ("offset" in val) {
				...
				// use 'latest'
				const partition = api.topicPartitions
					.get(record.topicName)
					?.first((p) => p.id == record.partitionId);
				return (
					<div
					...
					>
						<InfoText
							tooltip={
								<div>
									There is no offset for this partition at or after the given
									timestamp (
									<code>
										{new Date(this.props.selectedTime ?? 0).toLocaleString()}
									</code>
									). As a fallback, the last offset in that partition will be
									used.
								</div>
							}
				...
				>
					<span>
						{typeof content.offset == "number"
							? numberToThousandsString(content.offset)
							: content.offset}
					</span>
					<span
						style={{
					...
					<ModalBody>
						<Flex flexDirection="row" gap={6}>
							...
							<Box>
								<Box>
									<Text>
										Are you sure you want to delete offsets from this consumer
										group?
									</Text>
								...
								<Box>
									{mode === "group" && (
										<Box>
											<Text>
												Group offsets will be deleted for all topics and
												partitions:
											</Text>
										...
										<Box>
											...
											<UnorderedList fontWeight="600" my={2}>
												...
												<ListItem>
													Partition:{" "}
													<span className="codeBox">
														{offsetsByTopic[0].items[0].partitionId}
													</span>
												</ListItem>
											</UnorderedList>
										</Box>
	...
	async onDeleteOffsets() {
		...
		try {
			if (this.props.mode === "group") {
				...
				const deleteResponse = await api.deleteConsumerGroupOffsets(
					group.groupId,
					deleteRequest,
				);
			...
			showErrorModal(
				"Delete offsets",
				<span>
					Could not delete selected offsets in consumer group{" "}
					<span className="codeBox">{group.groupId}</span>.
				</span>,
				toJson(err, 4),
			);
			...
			const remainingOffsets =
				group.topicOffsets.sum((t) => t.partitionOffsets.length) -
				offsets.length;
