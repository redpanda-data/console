	...
	render() {
		...
		try {
			const quickSearchRegExp = new RegExp(
				uiSettings.consumerGroupList.quickSearch,
				"i",
			);
						...
						<DataTable<GroupDescription>
							...
							columns={[
								...
								},
								{
									...
									cell: ({ row: { original } }) => (
										<Link
											to={`/groups/${encodeURIComponent(original.groupId)}`}
										>
								...
								},
								{
									...
									size: 1,
									cell: ({ row: { original } }) => (
										<BrokerList brokerIds={[original.coordinatorId]} />
									),
								},
								{
									header: "Protocol",
								...
								},
								{
									...
									accessorKey: "lagSum",
									cell: ({ row: { original } }) =>
										ShortNum({ value: original.lagSum }),
								},
							]}
						/>
					</Section>
