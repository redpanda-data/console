...
class GroupDetails extends PageComponent<{ groupId: string }> {
	...
	render() {
		...
		const totalPartitions = group.members
			.flatMap((m) => m.assignments)
			.sum((a) => a.partitionIds.length);
...
class GroupByTopics extends Component<{
	...
	render() {
		...
		const topicEntries = lagGroupsByTopic.map((g) => {
			...
			const partitionsAssigned = g.partitions.filter(
				(c) => c.assignedMember,
			).length;
					...
					<Flex justifyContent="space-between">
						<Flex alignItems="center" gap={1}>
							...
							<Flex ml={2} gap={1}>
								...
								<Tooltip
									placement="top"
									label="Summed lag of all partitions of the topic"
									hasArrow
								>
								...
								<Tooltip
									placement="top"
									label="Number of assigned partitions"
									hasArrow
								>
									<Tag colorScheme="brand" variant="solid">
						...
						<Button
							...
							onClick={() =>
								appGlobal.history.push(
									`/topics/${encodeURIComponent(g.topicName)}`,
								)
							}
						>
							View Topic
					...
					<DataTable<{
						...
						columns={[
							...
							},
							{
								...
								cell: ({ row: { original } }) =>
									numberToThousandsString(original.highWaterMark),
							},
							{
								size: 120,
								header: "Group Offset",
								...
								cell: ({ row: { original } }) =>
									numberToThousandsString(original.groupOffset),
							},
							{
								size: 80,
								header: "Lag",
								accessorKey: "lag",
								cell: ({ row: { original } }) =>
									ShortNum({ value: original.lag, tooltip: true }),
							},
							{
								size: 1,
								header: "",
				...
				<Empty
					description={
						p.onlyShowPartitionsWithLag ? (
							<span>
								All {topicEntries.length} topics have been filtered (no lag on
								any partition).
							</span>
						) : (
							"No data found"
		...
		return (
			<Accordion
				items={topicEntries.filterNull()}
				defaultIndex={defaultExpand}
			/>
		);
	}
}
...
@observer
class GroupByMembers extends Component<{
	group: GroupDescription;
	onlyShowPartitionsWithLag: boolean;
}> {
	render() {
				...
				const assignmentsFlat = assignments
					.map((a) =>
						a.partitionIds.map((id) => {
							...
							const partLag = topicLag?.partitionOffsets.find(
								(p) => p.partitionId == id,
							)?.lag;
							return {
				...
				return {
					heading: (
						<Flex alignItems="baseline" gap="1">
							...
							<Tooltip
								placement="top"
								label="Summed lag over all assigned partitions of all topics"
								hasArrow
							>
								<Tag variant="solid" colorScheme="brand">
									lag: {totalLag}
			...
			return (
				<Empty
					description={
						p.onlyShowPartitionsWithLag ? (
							<span>
								All {memberEntries.length} members have been filtered (no lag on
								any partition).
							</span>
						) : (
							"No data found"
		...
		return (
			<Accordion
				items={memberEntries.filterNull()}
...
const renderMergedID = (id?: string, clientId?: string) => {
	...
	// A client might be connected but it hasn't any assignments yet because it just joined the group
		...
		return (
			<span className="consumerGroupCompleteID">{clientId ?? id ?? ""}</span>
		);
	}
...
const stateIcons = new Map<StateIcon, JSX.Element>([
	...
	[
		"completingrebalance",
		<MdHourglassBottom key="completingrebalance" size={16} color="#52c41a" />,
	],
	[
		"preparingrebalance",
		<MdHourglassEmpty key="preparingrebalance" size={16} color="orange" />,
	],
...
const stateIconDescriptions: Record<StateIcon, string> = {
	...
	preparingrebalance:
		"A reassignment of partitions is required, members have been asked to stop consuming",
...
export const GroupState = (p: { group: GroupDescription }) => {
	...
	return (
		<Popover
			...
			placement="right"
			hideCloseButton
			content={consumerGroupStateTable}
		>
...
function cannotEditGroupReason(group: GroupDescription): string | undefined {
	...
	if (!Features.patchGroup)
		return "This cluster does not support editting group offsets";
}
