...
const MTLS_CLIENT_KEY_SECRET_TEXT: SecretSelectorCustomText = {
	dialogDescription:
		"Create a new secret for your mTLS client private key. The secret will be stored securely.",
	...
	emptyStateDescription:
		"Create a secret to securely store your mTLS client private key",
};
	...
	<div className="flex flex-col gap-4">
		<div className="flex flex-col gap-4">
			<FormField
				...
				name="mtls.ca"
				render={() => (
					<FormItem className="w-1/2">{renderCertificateButton("ca")}</FormItem>
				)}
			/>
			...
			<FormField
				...
				render={() => (
					<FormItem className="w-1/2">
						{renderCertificateButton("clientCert")}
					</FormItem>
				)}
			/>
			...
			{!hideClientKey && (
				<FormField
					...
					render={() => (
						<FormItem className="w-1/2">
							{renderCertificateButton("clientKey")}
						</FormItem>
					)}
				/>
			)}
		...
		{Boolean(
			...
			<div
				...
				data-testid="mtls-certificates-errors"
			>
				{Boolean(errors.mtls?.ca?.message) && (
					<Text className="text-destructive text-sm">
						{String(errors.mtls?.ca?.message)}
					</Text>
				)}
				{Boolean(errors.mtls?.clientCert?.message) && (
					<Text className="text-destructive text-sm">
						{String(errors.mtls?.clientCert?.message)}
					</Text>
				)}
				{Boolean(errors.mtls?.clientKey?.message) && (
					<Text className="text-destructive text-sm">
						{String(errors.mtls?.clientKey?.message)}
					</Text>
				)}
			</div>
		)}
...
export const MtlsConfiguration = () => {
	...
	const availableSecrets = useMemo(() => {
		...
		return secretsData.secrets
			.filter(
				(secret): secret is NonNullable<typeof secret> & { id: string } =>
					!!secret?.id,
			)
			.map((secret) => ({
				id: secret.id,
	...
	const handleSaveCertificate = (value: {
		filePath?: string;
		pemContent?: string;
		fileName?: string;
	}) => {
	...
	const renderCertificateButton = (certType: CertificateType) => {
		...
		const displayValue =
			cert?.fileName || cert?.filePath || "Certificate added";
	...
	return (
		<>
			<div className="flex flex-col gap-4" data-testid="mtls-certificates-form">
				<Text variant="muted">
					Configure certificates for mutual TLS authentication. Upload embeds
					certificate content in the configuration, while file path references
					certificates already on the broker. Providing certificates enables
					mTLS; leaving them empty uses server-side TLS only.
				</Text>
							...
							<FormItem className="flex flex-col">
								...
								<FormControl>
									...
									>
										<TabsList>
											<TabsTrigger
												data-testid="mtls-mode-upload-tab"
												value="pem"
											>
												Upload
											</TabsTrigger>
											<TabsTrigger
												data-testid="mtls-mode-file-path-tab"
												value="file_path"
											>
												File path
											</TabsTrigger>
