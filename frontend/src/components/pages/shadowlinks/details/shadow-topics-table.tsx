...
const VirtualizedRows = ({
	...
	rows: ReturnType<
		ReturnType<typeof useReactTable<ShadowTopic>>["getRowModel"]
	>["rows"];
	isFetchingNextPage?: boolean;
	isFetching?: boolean;
	columnsLength: number;
	...
	if (isFetching) {
		return (
			<LoadingRow columnsLength={columnsLength} message="Loading topics..." />
		);
	}
				...
				header: () => (
					<div className="flex items-center gap-2">
						...
						<Tooltip>
							...
							<TooltipContent>
								<p>
									Maximum offset difference between source and replica topic
									partitions
								</p>
							</TooltipContent>
						</Tooltip>
				...
				cell: ({ row }) => {
					...
					return (
						<div className="flex items-center justify-end gap-2">
							<Button
								onClick={() =>
									navigate({ to: `/topics/${topic.topicName}?pageSize=10` })
								}
								size="sm"
								type="button"
								variant="outline"
							...
							{topic.state === ShadowTopicState.ACTIVE && (
								<Button
									onClick={() => onFailoverTopic(topic.topicName)}
									size="sm"
									type="button"
									variant="outline"
								>
