...
import {
	ShadowLinkLoadErrorState,
	ShadowLinkNotFoundState,
} from "../list/shadowlink-empty-state";
...
export const ShadowLinkDetailsPage = () => {
	...
	const { deleteShadowLink, isPending: isDeleting } =
		useDeleteShadowLinkUnified({ name: name ?? "" });

	const { mutate: failoverShadowLink, isPending: isFailingOver } =
		useFailoverShadowLinkMutation({
			onSuccess: async () => {
				toast.success(
					failoverTopicName
	...
	if (errorGetShadowLink?.code === Code.NotFound) {
		return (
			<ShadowLinkNotFoundState
				name={name ?? ""}
				onBackClick={() => navigate({ to: "/shadowlinks" })}
			/>
		);
	}
	...
	if (errorGetShadowLink) {
		return (
			<ShadowLinkLoadErrorState errorMessage={errorGetShadowLink.message} />
		);
	}
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="flex justify-end gap-3">
				...
				<Button
					onClick={() => navigate({ to: `/shadowlinks/${name}/edit` })}
					size="sm"
					variant="outline"
				>
			...
			{Boolean(hasPartialData) && (
				<Alert testId="partial-data-warning" variant="warning">
					...
					<AlertDescription>
						Some shadow link details could not be loaded. Task status and topic
			...
			<Tabs defaultValue="overview">
				...
				<TabsContents>
					...
					<TabsContent testId="tasks-content" value="tasks">
						<TasksTable
							dataUnavailable={hasPartialData}
							onRefresh={refetch}
							tasks={shadowLink.tasksStatus}
						/>
					</TabsContent>
