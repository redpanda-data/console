...
export const createColumns: ColumnDef<ListShadowLinksResponse_ShadowLink>[] = [
	...
	},
	{
		...
		header: "Status",
		cell: ({ row }) => (
			<Text> {getShadowLinkStateLabel(row.original?.state)} </Text>
		),
	},
];
...
export const ShadowLinkListPage = () => {
	...
	const shadowLinks = React.useMemo(
		() => shadowLinksData?.shadowLinks || [],
		[shadowLinksData],
	);
	...
	if (
		...
		error.message.includes("Cluster link feature is disabled")
	) {
		return (
	...
	if (!isLoading && shadowLinks.length === 0) {
		...
		return (
			<div className="my-2 flex justify-center gap-2">
				<ShadowLinkEmptyState
					onCreateClick={() => navigate({ to: "/shadowlinks/create" })}
				/>
			</div>
		);
	}
	...
	return (
		<div className="flex flex-col gap-4">
			<div className="flex items-center justify-between">
				<Text variant="muted">
					Manage shadow links to replicate topics from source clusters for
