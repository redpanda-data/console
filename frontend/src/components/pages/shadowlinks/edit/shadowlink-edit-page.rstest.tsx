...
// Mock the hooks - use full paths for RSTest compatibility
rs.mock('./src/react-query/api/shadowlink', () => ({
  useEditShadowLink: rs.fn(),
}));

// Mock ui-state
rs.mock('./src/state/ui-state', () => ({
  uiState: {
    pageTitle: '',
    pageBreadcrumbs: [],
  },
...
// Mock config - RSTest doesn't support async in rs.mock()
// We use a simple mock that returns false for all feature flags
rs.mock('./src/config', () => ({
  config: {
    jwt: 'test-jwt-token',
  },
...
rs.mock('@tanstack/react-router', () => {
  // Re-export commonly used items from the module
  return {
    useParams: () => ({ name: SHADOW_LINK_NAME }),
    useNavigate: () => rs.fn(),
    useRouter: () => ({ navigate: rs.fn() }),
    ...
    RouterContextProvider: ({ children }: any) => children,
    Outlet: () => null,
  };
});
...
describe('ShadowLinkEditPage', () => {
  ...
  beforeEach(() => {
    rs.clearAllMocks();
    mockUpdateShadowLink.mockImplementation((_request: any) => Promise.resolve({}));
  });
