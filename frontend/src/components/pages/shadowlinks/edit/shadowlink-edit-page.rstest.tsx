...
// Mock the hooks
rstest.mock('react-query/api/shadowlink', () => ({
  useEditShadowLink: rstest.fn(),
}));
...
// Note: config is already mocked globally in rstest.setup.ts
...
type TestCase = {
  ...
  verify: (updateRequest: any, exp: typeof expect) => void;
};
...
describe('ShadowLinkEditPage', () => {
  const mockUpdateShadowLink = rstest.fn();

  beforeEach(() => {
    rstest.clearAllMocks();
    mockUpdateShadowLink.mockImplementation((_request: unknown) => Promise.resolve({}));
  });
  ...
  test.each(testCases)('$description', async ({ actions, expectedFieldMaskPaths, verify }) => {
    ...
    rstest.mocked(useEditShadowLink).mockReturnValue({
      formValues,
      isLoading: false,
      error: null,
      isUpdating: false,
