...
const fallbackConnector: ConnectorMetadata = {
	logo: (
		<img src={RedpandaLogo} alt="Redpanda logo" className="connectorLogo" />
	),
};
...
const connectorMetadata: ConnectorMetadata[] = [
	// Apache Connectors
	{
		...
		description:
			"Imports messages from another Kafka cluster, using MirrorSourceConnector",
		learnMoreLink:
	...
	} as const,
	{
		...
		description:
			"Imports consumer group offsets from another Kafka cluster, using MirrorCheckpointConnector",
		learnMoreLink:
	...
	} as const,
	{
		...
		logo: (
			<img src={ElasticLogo} alt="Elastic logo" className="connectorLogo" />
		),
		author: "Confluent",
	} as const,
	{
	...
	} as const,
	{
		...
		logo: (
			<img src={CassandraLogo} alt="Cassandra logo" className="connectorLogo" />
		),
		author: "Confluent",
	} as const,
	...
	// Debezium Connectors
	{
		...
		description:
			"Imports a stream of changes from MySQL, Amazon RDS and Amazon Aurora",
		learnMoreLink:
	...
	} as const,
	{
		...
		logo: (
			<img src={MongoDBLogo} alt="MongoDB logo" className="connectorLogo" />
		),
		author: "Debezium",
	} as const,
	{
	...
	} as const,
	{
		...
		friendlyName: "JDBC",
		description:
			"Imports batches of rows from MySQL, PostgreSQL, SQLite and SQL Server",
		learnMoreLink:
	...
	} as const,
	{
		...
		friendlyName: "JDBC",
		description:
			"Exports messages to tables in MySQL, PostgreSQL, SQLite and SQL Server",
		learnMoreLink:
	...
	// Snowflake Connectors
	{
		...
		logo: (
			<img src={SnowflakeLogo} alt="Snowflake logo" className="connectorLogo" />
		),
		author: "Snowflake",
	...
	// Iceberg Connectors
	{
		...
		logo: (
			<img src={IcebergLogo} alt="Iceberg logo" className="connectorLogo" />
		),
		author: "Tabular",
	...
	// Community Connector
	{
		...
		logo: (
			<img src={TwitterLogo} alt="Twitter logo" className="connectorLogo" />
		),
	...
	// Fallbacks with a very generous classname prefix (usually just the maintainers' logo)
	{
		...
		logo: (
			<img src={DebeziumLogo} alt="Debezium logo" className="connectorLogo" />
		),
		author: "Debezium",
	} as const,
	{
		...
		logo: (
			<img src={ConfluentLogo} alt="Confluent logo" className="connectorLogo" />
		),
		author: "Confluent",
	} as const,
	{
...
export function findConnectorMetadata(
	className: string,
...
export const ConnectorClass = observer(
	(props: { observable: { class: string } }) => {
...
export const OverviewStatisticsCard = observer(() => {
	...
	const totalConnectors =
		api.connectConnectors?.clusters?.sum((c) => c.totalConnectors) ?? "...";
...
export const ClusterStatisticsCard = observer((p: { clusterName: string }) => {
	...
	const cluster = api.connectConnectors?.clusters?.first(
		(x) => x.clusterName == p.clusterName,
	);
	...
	return (
		<Section py={4}>
			<div style={{ display: "flex", gap: "1em" }}>
				...
				<Statistic
					title="Connectors"
					value={`${runningConnectors} / ${totalConnectors}`}
				/>
...
export const ConnectorStatisticsCard = observer(
	(p: { clusterName: string; connectorName: string }) => {
		const cluster = api.connectConnectors?.clusters?.first(
			(x) => x.clusterName == p.clusterName,
		);
		const connector = cluster?.connectors.first(
			(x) => x.name == p.connectorName,
		);
		...
		return (
			<Section py={4}>
				<div style={{ display: "flex", gap: "1em" }}>
					...
					<Statistic
						title="Tasks"
						value={`${connector?.runningTasks} / ${connector?.totalTasks}`}
					/>
				</div>
			</Section>
		);
			...
			<Section>
				<VStack gap={4}>
					...
					<Text textAlign="center">
						...
						<br />
						Setup the connection details to your Kafka Connect cluster in your
						Redpanda Console config, to view and control all your connectors and
						tasks.
					</Text>
					<a
						target="_blank"
...
export const TasksColumn = observer(
	(props: { observable: ClusterConnectors | ClusterConnectorInfo }) => {
...
export const ConnectorsColumn = observer(
	(props: { observable: ConnectorInfo | ConnectorInfo[] }) => {
		let running = 0;
		let total = 0;
...
export const TaskState = observer(
	(p: {
		observable: {
			state: ClusterConnectorTaskInfo["state"];
			trace?: string;
			taskId?: number;
		};
	}) => {
					...
					<ModalContent minW="5xl">
						...
						<ModalBody>
							<Box
								...
								w="full"
								className="codeBox"
								style={{ whiteSpace: "pre", overflow: "scroll" }}
							>
								{err}
							</Box>
						</ModalBody>
