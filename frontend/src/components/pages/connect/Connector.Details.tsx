...
const KafkaConnectorMain = observer(
	({
		...
		const [connectClusterStore] = useState(
			ConnectClusterStore.getInstance(clusterName),
		);
		...
		return (
			<>
				...
				<Flex flexDirection="row" alignItems="center" gap="3">
					{/* [Pause/Resume] */}
					{connectClusterStore.validateConnectorState(connectorName, [
						"RUNNING",
						"PAUSED",
					]) ? (
						<Tooltip
				...
				<Tabs
					...
					items={[
											...
											<Button
												...
												isDisabled={(() => {
													...
													if (
														comparer.shallow(
															connector.config,
															connectorStore.getConfigObject(),
														)
													)
														return true;
												})()}
												onClick={() => {
													$state.updatingConnector = {
														clusterName,
														connectorName,
													};
												}}
						...
						},
						{
							key: "logs",
							name: "Logs",
							isDisabled: logsTopic
								? false
								: `Logs topic '${LOGS_TOPIC_NAME}' does not exist.`,
							component: (
								<Box mt="8">
									<LogsTab
										clusterName={clusterName}
					...
					content={(c) => (
						<>
							{connectClusterStore.validateConnectorState(connectorName, [
								...
								? "Pause"
								: "Resume"}{" "}
							connector <strong>{c.name}</strong>?
						</>
					)}
					successMessage={(c) => (
						<>
				...
				<ConfirmModal<RestartingTaskData>
					...
					content={(c) => (
						<>
							Restart task <strong>{c.taskId}</strong> of{" "}
							<strong>{c.connectorName}</strong>?
						</>
					)}
					successMessage={(c) => (
						<>
							Successfully restarted <strong>{c.taskId}</strong> of{" "}
							<strong>{c.connectorName}</strong>
						</>
					)}
					onOk={async (c) => {
					...
					<Section gridArea="health">
						<Flex flexDirection="row" gap="4" m="1">
							<Box
								width="5px"
								borderRadius="3px"
								background={statusColors[connector.status]}
							/>
					...
					<Section py={4} gridArea="tasks" minWidth="500px">
						<Flex alignItems="center" mt="2" mb="6" gap="2">
							...
							<Text opacity=".5" fontWeight="normal">
								(
								{connectClusterStore.getConnectorTasks(connectorName)?.length ||
									0}
								)
							</Text>
						</Flex>
						<DataTable<ClusterConnectorTaskInfo>
							...
							columns={[
								...
								},
								{
									...
									cell: ({ row: { original } }) => (
										<Code nowrap>{original.workerId}</Code>
									),
								},
							]}
						/>
...
const ConnectorErrorModal = observer((p: { error: ConnectorError }) => {
	...
	const hasConnectorLogs = api.topics?.any(
		(x) => x.topicName == LOGS_TOPIC_NAME,
	);
				...
				<ModalContent>
					...
					<ModalFooter gap={2}>
						{hasConnectorLogs && (
							<Button
								onClick={() =>
									appGlobal.history.push(`/topics/${LOGS_TOPIC_NAME}`)
								}
								mr="auto"
							>
								Show Logs
...
@observer
class KafkaConnectorDetails extends PageComponent<{
	clusterName: string;
	connector: string;
}> {
	...
	(p: {
		...
		const displayEntries = items.map((e) => {
			...
			if (e.entry.metadata.recommended_values?.length) {
				const match = e.entry.metadata.recommended_values.find(
					(x) => x.value == e.value,
				);
				if (match) {
...
const LogsTab = observer(
	(p: {
		...
		const createLogsTabState = () => {
			...
			const searchPromise = executeMessageSearch(
				search,
				topicName,
				connectorName,
			);
			searchPromise
				.catch((x) => (state.error = String(x)))
				.finally(() => (state.isComplete = true));
			return state;
		};

		const [state, setState] = useState(createLogsTabState);

		const loadLargeMessage = async (
			topicName: string,
			partitionID: number,
			offset: number,
		) => {
			...
			if (messages && messages.length == 1) {
				...
				const indexOfOldMessage = state.messages.findIndex(
				...
				if (indexOfOldMessage > -1) {
					...
					console.error(
						"LoadLargeMessage: cannot find old message to replace",
						{
							searchReq,
							messages,
						},
			...
			},
			{
				...
				cell: ({ row: { original } }) => (
					<MessagePreview
						...
						previewFields={() => []}
						isCompactTopic={
							topic ? topic.cleanupPolicy.includes("compact") : false
						}
					/>
				),
				...
				<Section minWidth="800px">
					<Flex mb="6">
						...
						<Button
							variant="outline"
							ml="auto"
							onClick={() => setState(createLogsTabState())}
						>
							Refresh logs
						</Button>
					</Flex>
					...
					<DataTable<TopicMessage>
						...
						subComponent={({ row: { original } }) => (
							<ExpandedMessage
								...
								loadLargeMessage={() =>
									loadLargeMessage(
										state.search.searchRequest!.topicName,
...
async function executeMessageSearch(
	...
	return transaction(async () => {
		try {
			...
			console.error(
				"error in connectorLogsMessageSearch: " +
