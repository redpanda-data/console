...
class KafkaConnectOverview extends PageComponent {
	...
	render() {
		...
		const tabs = [
			{
				...
				content: (
					<Box>
						<Text mb={4}>
							Redpanda Connect is an alternative to Kafka Connect. Choose from a
							growing ecosystem of readily available connectors.{" "}
							<Link
			...
			},
			{
				...
				content: (
					<Box>
						<Text mb={4}>
							Kafka Connect is our set of managed connectors. These provide a
							way to integrate your Redpanda data with different data systems.{" "}
							<Link
		...
		if (isServerless()) tabs.removeAll((x) => x.key == "kafkaConnect");

		return (
			<PageContent>
				<Text>
					There are two ways to integrate your Redpanda data with data from
					external systems: Redpanda Connect and Kafka Connect.
				</Text>
				{tabs.length == 1 ? (
					...
					<Tabs
						...
						defaultSelectedTabKey={
							showPipelines ? "redpandaConnect" : "kafkaConnect"
						}
					/>
				)}
			</PageContent>
			...
			<DataTable<ClusterConnectors>
				...
				columns={[
								...
								<span
									...
									onClick={() =>
										appGlobal.history.push(
											`/connect-clusters/${encodeURIComponent(r.clusterName)}`,
										)
									}
								>
									{r.clusterName}
					...
					},
					{
						...
						header: "Connectors",
						cell: ({ row: { original } }) => (
							<ConnectorsColumn observable={original} />
						),
					},
					{
						accessorKey: "connectors",
						...
						header: "Tasks",
						cell: ({ row: { original } }) => (
							<TasksColumn observable={original} />
						),
					},
				]}
			/>
...
const TabConnectors = observer(() => {
	...
	const allConnectors: ConnectorType[] =
		clusters?.flatMap((cluster) =>
			cluster.connectors.map((c) => ({ cluster, ...c })),
		) ?? [];
	...
	const isFilterMatch = (filter: string, item: ConnectorType): boolean => {
		try {
			const quickSearchRegExp = new RegExp(
				uiSettings.clusterOverview.connectorsList.quickSearch,
				"i",
			);
			return (
				Boolean(item.name.match(quickSearchRegExp)) ||
				Boolean(item.class.match(quickSearchRegExp))
			);
		} catch (e) {
			...
			<DataTable<ConnectorType>
				...
				columns={[
					...
					},
					{
						...
						accessorKey: "class",
						cell: ({ row: { original } }) => (
							<ConnectorClass observable={original} />
						),
					},
					{
						header: "Type",
					...
					},
					{
						header: "Cluster",
						cell: ({ row: { original } }) => (
							<Code nowrap>{original.cluster.clusterName}</Code>
						),
					},
				]}
			/>
