...
export class CommaSeparatedStringList extends Component<{
	...
	Item = observer(
		(props: { item: { id: string }; index: number }): JSX.Element => {
			...
			return (
				<>
					{/* Input */}
					<Tooltip
						label="[Enter] confirm, [ESC] cancel"
						isOpen={hasFocus}
						placement="top"
						hasArrow={true}
					>
	...
	AddButton = observer(() => {
		return (
			<div className="createEntryRow">
				<div
					className={"inputWrapper" + (this.newEntryError ? " hasError" : "")}
					style={{ height: "100%" }}
				>
					<Input
						...
						onChange={(e) => {
							...
							if (/^[a-z][a-z_\d]*$/i.test(this.newEntry) == false)
								this.newEntryError =
									"Name is not valid (only letters, digits, underscore)";
						}}
						placeholder={
							this.props.locale?.addInputPlaceholder ?? "Enter a name..."
						}
						spellCheck={false}
					/>
