...
export const ConfigPage: React.FC<ConfigPageProps> = observer(
	({ connectorStore, context }: ConfigPageProps) => {
		...
		for (const step of connectorStore.connectorStepDefinitions) {
			const groups = connectorStore.allGroups.filter(
				(g) => g.step.stepIndex == step.stepIndex,
			);
...
function ConnectorJsonEditor(p: {
	...
	const [jsonText, setJsonText] = useState(() => {
		...
		if (connectorName) {
			console.log(
				"trying to obtain initial config from existing connector...",
				{ name: connectorName },
			);
			const cluster = api.connectConnectors?.clusters?.first(
				(c) => c.clusterName == connectorStore.clusterName,
			);
			const connector = cluster?.connectors.first(
				(x) => x.name == connectorName,
			);
			if (connector) {
			...
			console.log(
				"unable to find existing connector for known connector name!",
				{
					clusterName: connectorStore.clusterName,
					connectorName,
				},
