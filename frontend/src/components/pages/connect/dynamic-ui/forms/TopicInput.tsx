...
export const TopicInput = observer(
	(p: { properties: Property[]; connectorType: "sink" | "source" }) => {
		...
		const errors = showErrors
			? state.property.errors
			: state.property.lastErrors;
		const errorToShow = showErrors
			? errors[state.property.currentErrorIndex % errors.length]
			: undefined;
		const cycleError = showErrors
			? () => state.property.currentErrorIndex++
			: undefined;
				...
				<FormControl position="relative">
					{state.properties.has("topics.regex") && (
						<Checkbox
							...
							onChange={(e) =>
								state.setSelectedProp(
									e.target.checked ? "topics.regex" : "topics",
								)
							}
						>
					...
					{showErrors && (
						<FormErrorMessage onClick={cycleError}>
							{errorToShow}
						</FormErrorMessage>
					)}
				</FormControl>
