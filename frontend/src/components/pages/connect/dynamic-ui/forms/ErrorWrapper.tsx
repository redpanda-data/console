...
const isRequiredError = (name: string) =>
	`Required configuration "${name}" must be provided`;
const isEmpty = (property: Property) =>
	property.value == "" || property.value == null;

export const ErrorWrapper = observer(function (
	props: PropsWithoutRef<{ property: Property; input: JSX.Element }>,
) {
			...
			<FormField
				...
				errorText={
					isEmpty(property) && isRequired
						? errorToShow || isRequiredError(property.name)
						: errorToShow
				}
				description={
					<ExpandableText maxChars={60}>
						{property.entry.definition.documentation}
					</ExpandableText>
				}
				onClick={cycleError}
			>
