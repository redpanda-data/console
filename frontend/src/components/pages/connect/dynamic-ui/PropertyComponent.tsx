...
export const PropertyComponent = observer((props: { property: Property }) => {
	...
	switch (def.type) {
			...
			if (metadata?.component_type === "RADIO_GROUP") {
				const options =
					metadata.recommended_values && metadata.recommended_values?.length > 0
						? metadata.recommended_values.map(
								({ value, display_name: label }) => ({ value, label }),
							)
						: recValues.map((v) => ({
								value: v,
		...
		case "FLOAT":
			inputComp = (
				<NumberInput value={Number(v)} onChange={(e) => (p.value = e)} />
			);
			break;
		...
		case "BOOLEAN":
			inputComp = (
				<Switch
					isChecked={Boolean(v)}
					onChange={(e) => (p.value = e.target.checked)}
				/>
			);
			break;
