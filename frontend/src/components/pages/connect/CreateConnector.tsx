...
const ConnectorType = observer(
	(p: {
		...
		if (p.activeCluster) {
			const allPlugins = api.connectAdditionalClusterInfo.get(
				p.activeCluster,
			)?.plugins;
			...
			filteredPlugins =
				allPlugins?.filter((p) => {
					...
					if (state.textFilter) {
						...
						if (
							meta.friendlyName &&
							containsIgnoreCase(meta.friendlyName, state.textFilter)
						)
							matchesFilter = true;
						...
						if (
							meta.description &&
							containsIgnoreCase(meta.description, state.textFilter)
						)
							matchesFilter = true;
				...
				{p.activeCluster && (
					<>
						<Flex direction="column" gap="1em">
							<Box maxWidth="600px">
								<Text>
									Select a managed connector. Connectors simplify importing and
									exporting data between Redpanda and popular data sources.{" "}
						...
						<HiddenRadioList<ConnectorPlugin>
							...
							options={filteredPlugins.map((plugin) => ({
								value: plugin,
								render: (card) => (
									<ConnectorBoxCard {...card} connectorPlugin={plugin} />
								),
							}))}
						/>
...
class CreateConnector extends PageComponent<{ clusterName: string }> {
	initPage(p: PageInitHelper) {
		...
		p.addBreadcrumb(
			clusterName,
			`/connect-clusters/${encodeURIComponent(clusterName)}`,
		);
		p.addBreadcrumb(
			"Create Connector",
			`/connect-clusters/${encodeURIComponent(clusterName)}/create-connector`,
		);
...
const ConnectorWizard = observer(
	({ connectClusters, activeCluster }: ConnectorWizardProps) => {
		const toast = useToast();
		const history = appGlobal.history;
		const [currentStep, setCurrentStep] = useState(0);
		const [selectedPlugin, setSelectedPlugin] =
			useState<ConnectorPlugin | null>(null);
		const [invalidValidationResult, setInvalidValidationResult] =
		...
		const [parsedUpdatedConfig, setParsedUpdatedConfig] = useState<any | null>(
			null,
		);
		const [postCondition, setPostCondition] = useState<boolean>(false);
		const [loading, setLoading] = useState<boolean>(false);
		const [connectClusterStore, setConnectClusterStore] = useState(
			ConnectClusterStore.getInstance(activeCluster),
		);
		const {
			isOpen: isCreatingModalOpen,
			onOpen: openCreatingModal,
		...
		const steps: Array<WizardStep> = [
					...
					<ConnectorType
						...
						onActiveClusterChange={(clusterName) => {
							uiState.pageBreadcrumbs = [
								...
								{
									title: clusterName!,
									linkTo: `/connect-clusters/${encodeURIComponent(clusterName!)}`,
								},
								{
			...
			},
			{
				...
				content: (
					<>
						...
						{selectedPlugin ? (
							<Box maxWidth="800px">
								<ConfigPage
									connectorStore={connectClusterStore.getConnector(
				...
				transitionConditionMet: async () => {
					if (selectedPlugin) {
						connectClusterStore
							.getConnector(selectedPlugin.class)
							.getConfigObject();
						setStringifiedConfig(
							connectClusterStore.getConnector(selectedPlugin.class).jsonText,
						);
			...
			},
			{
				title: "Review",
				description:
					"Review and optionally patch the created connector config.",
				...
				async transitionConditionMet(): Promise<{ conditionMet: boolean }> {
					clearErrors();
					setLoading(true);
					const connectorRef = connectClusterStore.getConnector(
						selectedPlugin!.class,
					);
					...
					try {
						...
						const errorCount = validationResult.configs.sum(
							(x) => x.value.errors.length,
						);
					...
					try {
						...
						await connectClusterStore.createConnector(
							selectedPlugin!.class,
							parsedUpdatedConfig,
						);
						...
						const connectorName = connectorRef.propsByName.get("name")!
							.value as string;

						while (true) {
							const elapsedTime = timer.value;
							console.log("scanning for new connector...", {
								connectorName,
								elapsedTime,
							});
							if (elapsedTime > maxScanTime) {
								...
								history.push(
									`/connect-clusters/${encodeURIComponent(activeCluster)}`,
								);
								break;
							}
							...
							const connector =
								connectClusterStore.getConnectorState(connectorName);
			...
			<>
				<Wizard
					state={{
						...
						next: async () => {
							const transitionConditionMet =
								steps[currentStep].transitionConditionMet;
