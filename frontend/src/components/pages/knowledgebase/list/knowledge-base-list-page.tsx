...
const embeddingModelOptions = ALL_EMBEDDING_MODELS.map(
...
const rerankerModelOptions = COHERE_RERANKER_MODELS.map(
	(model: RerankerModel) => ({
		value: model.name,
		label: model.name,
		icon: CohereIcon,
...
const getInputTopics = (kb: KnowledgeBase): string[] =>
	kb.indexer?.inputTopics || [];

const getEmbeddingGeneratorDisplay = (
	kb: KnowledgeBase,
...
export const createColumns = (
	...
	const { handleDelete, navigate, isDeletingKnowledgeBase, availableTopics } =
		options;
	return [
		{
			...
			cell: ({ row }) => {
				...
				return (
					<Text className="wrap-break-word font-medium">{displayName}</Text>
				);
			},
		},
		{
		...
		},
		{
			...
			header: ({ column }) => (
				<DataTableColumnHeader column={column} title="Input Topics" />
			),
			cell: ({ row }) => {
		...
		},
		{
			...
			header: ({ column }) => (
				<DataTableColumnHeader column={column} title="Embedding Generator" />
			),
			cell: ({ row }) => {
		...
		},
		{
			...
			header: ({ column }) => (
				<DataTableColumnHeader column={column} title="Reranker Model" />
			),
			cell: ({ row }) => {
...
function KnowledgeBaseDataTableToolbar({
	table,
}: {
	table: TanstackTable<KnowledgeBaseTableRow>;
}) {
...
export const KnowledgeBaseListPage = () => {
	...
	const {
		...
		isPending: isDeletingKnowledgeBase,
	} = useDeleteKnowledgeBaseMutation();
		...
		<div className="flex flex-col gap-4">
			<header className="flex flex-col gap-2">
				...
				<Text variant="muted">
					...
					information, providing accurate, contextual responses grounded in your
					data. Upload documents, configure embeddings, and create intelligent
					systems that can answer questions and provide insights from your
			...
			<div className="flex items-center justify-between">
				...
				<Button
					onClick={() => navigate({ to: "/knowledgebases/create" })}
					size="sm"
					variant="secondary"
				>
