...
export const KnowledgeBaseDetailsPage = () => {
  ...
  const consumerGroupId = knowledgeBase?.id ? `${knowledgeBase.id}-indexer` : '';
  const { data: consumerGroup } = useLegacyConsumerGroupDetailsQuery(consumerGroupId, {
  ...
  const handleSave = async () => {
    ...
    if (formData.indexer) {
      const exactTopics = (formData.indexer.exactTopics || []).filter((topic) => topic && topic.trim() !== '');
      const regexPatterns = (formData.indexer.regexPatterns || []).filter(
    ...
    updateMask = updateMask.map((path) => {
      ...
      if (path === 'indexer.exact_topics' || path === 'indexer.regex_patterns') {
