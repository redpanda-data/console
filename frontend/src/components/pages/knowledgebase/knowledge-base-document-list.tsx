...
const transformRetrievalResult = (result: RetrievalResult, index: number): RetrievalResultRow => ({
...
const createColumns = (): ColumnDef<RetrievalResultRow>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      return <Text variant="default">{score !== undefined ? score.toFixed(3) : '-'}</Text>;
    },
  },
  {
    accessorKey: 'topic',
  ...
  },
  {
    ...
    cell: ({ row }) => <Text variant="default">{row.getValue('documentName')}</Text>,
  },
  {
    accessorKey: 'chunkId',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Chunk ID" />,
    cell: ({ row }) => <Text variant="muted">{row.getValue('chunkId')}</Text>,
  },
  {
    accessorKey: 'content',
    ...
    cell: ({ row }) => {
      ...
      return (
        <Text className="wrap-break-word line-clamp-2 max-w-[400px]" variant="default">
          {content}
        </Text>
      );
    },
...
function RetrievalResultsToolbar({ table }: { table: ReturnType<typeof useReactTable<RetrievalResultRow>> }) {
...
type KnowledgeBaseDocumentListProps = {
  ...
  isLoading: boolean;
  onRowClick: (result: RetrievalResult) => void;
};

export const KnowledgeBaseDocumentList: React.FC<KnowledgeBaseDocumentListProps> = ({
  results,
  isLoading,
  onRowClick,
}) => {
