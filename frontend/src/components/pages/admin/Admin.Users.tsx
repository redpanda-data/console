...
export class AdminUsers extends Component<{}> {
	...
	render() {
		...
		try {
			...
			users = users.filter(
				(u) =>
					u.internalIdentifier.match(quickSearchRegExp) ||
					u.oauthUserId.match(quickSearchRegExp),
			);
		} catch (e) {
				...
				columns={[
						...
						cell: ({ row }) => {
							if (
								...
								return (
									<Flex gap={2}>
										<Tooltip
											label="You are currently logged in as this user"
											placement="top"
											hasArrow
										>
											<Box>
					...
					{ size: 1, header: "OAuthUserID", accessorKey: "oauthUserId" },
					{
						...
						cell: ({ row: { original: user } }) =>
							user.grantedRoles.map((r) => r.role.name).join(", "),
					}, // can't sort
