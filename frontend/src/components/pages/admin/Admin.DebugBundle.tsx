...
import { Alert, AlertIcon, Box, Button, Checkbox, ConfirmModal, DateTimeInput, Flex, FormField, Grid, GridItem, Input, isMultiValue, isSingleValue, PasswordInput, Select, Text } from '@redpanda-data/ui';
...
const Header = () => <Text>Collect environment data that can help debug and diagnose issues with a Redpanda cluster, a broker, or the machine it’s running on. This will bundle the collected data into a ZIP file.</Text>
...
const NewDebugBundleForm: FC<{ onSubmit: (data: CreateDebugBundleRequest) => void, error?: ErrorResponse, debugBundleExists: boolean }> = observer(({ onSubmit, error, debugBundleExists }) => {
    ...
    const generateNewDebugBundle = () => {
        onSubmit(advancedForm ? new CreateDebugBundleRequest({
        ...
        <Box mt={4}>
            ...
            <Flex gap={2} mt={4}>
                {debugBundleExists ? <ConfirmModal trigger={advancedForm ? 'Generate':'Generate default'} heading="Generate new debug bundle" onConfirm={() => {
                    ...
                    <Button onClick={() => {
                        generateNewDebugBundle();
                    }}>{advancedForm ? 'Generate':'Generate default'}</Button>
                }
                {advancedForm ?
