...
const Header = () => (
	<Text>
		Collect environment data that can help debug and diagnose issues with a
		Redpanda cluster, a broker, or the machine it’s running on. This will bundle
		the collected data into a ZIP file.
	</Text>
);
	...
	render() {
		...
		if (api.isDebugBundleInProgress) {
			return (
				<Box>
					...
					<Text>
						Started{" "}
						{api.debugBundleStatus?.createdAt?.toDate().toLocaleString()}
					</Text>
				</Box>
			);
		}
		...
		return (
			<Box>
				...
				{api.debugBundleStatuses.length === 0 && (
					<Text>No debug bundle available for download.</Text>
				)}
							...
							api
								...
								.then((result) => {
									appGlobal.history.push(
										`/admin/debug-bundle/progress/${result.jobId}`,
									);
								})
			...
			advancedForm
				? new CreateDebugBundleRequest({
						...
						controllerLogsSizeLimitBytes:
							formState.controllerLogsSizeLimitBytes,
						cpuProfilerWaitSeconds: formState.cpuProfilerWaitSeconds,
						logsSince: formState.logsSince
							? Timestamp.fromDate(new Date(formState.logsSince))
							: undefined,
						logsSizeLimitBytes:
							formState.logsSizeLimitBytes * formState.logsSizeLimitUnit,
						logsUntil: formState.logsUntil
							? Timestamp.fromDate(new Date(formState.logsUntil))
							: undefined,
						...
						labelSelector: formState.labelSelectors.map(
							(x) => new LabelSelector(x),
						),
		...
		<Box mt={4}>
			{advancedForm && (
				...
				>
					<Alert status="info" my={2}>
						<AlertIcon />
						This is an advanced feature, best used if you have received
						direction to do so from Redpanda support.
					</Alert>
					<FormField
						label="SCRAM user"
					...
					<FormField
						label="Broker(s)"
						description="Specify broker IDs (or leave blank for all)"
					>
					...
					>
						<Input
							...
							onChange={(e) =>
								formState.setControllerLogsSizeLimitBytes(
					...
					>
						<Input
							...
							type="number"
							onChange={(e) =>
								formState.setCpuProfilerWaitSeconds(e.target.valueAsNumber)
							}
						/>
					</FormField>
					<FormField
					...
					>
						<Flex gap={2}>
							<Input
								...
								onChange={(e) =>
									formState.setLogsSizeLimitBytes(e.target.valueAsNumber)
								}
							/>
							<Select
								chakraStyles={{
					...
					>
						<Input
							...
							onChange={(e) =>
								formState.setMetricsIntervalSeconds(e.target.valueAsNumber)
							}
						/>
					</FormField>
					<FormField
			...
			<Flex gap={2} mt={4}>
				{debugBundleExists ? (
					...
					>
						You have an existing debug bundle; generating a new one will delete
						the previous one. Are you sure?
					</ConfirmModal>
				) : (
					<Button
						onClick={() => {
