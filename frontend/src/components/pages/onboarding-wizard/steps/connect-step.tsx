...
const getComponentTypeLabel = (
	type: ConnectComponentType | undefined,
): string => {
...
const ConnectByConnect = ({
	...
	const { data: connectConfig } = useConnectConfig();
	const connectionName = connectConfig?.connectionName;
	const componentConfig = getComponentByName(
		connectionName,
		additionalComponents,
	);
	const {
	...
	return (
		<div className="flex flex-col gap-8">
			<Card size="full">
				...
				<CardContent>
					Add any additional configurations to the yaml config I've generated
					for you.
				</CardContent>
			</Card>
...
const useExternalDocumentation = (
	url?: string,
...
const ExternalSetupGuidanceCard = () => {
	return (
		<Card size="full">
			...
			<CardContent>
				<Text>
					Complete your setup to start streaming data to your Redpanda cluster
					using the Kafka API. Follow the documentation below to configure your
					connection and begin processing data.
				</Text>
			</CardContent>
		</Card>
	);
...
const ConnectByExtendedComponent = ({
	component,
}: {
	component: ExtendedConnectComponentSpec;
}) => {
	...
	const {
		...
		component.externalDocs?.primaryUrl,
		component.externalDocs?.format,
	);
	...
	const {
		...
		component.externalDocs?.secondaryUrl,
		component.externalDocs?.format,
	);
			...
			{isLoading ? (
				...
				<Card size="full">
					<CardContent className="text-center py-8">
						<Text variant="muted">
							Unable to load documentation. Please check the external
							documentation URLs.
						</Text>
				...
				<div className="flex flex-col gap-6">
					...
					{!primaryContent && !secondaryContent && (
						<Card size="full">
							<CardContent className="text-center py-8">
								<Text variant="muted">
									No external documentation content available for this
									connection.
								</Text>
							</CardContent>
						</Card>
