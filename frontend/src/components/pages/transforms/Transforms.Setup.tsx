import { Alert, AlertDescription, AlertIcon, AlertTitle, Box, CodeBlock, Grid, Heading, ListItem, OrderedList, Stack, Text } from '@redpanda-data/ui';
    ...
    return <>
        <OrderedList display="flex" flexDirection="column" gap="0.5rem">
            <ListItem>
                ...
                <Text mt={3}>If you do not include the <code>--language</code> flag, the <code>transform init</code> command will prompt you for the language.</Text>
                ...
                <Text mt={3}>The <code>transform.go</code> file contains the transform logic, and the <code>transform.yaml</code> file specifies the transform's configuration.</Text>
                ...
                <Alert status="info" >
                    ...
                    <Box>
                        ...
                        <AlertDescription>
                            When creating a custom data transform, initialization steps can be done either in <code>main</code> (because it's only run once at the start of the package) or in Go's standard predefined <code>init()</code> function.
                            Although state can be cached in global variables, Redpanda may restart a WASM module at any point, which causes the state to be lost.
                        </AlertDescription>
                    </Box>
                </Alert>
            </ListItem>
