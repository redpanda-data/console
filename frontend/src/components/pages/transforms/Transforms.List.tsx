...
export const PartitionStatus = observer(
	(p: { status: PartitionTransformStatus_PartitionStatus }) => {
		switch (p.status) {
								...
								cell: ({ row: { original: r } }) => {
									return (
										<Box wordBreak="break-word" whiteSpace="break-spaces">
											<Link
												to={`/transforms/${encodeURIComponentPercents(r.name)}`}
											>
												{r.name}
											</Link>
										</Box>
								...
								cell: ({ row: { original: r } }) => {
									if (
										r.statuses.all(
											(x) =>
												x.status ==
												PartitionTransformStatus_PartitionStatus.RUNNING,
										)
									)
										return (
