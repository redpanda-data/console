					...
					tabs={[
						{
							key: "overview",
							title: <>Overview</>,
							content: <OverviewTab transform={transform} />,
						},
						{
							key: "logs",
							title: <>Logs</>,
							content: <LogsTab transform={transform} />,
						},
					]}
				/>
			</PageContent>
...
const OverviewTab = observer(
	(p: {
		...
		let overallStatus = <></>;
		if (
			p.transform.statuses.all(
				(x) => x.status == PartitionTransformStatus_PartitionStatus.RUNNING,
			)
		)
			overallStatus = (
				<PartitionStatus
					status={PartitionTransformStatus_PartitionStatus.RUNNING}
				/>
			);
		else {
			const s = p.transform.statuses.first(
				(x) => x.status != PartitionTransformStatus_PartitionStatus.RUNNING,
			)!;
...
async function executeMessageSearch(
	...
	return runInAction(async () => {
		try {
			...
			console.error(
				"error in transformLogsMessageSearch: " +
