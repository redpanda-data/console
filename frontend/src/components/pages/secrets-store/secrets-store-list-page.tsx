	...
	return [
		{
			...
			cell: ({ row }) => (
				<Text className="font-medium">{row.getValue("id")}</Text>
			),
		},
		{
			accessorKey: "labels",
...
function SecretsStoreDataTableToolbar({
	table,
}: {
	table: TanstackTable<SecretTableRow>;
}) {
...
export const SecretsStoreListPage = () => {
	...
	const { mutateAsync: deleteSecret, isPending: isDeletingSecret } =
		useDeleteSecretMutation();
		...
		<TooltipProvider>
			<div className="flex flex-col gap-4">
				<header className="flex flex-col gap-2">
					...
					<Text variant="muted">
						This page lets you list, edit, and delete the secrets used in your
						dataplane. You can create secrets on this page and reference them
						when creating a new resource such as Redpanda Connect pipelines.
					</Text>
				</header>
				...
				<div className="flex items-center justify-between">
					...
					<Button
						onClick={() => navigate({ to: "/secrets/create" })}
						size="sm"
						variant="secondary"
					>
