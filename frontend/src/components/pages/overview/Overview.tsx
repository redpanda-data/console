		...
		const renderIdColumn = (
			...
			return (
				<Flex alignItems="flex-start" gap={4}>
					{text}
					<Tooltip
						label="This broker is the current controller of the cluster"
						placement="right"
						hasArrow
					>
						<Box>
				...
				<PageContent className="overviewGrid">
					<Section py={5} my={4}>
						<Flex>
							...
							<Statistic
								title="Replicas"
								value={overview.kafka.replicasCount}
							/>
						</Flex>
					</Section>

					<Grid
						gridTemplateColumns={{ base: "1fr", lg: "fit-content(60%) 1fr" }}
						gap={6}
					>
									...
									columns={[
										{
											...
											cell: ({ row: { original: broker } }) =>
												renderIdColumn(`${broker.brokerId}`, broker),
										},
										{
											header: "Status",
											cell: ({ row: { original: broker } }) => (
												<Flex gap={2}>
													{api.clusterHealth?.offlineBrokerIds.includes(
														broker.brokerId,
													) ? (
														<>
										...
										},
										{
											...
											cell: ({
												...
												totalLogDirSizeBytes &&
												prettyBytesOrNA(totalLogDirSizeBytes),
										},
										{
											id: "view",
											size: 100,
											...
											? [
													{
														...
														cell: ({
															row: { original: broker },
														}: { row: Row<BrokerWithConfigAndStorage> }) =>
															broker.rack,
													},
												]
											: []),
							...
							<Section py={4}>
								...
								>
									<ul className="resource-list">
										<li>
											<a
												href="https://docs.redpanda.com/docs/home/"
												rel=""
...
type DetailsProps = {
	title: string;
	content: ([left?: React.ReactNode, right?: React.ReactNode] | undefined)[];
};
...
function ClusterDetails() {
	...
	const totalPrimaryStorageBytes = brokers.sum(
		(x) => x.totalPrimaryLogDirSizeBytes ?? 0,
	);
	const totalReplicatedStorageBytes =
		totalStorageBytes - totalPrimaryStorageBytes;

	const serviceAccounts =
		overview.redpanda.userCount ?? "Admin API not configured";

	const aclCount =
		overview.kafka.authorizer?.aclCount ?? "Authorizer not configured";
	...
	const hasConnect =
		overview.kafkaConnect?.isConfigured == true && clusters.length > 0;
	...
	return (
		...
		>
						...
						overview.schemaRegistry.isConfigured
							? [
									[
										...
										overview.schemaRegistry.status == "HEALTHY" &&
										overview.schemaRegistry.isConfigured
			...
			<DetailsBlock title="Storage">
				<Details
					title="Total Bytes"
					content={[[prettyBytesOrNA(totalStorageBytes)]]}
				/>

				<Details
					title="Primary"
					content={[[prettyBytesOrNA(totalPrimaryStorageBytes)]]}
				/>

				<Details
					title="Replicated"
					content={[[prettyBytesOrNA(totalReplicatedStorageBytes)]]}
				/>
			</DetailsBlock>
						...
						? [
								[
									<Flex key="error" gap={1} alignItems="center">
										<MdOutlineError color={colors.brandError} size={16} />{" "}
