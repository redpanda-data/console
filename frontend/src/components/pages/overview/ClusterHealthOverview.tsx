			...
			<List spacing={3}>
						...
						<Box>
							{titleCase(
								api.clusterHealth?.unhealthyReasons
									?.map(
										(x) =>
											HUMAN_READABLE_UNHEALTHY_REASONS[x].toLowerCase() ?? x,
									)
									.join(", ") ?? "",
							)}
						</Box>
				...
				<ListItem>
					<Grid templateColumns={{ sm: "1fr", md: "1fr 1fr" }} gap={4}>
						...
						<Flex gap={1}>
							{api.clusterHealth?.offlineBrokerIds &&
								api.clusterHealth?.offlineBrokerIds.length > 0 && (
				...
				{api.clusterHealth?.unhealthyReasons.includes(
					...
					<ListItem>
						<Grid templateColumns={{ sm: "1fr", md: "1fr 1fr" }} gap={4}>
							<Box fontWeight="bold">
								{
									HUMAN_READABLE_UNHEALTHY_REASONS[
										UnhealthyReason.LEADERLESS_PARTITIONS
									]
								}
							</Box>
						...
						<Grid templateColumns={{ sm: "1fr", md: "1fr 1fr" }} gap={4}>
							...
							<Flex gap={2}>
								<Flex gap={1}>
									...
									<Text>
										{api.clusterHealth?.underReplicatedPartitionsCount}
									</Text>
								</Flex>{" "}
								<Link as={ReactRouterLink} to="/topics">
									View topics
