...
import {
	...
	FeatureEntry,
	isSupported,
	shouldHideIfNotSupported,
} from "../state/supportedFeatures";
...
} from "./pages/schemas/Schema.Create";
...
export function createVisibleSidebarItems(
	entries: IRouteEntry[],
): NavLinkProps[] {
	return entries
		.map((entry) => {
...
export const APP_ROUTES: IRouteEntry[] = [
	...
	MakeRoute<{}>(
		...
		() => {
			if (isServerless()) {
				...
				console.debug(
					"Pipeline Service NOT enabled. NOT showing sidebar link.",
				);
				return {
					visible: false,
					disabledReasons: [DisabledReasons.notSupported],
				};
			} else {
				...
				console.debug(
					"Pipeline Service state does not matter. Showing sidebar link.",
				);
				return { visible: true, disabledReasons: [] };
			}
		},
	...
	MakeRoute<{ transformName: string }>(
	...
	MakeRoute<{ pipelineId: string }>(
		...
		RpConnectPipelinesDetails,
		"Connectors",
	),
	MakeRoute<{ pipelineId: string }>(
		"/rp-connect/:pipelineId/edit",
		RpConnectPipelinesEdit,
		"Connectors",
	),
	...
	MakeRoute<{}>(
		"/admin/debug-bundle/progress/:jobId",
