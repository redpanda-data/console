...
export const WhiteCard = (
	p: PropsWithChildren<{ style?: React.CSSProperties; title?: string }>,
) => (
	<div
		style={{
			margin: "1em 2em",
	...
	return (
		...
		>
			...
			<ModalContent minW="xl">
				...
				<ModalBody>
					The page must be reloaded to apply the newest version of the frontend.
				</ModalBody>
				<ModalFooter gap={2}>
					<Button
						variant="outline"
...
export function renderLogDirSummary(summary: TopicLogDirSummary): JSX.Element {
	...
	return (
		<>
			{prettyBytesOrNA(summary.totalSizeBytes)}{" "}
			<WarningToolip content={summary.hint} position="left" />
		</>
	);
}

export function WarningToolip(p: {
		...
		<ZeroSizeWrapper>
			...
			>
				...
				<span
					...
					style={p.position == "left" ? styleLeft : undefined}
				>
					{p.content}
				</span>
			</div>
