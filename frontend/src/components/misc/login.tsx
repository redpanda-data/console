...
class Login extends Component {
	...
	render() {
		...
		const providerButtons = allProviders?.filter(
			(x) => x.authenticationMethod != "PLAIN_CREDENTIALS",
		);
		const plainLoginProvider = allProviders?.first(
			(x) => x.authenticationMethod == "PLAIN_CREDENTIALS",
		);
				...
				<div className="loginContainer">
					<div className="loginLeft">
						...
						<Stack spacing="2">
							...
							<Text fontSize="lg">
								Sign in with an OAuth provider to&nbsp;continue
							</Text>
						</Stack>
					</div>
					...
					<div className="loginRight">
						<div className="loginContainerRight">
							...
							<div style={{ marginTop: "auto", fontWeight: "normal" }}>
								Copyright © {new Date().getFullYear()} Redpanda Data, Inc. All
								rights reserved.
							</div>
						</div>
					</div>
