...
export function ConfigList({
	...
	const allTypesUnknown = configEntries.all((x) =>
		equalsIgnoreCase(x.type, "unknown"),
	);
	...
	const tableColumns: ColumnDef<ConfigEntry>[] = [
		{
			...
			cell: ({ row: { original: record } }) => {
				...
				const sensitive = record.isSensitive && (
					<Tooltip
						label="Value has been redacted because it's sensitive"
						placement="top"
						hasArrow
					>
						<Box>
			...
			<span className={styles.sourceHeader}>
				...
				<Tooltip
					label={
						<>
							<p>
								Resources can be configured at different levels. Example: A
								topic config may be inherited from the static broker config.
							</p>
							<p>
								Valid sources are: Dynamic Topic, Dynamic Broker, Default
								Broker, Static Broker, Dynamic Broker Logger and Default config.
							</p>
						</>
					}
					placement="left"
	...
	return (
		<DataTable<ConfigEntry>
			...
			rowClassName={(row) =>
				row.original.isExplicitlySet ? styles.overidden : styles.default
			}
			columns={tableColumns}
		/>
	);
