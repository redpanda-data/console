...
const ErrorPage = observer(() => {
	fetchWithTimeout(`${config.restBasePath}/console/endpoints`, 5 * 1000).then(
		(r) => {
			if (r.ok) {
				...
				<HStack spacing={12} maxW="container.md">
					<VStack spacing={4} display="flex" alignItems="flex-start">
						...
						<Box margin={0} padding={0}>
							<Text>
								You may encounter this error if your user account was recently
								created or user permissions were recently changed. It can take
								up to 15 minutes for user account and permission changes to
								propagate across all clusters. Try again in 15 minutes and if
								the error persists{" "}
								<Button
									as={Link}
									margin={0}
