	...
	if (api.isRedpanda) {
		if (msToExpiration > 15 * MS_IN_DAY && msToExpiration < 30 * MS_IN_DAY) {
			return {
				message: (
					<Box>
						Your Redpanda Enterprise trial will expire in{" "}
			...
			if (consoleHasEnterpriseFeature("SINGLE_SIGN_ON")) {
				return {
					message: (
						<Box>
							<Text>
								Your Redpanda Enterprise trial is expiring in{" "}
								{getPrettyTimeToExpiration(license)} and Console SSO/RBAC is
								enabled. As a result, Console will be inaccessible after license
								expiry. To prevent this,{" "}
				...
				return {
					message: (
						<Box>
							<Text>
								Your Redpanda Enterprise trial is expiring in{" "}
								{getPrettyTimeToExpiration(license)}; at that point, your
								enterprise features will become unavailable. To get a full
...
export const OverviewLicenseNotification: FC = observer(() => {
	...
	const license = api.licenses
		.filter((license) => license.type === License_Type.TRIAL)
		.first();
