...
export const LicenseNotification = observer(() => {
	...
	const enterpriseLicenses = api.licenses.filter(
		(license) => license.type === License_Type.ENTERPRISE,
	);

	const visibleExpiredEnterpriseLicenses =
		enterpriseLicenses.filter(licenseIsExpired) ?? [];
	const soonToExpireLicenses =
		enterpriseLicenses.filter((license) => licenseSoonToExpire(license)) ?? [];

	const showSomeLicenseExpirationInfo =
		visibleExpiredEnterpriseLicenses.length || soonToExpireLicenses.length;
	...
	if (
		location.pathname === "/admin/upload-license" ||
	...
	const activeEnterpriseFeatures = api.enterpriseFeaturesUsed.filter(
		(x) => x.enabled,
	);

	const visibleSoonToExpireLicenses =
		soonToExpireLicenses.length > 1 &&
		new Set(soonToExpireLicenses.map((x) => x.expiresAt)).size === 1
			? soonToExpireLicenses.filter(
					(x) => x.source === License_Source.REDPANDA_CORE,
				)
	...
	const visibleExpiredLicenses =
		...
		new Set(visibleExpiredEnterpriseLicenses.map((x) => x.expiresAt)).size === 1
			? visibleExpiredEnterpriseLicenses.filter(
					(x) => x.source === License_Source.REDPANDA_CORE,
				)
				...
				<AlertDescription>
					{visibleSoonToExpireLicenses.length > 0 && (
						<Box>
							{visibleSoonToExpireLicenses.map((license, idx) => (
								<Text key={idx}>
									Your {prettyLicenseType(license, true)} license is expiring in{" "}
									{getPrettyTimeToExpiration(license)}.
								</Text>
							))}
						</Box>
					)}
					...
					{api.licenseViolation && (
						<Text>
							...
							in your connected Redpanda cluster.{" "}
							{activeEnterpriseFeatures.length === 1
