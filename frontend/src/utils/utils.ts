...
function collectElementsRecursive(
	ctx: PropertySearchExContext,
	obj: any,
...
export const prettyBytes = function (
	n: number | string | null | undefined,
	options?: PrettyValueOptions,
) {
...
export function scrollTo(
	targetId: string,
	anchor: "start" | "end" | "center" = "center",
	offset?: number,
): void {
	const mainLayout = document.getElementById("mainLayout");
	if (!mainLayout) return;
	...
	mainLayout.scrollTo({
		behavior: "smooth",
		top:
			target.getBoundingClientRect().top + mainLayout.scrollTop + (offset ?? 0),
	});
}
...
export function retrier<T>(
	operation: () => Promise<T>,
	{ attempts = Infinity, delayTime = 100 },
): Promise<T> {
 ...
 */
export function substringWithEllipsis(
	input: string,
	maxLength: number,
): string {
