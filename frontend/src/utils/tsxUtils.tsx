...
@observer
export class TimestampDisplay extends Component<{
...
export function QuickTable(
	...
	// array of [any, any] ?
		...
		// Convert to array of key-value objects
		entries = (data as [any, any][]).map((ar) => ({
			key: ar[0],
			value: ar[1],
		}));
	}
	...
	const o = Object.assign(
		{} as QuickTableOptions,
		DefaultQuickTableOptions,
		options,
	);

	const showVerticalGutter =
		(typeof o.gapHeight === "number" && o.gapHeight > 0) ||
		typeof o.gapHeight === "string";
			...
			<tbody>
				{entries.map((obj, i) => (
					<React.Fragment key={i}>
						<tr>
							<td
								style={{ textAlign: o.keyAlign, ...o.keyStyle }}
								className="keyCell"
							>
								{React.isValidElement(obj.key)
									? obj.key
									: toSafeString(obj.key)}
							</td>
							<td
								style={{ minWidth: "0px", width: o.gapWidth, padding: "0px" }}
							></td>
							<td
								style={{ textAlign: o.valueAlign, ...o.valueStyle }}
								className="valueCell"
							>
								{React.isValidElement(obj.value)
									? obj.value
									: toSafeString(obj.value)}
							</td>
						</tr>
...
const style_flexColumn: CSSProperties = {
	display: "flex",
...
export const InfoText = (p: {
	...
	const overlay =
		p.maxWidth || p.align ? (
			<div style={{ maxWidth: p.maxWidth, textAlign: p.align }}>
				{p.tooltip}
			</div>
		) : (
			p.tooltip
				...
				options={p.options.map((kv) => ({
					...
					label: (
						<Box p={3}>
							...
							{kv.content &&
								(p.showContent == "always" || p.value == kv.value) && (
									<Box
										key={String(kv.value)}
										style={{ marginLeft: "27px", marginTop: "12px" }}
									>
										<div>{kv.content}</div>
									</Box>
								)}
				...
				<Flex fontSize="sm" fontWeight="bold">
					<div>
						{this.showWaitingText
							? "Redpanda Console is waiting for new messages..."
				...
				{this.props.bytesConsumed && this.props.messagesConsumed && (
					<Flex fontSize="sm" fontWeight="bold" justifyContent="space-between">
						<Flex alignItems="center" gap={2}>
							<MdOutlineDownload color={colors.brandOrange} size={14} />{" "}
							{this.props.bytesConsumed}
						</Flex>
						<Flex alignItems="center" gap={2}>
							<MdContentCopy color={colors.brandOrange} size={14} />{" "}
							{this.props.messagesConsumed} messages
						</Flex>
					</Flex>
				)}
			</Box>
	...
	render() {
		...
		return (
			<span className="verticalCenter" style={style}>
				<span
					style={
						p.positionContentAbsolute ? { position: "absolute" } : undefined
					}
				>
					{this.props.children}
				</span>
...
export type ButtonProps = Omit<RpButtonProps, "disabled" | "isDisabled"> & {
...
export const navigatorClipboardErrorHandler = (e: DOMException) => {
	toast({
		status: "error",
		description:
			"Unable to copy settings to clipboard. See console for more information.",
	});
