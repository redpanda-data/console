...
function findGeneric(
	...
	if (typeof arg1 == "string") {
		...
		throw new Error(
			"first parameter of find() must be: string, or function, or pattern object",
		);
	}
}
...
function findByName(
	...
	const isMatch = caseSensitive
		...
		: (_: any, prop: string | number) =>
				String(prop).toUpperCase() == propertyName.toUpperCase();
...
function findByPattern(
	...
	const isPatternMatch = (obj: object, pattern: object): boolean => {
		if (typeof obj !== typeof pattern) {
			log(
				`  type mismatch obj<>pattern: '${typeof obj}' != '${typeof pattern}'`,
			);
			return false;
		}
		...
		for (const k in pattern) {
			...
			if (typeof objValue !== typeof patternValue) {
				log(
					`  property type mismatch: '${typeof objValue}' != '${typeof patternValue}'`,
				);
				return false;
			}
			...
			if (typeof objValue === "string") {
				...
				if (caseSensitive) {
					if (objValue != patternValue) {
						log(
							`  strings don't match (case sensitive): "${objValue}" != "${patternValue}"`,
						);
						return false;
					}
				} else {
					if (
						String(objValue).toUpperCase() != String(patternValue).toUpperCase()
					) {
						log(
							`  strings don't match (ignore case): "${objValue}" != "${patternValue}"`,
						);
						return false;
					}
				}
