...
import { getSidebarItemTitle } from 'components/pages/agents/agent-list-page';
...
export const embeddedAvailableRoutesObservable = observable({
  get routes() {
    return APP_ROUTES.map((route) => {
      if (route.path === '/agents' || route.path === '/knowledgebases' || route.path === '/remote-mcp') {
        return {
          ...route,
          // Needed because we cannot use JSX in this file
          title: getSidebarItemTitle({ route }), // This will automatically add the beta badge
        };
      }
      return route;
    })
