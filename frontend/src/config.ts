...
import {
	Interceptor as ConnectRpcInterceptor,
	StreamRequest,
	UnaryRequest,
	createPromiseClient,
	PromiseClient,
...
const getRestBasePath = (overrideUrl?: string) =>
	overrideUrl ?? DEFAULT_API_BASE;

const getGrpcBasePath = (overrideUrl?: string) => overrideUrl ?? getBasePath();

const addBearerTokenInterceptor: ConnectRpcInterceptor =
 ...
 *
 */
const checkExpiredLicenseInterceptor: ConnectRpcInterceptor =
	(next) => async (req: UnaryRequest | StreamRequest) => {
		try {
			return await next(req);
		} catch (error) {
...
const setConfig = ({
	...
	const assetsUrl =
		urlOverride?.assets === "WEBPACK"
			? String(__webpack_public_path__).removeSuffix("/")
			: urlOverride?.assets;
	...
	const debugBundleGrpcClient = createPromiseClient(
		DebugBundleService,
		transport,
	);
	...
	loader.init().then(async (monaco) => {
		window.MonacoEnvironment = {
			...
			getWorker(moduleId, label) {
				console.debug(
					`window.MonacoEnvironment.getWorker looking for moduleId ${moduleId} label ${label}`,
				);
				switch (label) {
