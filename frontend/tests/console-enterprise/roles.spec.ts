...
test.describe('Roles', () => {
  ...
  test('should be able to search for role with regexp', async ({ page }) => {
    ...
    expect(
      await page.getByTestId('data-table-cell').locator(`a[href='/security/roles/${roleName1}/details']`).count(),
    ).toEqual(1);
    expect(
      await page.getByTestId('data-table-cell').locator(`a[href='/security/roles/${roleName2}/details']`).count(),
    ).toEqual(1);
    expect(
      await page.getByTestId('data-table-cell').locator(`a[href='/security/roles/${roleName3}/details']`).count(),
    ).toEqual(0);
  ...
  test('Allow All Operations in Roles should pre-set the form', async ({ page }) => {
    ...
    expect(
      await page.getByTestId('create-role-transactional-ids-section').getByRole('textbox').nth(0).inputValue(),
    ).toBe('*');
  });
