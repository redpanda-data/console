...
export class ShadowlinkPage {
	...
	async goto() {
		...
		await expect(this.page.getByRole('heading', { name: /shadow links/i })).toBeVisible({ timeout: 10000 });
	}

	async gotoCreate() {
		await this.page.goto(`${this.baseURL}/shadowlinks/create`);
		await expect(this.page.getByRole('heading', { name: /create shadow link/i })).toBeVisible({ timeout: 10000 });
	}
	...
	async gotoEdit(name: string) {
		...
		await expect(this.page.getByRole('heading', { name: /edit shadow link/i })).toBeVisible({ timeout: 10000 });
	}
	...
	async verifyOnHomePage(shadowlinkName: string, timeout = 30000) {
		// Navigate to home page (root after login)
		await this.page.goto(this.baseURL);

		// Wait for home page to load - look for common elements
		await expect(this.page.getByRole('heading', { name: /overview|dashboard|home/i })).toBeVisible({ timeout: 10000 });
	...
	async verifyStatus(expectedStatus: 'ACTIVE' | 'PAUSED' | 'FAILED_OVER', timeout = 30000) {
		await expect(async () => {
			await this.page.reload({ waitUntil: 'domcontentloaded', timeout: 5000 });
	...
	async verifyTopicCount(expectedCount: number, timeout = 30000) {
	...
	async verifyTopicExists(topicName: string) {
		...
		await expect(topicElement).toBeVisible({ timeout: 30000 });
	}

	async verifyMetrics(expectedMetrics: {
		totalTopics?: number;
	 ...
	 * Edit methods
	 */
	async updateTopicFilters(filters: Array<{
		type: 'PREFIX' | 'LITERAL';
		filter: 'INCLUDE' | 'EXCLUDE';
		pattern: string;
	}>) {
		...
		for (const btn of buttonsForChevron) {
			...
			const isChevron = await btn.evaluate((button) => {
				...
				return classes.includes('lucide-chevron-down') || (classes.includes('h-4') && classes.includes('w-4') && classes.includes('transition-transform'));
			});
	...
	async deleteShadowlink() {
		...
		await expect(this.page).toHaveURL(`${this.baseURL}/shadowlinks`, { timeout: 15000 });
