  ...
  [
    ...
    [
      ...
      test(`${testName} - ${type}`, async ({ page }) => {
        ...
        // Verify the created ACL/role appears in the list using specific test ID
...
test.describe('ACL Disable Rules Validation', () => {
  test('check disable rules', async ({ page }) => {
    ...
    // Configure the first two rules (cluster and schema registry)
    await aclPage.configureRules(rules);

    // Add a third rule to test disabled buttons
    await aclPage.addNewRule();

    // Validate that cluster and schema registry buttons are disabled for the new rule (index 2)
    ...
    // Select a different resource type for the third rule (topic should be available)
    await aclPage.selectResourceType(2, newRule);

    await aclPage.configureRule(2, newRule);

    // Validate the summary shows all configured rules
    ...
    // Verify ACL creation was successful
  ...
  [
    ...
    [
      ...
      test(`${testName} - ${type}`, async ({ page }) => {
        ...
        // Validate the summary shows all operations as Allow All
