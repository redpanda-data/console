...
`;
    ...
    // Pre-create Kafka Connect internal topics
...
`;
    ...
    const backendCwd = resolve(__dirname, '../../backend/cmd/api');
    const backendConfigPath = resolve(__dirname, 'config/console.config.yaml');
    ...
    const frontendProcess = spawn('bun', ['run', 'start'], {
      cwd: frontendCwd,
      stdio: 'pipe',
    });
    ...
    writeFileSync(CONTAINER_STATE_FILE, JSON.stringify(state, null, 2));
