...
describe('CreatePipelineForm', () => {
  test('clicking submit button triggers gRPC mutation with form data', async () => {
    // Mock the gRPC service method
    const mockCreatePipeline = vi.fn(() =>
      Promise.resolve({ id: '123', name: 'my-pipeline' })
    );

    // Create router transport with mocked RPC
    const transport = createRouterTransport(({ rpc }) => {
      rpc(createPipeline, mockCreatePipeline);
    });

    // Render with mocked transport
    render(<CreatePipelineForm />, { transport });
    ...
    // Verify mutation was triggered with correct data
    await waitFor(() => {
