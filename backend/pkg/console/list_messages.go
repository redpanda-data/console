...
type ListMessageRequest struct {
	...
	MessageCount          int   // -1 enables pagination mode, 1-500 uses legacy mode
	...
	// Pagination fields (only used when MessageCount == -1)
	PageToken string
	PageSize  int // Number of messages per page (defaults to 50 if not set)
}
...
func (s *Service) ListMessages(ctx context.Context, listReq ListMessageRequest, progress IListMessagesProgress) error {
	...
	if listReq.MessageCount == -1 {
		...
		if listReq.PageToken != "" {
			...
			// First page - create initial token with page size from request (default 50)
	...
	if listReq.MessageCount == -1 && token != nil {
		// Pagination mode: use direction from token
	...
	for {
		...
		default:
			...
			for !iter.Done() {
				...
				// Stop reading from this partition when we've passed the EndOffset
				// EndOffset is inclusive, so we need to check > not >=
