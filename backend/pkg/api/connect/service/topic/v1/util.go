...
func (*Service) handleKafkaTopicPartitionError(kafkaErr error, errorMessage string) *connect.Error {
	...
	if kErr, ok := errors.AsType[*kerr.Error](kafkaErr); ok {
		return apierrors.NewConnectErrorFromKafkaErrorCode(kErr.Code, msg)
	}

	return apierrors.NewConnectErrorFromKafkaError(kafkaErr)
}
