	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, detail := range details {
				...
				if errorInfo, ok := value.(*errdetails.ErrorInfo); ok && errorInfo.Reason == tc.expectedReason {
					found = true
					// For Schema Registry errors, verify metadata is present
			...
			assert.True(t, found, "Expected to find ErrorInfo with correct reason")
		})
	}
}
