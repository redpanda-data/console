...
import (
	...
	cborsrv "github.com/redpanda-data/console/backend/pkg/cbor"
	"strings"

	"github.com/twmb/franz-go/pkg/kgo"
)

var cborDecoder = cbor.DecOptions{}

var _ Serde = (*CborSerde)(nil)

// CborSerde represents the serde for dealing with CBOR types.
type CborSerde struct {
	CborService *cborsrv.Service
}
...
func (d CborSerde) DeserializePayload(_ context.Context, record *kgo.Record, payloadType PayloadType) (*RecordPayload, error) {
	if d.CborService == nil {
		return &RecordPayload{}, fmt.Errorf("no cbor service configured")
	}

	if !d.CborService.IsTopicAllowed(record.Topic) {
	...
	if err != nil {
		return &RecordPayload{}, fmt.Errorf("decoding cbor payload: %w", err)
	}
